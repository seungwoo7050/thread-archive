...
### 이 개발 흐름에 명시적으로 연결되는 source 불변 조건 / engineering difficulty

- 불변 조건: 누적 count는 `INT_MAX`를 넘어 narrow/overflow하지 않습니다.
- 불변 조건: positive short write는 buffer를 전진시키고, `EINTR`는 retry하며, request는 `SSIZE_MAX`를 넘지 않고, non-retryable 또는 zero-byte result는 output을 error로 중단합니다.
- 불변 조건: library는 process의 `SIGPIPE` disposition을 바꾸지 않으며, 이미 OS가 받아들인 byte를 rollback할 수 없습니다.
...
- 출력 descriptor, 누적 count, sticky error의 소유권은 언제 하나의 상태로 묶이는가?
- positive short write, `EINTR`, zero progress, permanent error는 각각 state와 buffer offset을 어떻게 바꾸는가?
- `ssize_t` 결과와 public `int` return count 사이의 범위 경계는 어디에서 보장됩니까?
...
- 64-byte padding chunk가 성능만 바꾸고 공통 output 불변 조건은 바꾸지 않는다는 근거를 해당 SHA 코드와 테스트에서 제시할 수 있습니다.
- caller-owned `SIGPIPE` disposition과 library-owned error propagation의 경계를 설명할 수 있습니다.

## 4. 커밋 목록

| SHA | 제목 | 중요도 | 태그 | 원자료에서 확인된 역할 |
| --- | --- | --- | --- | --- |
...
> 원칙: 아래 기록은 최종 HEAD가 아니라 각 항목의 정확한 SHA에서 작성합니다. source가 확정하지 않은 파일명/함수명은 현재 골격에서 추측하지 않습니다.
...
- 태그: `CORE, OUTPUT`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Starts with a local write-and-count helper that rejects short writes.
- 커밋 분류 요약: Creates the public 진입점, archive, literal loop, percent escape, and initial counting.
...
### 해당 SHA에서 확인할 코드
- 해당 SHA에서 public `ft_printf` 진입점과 local write/count helper의 실제 이름과 위치를 찾습니다.
...
- 태그: `ARCH, OUTPUT, CORE`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Introduces shared descriptor, count, and sticky-error state and resumes after positive short writes.
- 커밋 분류 요약: Introduces a shared output context with descriptor, count, sticky error, and short-write progress.
...
### 학습 깊이
- 이 commit은 architecture/불변 조건의 핵심으로 취급합니다.
- 학습자 기록 — 직전 상태:
...
- 학습자 기록 — 기존 설계가 충분하지 않았던 이유:
  - 진입점 전용 helper에는 공유 가능한 sticky failure state가 없고, caller마다 count와 write 결과를 다시 처리하게 될 위험이 있었습니다.
...
- 학습자 기록 — 소유권 / lifecycle / 상태 전이:
  - caller가 stack 문맥을 생성하고 initializer가 세 field를 설정합니다. output API만 `count`와 `error`를 mutate합니다. positive result마다 pointer와 remaining length를 진행시키며, failure 후 `error == 1`은 이후 write를 차단합니다.
- 학습자 기록 — failure scenario와 public consequence:
  - 이 SHA의 API 자체는 `written <= 0` 또는 count overflow에서 `error = 1`, `-1`을 반환합니다. 다만 이 commit 시점의 `ft_printf`는 아직 새 문맥을 호출하지 않으므로 public entry와의 통합은 다음 SHA에서 이루어집니다.
...
- 학습자 기록 — 아직 보장하지 않는 것:
  - `EINTR` 재시도, `SSIZE_MAX` request cap, `ssize_t`를 `int`로 narrow하기 전 개별 범위 검사, 실제 진입점 migration은 아직 없습니다.
...
- 부모 커밋 SHA와 diff하여 이전 local state가 제거/대체된 정확한 코드 지점을 기록합니다.
- 학습자 기록 — 실제 파일 경로 / 함수 / 구조체 / branch:
...
- 학습자 기록 — 직전 관련 SHA와 비교한 핵심 diff:
  ...
  - 이후: 별도 private 문맥이 fd/count/error를 소유하고, `ft_printf_write`가 positive result만큼 suffix를 전진시킵니다. 진입점 migration은 아직 일어나지 않았습니다.

### 커밋 설명
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - formatter 전반이 공유할 output 상태 머신의 핵심을 도입한 commit입니다. descriptor, 누적 count, sticky error를 하나의 private 문맥에 모으고 short write를 정상 progress로 바꾸었지만, 이 시점에는 새 API가 아직 public loop에 연결되지 않았고 POSIX interruption/range 세부 규칙도 미완성입니다.
...
- 태그: `OUTPUT, REFACTOR`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Migrates literal output to the shared context, eliminating duplicate accounting.
- 커밋 분류 요약: Migrates literal and percent output to the shared context.
...
- 학습자 기록 — 해당 SHA에서 확인한 핵심 상태/flow 변화:
  - `ft_printf`가 stack `t_printf ctx`를 초기화하고 traversal 동안 같은 문맥을 전달합니다. loop 종료 뒤 `ctx.error`이면 `-1`, 아니면 `ctx.count`를 반환합니다.
- 학습자 기록 — 이후 commit이 보강하거나 대체하는 부분:
  - 이후 conversion dispatch도 이 문맥을 공유하며, output loop의 range/EINTR/request 정책은 후속 fix에서 강화됩니다.

### 해당 SHA에서 확인할 코드
- 해당 SHA의 `ft_printf`에서 literal/escaped-percent가 shared output API를 호출하는 지점을 찾습니다.
- 진입점이 format traversal, variadic traversal init/close, final context-to-public-result translation만 담당하는지 실제 코드로 확인합니다.
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - 새 architecture를 실제 public 경로에 연결한 integration commit입니다. 진입점에서 중복 count/write 처리를 없애고, 모든 현재 출력이 같은 문맥의 count와 sticky error를 통과하도록 만들었습니다.

## 5.4 `c627bd1f85bb` — fix(output): 쓰기 결과를 집계하기 전에 범위 검증

- 중요도: `A`
- 태그: `OUTPUT, RISK, DEBUG`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Validates a wide `ssize_t` result before narrowing it into the public `int` count.
- 커밋 분류 요약: Rejects a write result wider than int before narrowing and adding it.
- Importance 근거: The small fix restores the public count 불변 조건 at the exact conversion boundary and avoids implementation-defined narrowing. Its impact is significant despite the one-line diff.

### 학습 깊이
- 이 commit은 주요 subsystem/boundary/실패 처리/integration point 수준으로 추적합니다.
- 학습자 기록 — 직전 상태와 문제:
...
- 수정된 decision/불변 조건: 각 `written`이 `int`에 representable한지 먼저 확인하고, 그 다음 accumulated sum을 `INT_MAX`에 대해 검증
- 학습자 기록 — 실제 수정 코드:
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - 한 줄이지만 public return 불변 조건을 지키는 type boundary fix입니다. wide syscall result를 좁힌 뒤 검사하던 순서를 뒤집어, representable한 성공값만 `int` count에 반영합니다.
...
- 태그: `OUTPUT, CORE, RISK`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Caps requests at `SSIZE_MAX`, retries `EINTR`, and creates a deterministic write seam.
- 커밋 분류 요약: Caps write requests at SSIZE_MAX, retries EINTR, and exposes a deterministic write test seam.
- Importance 근거: This completes the project-defining POSIX output policy beyond simple short-write handling. It establishes progress, interruption, and request-range 불변 조건 used by every conversion and enables direct failure-path proof.
...
- 학습자 기록 — 직전 관련 SHA와 비교한 핵심 diff:
  ...
  - 이후: bounded request와 `errno == EINTR` retry가 positive/terminal branch보다 앞서며, build macro가 호출 지점만 test seam으로 바꿉니다.
...
- 수정된 decision/불변 조건: `SSIZE_MAX` request cap + `EINTR` transparent retry + nonpositive non-`EINTR` sticky failure + deterministic seam
- 학습자 기록 — 실제 수정 코드:
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - POSIX write boundary를 완성한 핵심 fix입니다. “성공 byte만 state를 진행시키고, interruption은 그대로 재시도하며, progress 불가능한 결과만 terminal error”라는 규칙과 이를 검증할 seam을 같은 호출 지점에 고정했습니다.
...
- 태그: `OUTPUT, PERF`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Emits wide padding in bounded chunks rather than one system call per byte.
- 커밋 분류 요약: Emits repeated padding through bounded 64-byte chunks instead of one write per character.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - 성능 모델만 O(width) one-byte calls에서 약 `ceil(width/64)` shared writes로 바뀌고, count/error/progress는 기존 문맥에서 동일하게 처리됩니다.
- 학습자 기록 — failure 또는 경계 조건:
  - negative/zero length는 loop를 실행하지 않습니다. 마지막 chunk는 64보다 작을 수 있으며, 어느 chunk에서든 shared write가 실패하면 즉시 `-1`입니다.
- 학습자 기록 — 보장하는 것 / 보장하지 않는 것:
  - 보장: stack 사용량은 width와 무관하게 64 bytes로 제한되고, 각 요청은 64 이하이며 공통 실패 처리를 유지합니다.
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - output 불변 조건을 건드리지 않고 padding producer의 단위만 키운 성능 개선입니다. 고정 크기 stack buffer이므로 width가 커져도 메모리 사용은 증가하지 않고, failure/count는 기존 상태 머신을 그대로 통과합니다.
...
- 태그: `OUTPUT, TEST, RISK`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Scripts partial progress, interruption, zero writes, `EPIPE`, and verifies `SIGPIPE` and chunking policy.
- 커밋 분류 요약: Injects partial writes, EINTR, EPIPE, zero progress, and verifies SIGPIPE and padding chunks.
- Importance 근거: This provides deterministic evidence for the S-level output 상태 머신 and confirms that the library does not mutate process signal policy. It is unusually strong failure-path verification.

### 학습 깊이
- 이 commit은 주요 subsystem/boundary/실패 처리/integration point 수준으로 추적합니다.
- 학습자 기록 — 직전 상태와 문제:
  - retry/chunk 정책은 구현돼 있었지만 실제 kernel timing으로는 원하는 partial/EINTR/EPIPE 순서를 재현하기 어렵고, SIGPIPE 소유권과 accepted-prefix 보존을 명시적으로 증명하지 못했습니다.
- 학습자 기록 — 설계 판단 / boundary 변화:
  - production source를 `FT_PRINTF_TEST_WRITE`로 다시 compile한 별도 fault binary와 실제 archive를 쓰는 normal test를 함께 둡니다.
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - production code는 바뀌지 않습니다. 테스트가 scripted step, call count, 최대 request, accepted output을 관찰 가능한 상태로 만들어 기존 불변 조건을 고정합니다.
- 학습자 기록 — failure 또는 경계 조건:
  - PART→ALL, EINTR/PART/EINTR/ALL, immediate EPIPE, PART→EPIPE, zero result, width 1000, real broken pipe를 각각 분리합니다.
- 학습자 기록 — 보장하는 것 / 보장하지 않는 것:
  - 보장: 지정한 deterministic sequence에서 offset/retry/stop/chunk 동작과 caller handler 보존이 assertion과 일치해야 suite가 통과합니다.
...
### 테스트 커밋 분석
- production 불변 조건 대상: positive short write progress, `EINTR` retry, zero/permanent failure stop, `SIGPIPE` 소유권 boundary, 64-byte padding chunk policy
...
- 후속 회귀 방지 역할: output loop, retry policy, padding optimization이 바뀌어도 동일 상태 전이와 signal boundary를 유지하도록 막습니다.
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 현재 실행 환경에서 GitHub host DNS/checkout이 차단되어 해당 SHA source tree를 로컬로 구성할 수 없었습니다.
  - result: 실행 결과를 주장하지 않습니다. `1223518652bd`의 Makefile과 두 test source는 해당 커밋 diff/code inspection으로 확인했습니다.

### 커밋 설명
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - 드문 write 결과를 운에 맡기지 않고 production call boundary에 순서대로 주입하는 회귀 suite입니다. accepted bytes와 요청 단위까지 기록해 상태 전이를 검증하며, 실제 broken pipe에서는 library가 caller의 SIGPIPE disposition을 바꾸지 않는다는 별도 각 부분이 맡는 일의 구분도 확인합니다.

## 6. 불변 조건 ledger

원자료에서 확인된 변화 축을 아래에 배치했습니다. “실제 코드 근거”는 학습자가 해당 SHA를 읽고 채웁니다.

| 불변 조건 / concern | 도입 또는 초기 상태 | 강화 / 수정 | 고정한 검증 | 실제 코드 근거 |
| --- | --- | --- | --- | --- |
...
- source가 명시한 불변 조건 범위 안에서만 필요한 행을 추가합니다. 새 불변 조건을 확정 사실처럼 만들지 않습니다.
- 추가 기록:
  - 별도 행 추가는 필요하지 않습니다. 이미 네 행이 이 개발 흐름의 source-defined count/progress/cost/signal 축을 모두 포함합니다.

## 7. 실패 → 수정 → 검증 연결

| 기존 failure / risk | 수정 또는 변경 | 수정 decision | Test / 학습 확인 |
| --- | --- | --- | --- |
...
| 시점 | 원자료 기준 owner / boundary | 원자료 기준 responsibility 변화 | 해당 SHA 코드 근거 |
| --- | --- | --- | --- |
...
- 원자료에서 확인된 도달점: 모든 conversion이 공유하는 output 상태 머신에서 count range, progress, interruption, permanent failure, syscall cost, signal policy가 명시되고 별도로 검증된 상태입니다.
...
- [x] 커밋 목록의 모든 SHA를 정확한 시점의 코드로 확인했습니다.
- [x] 각 commit의 subject, importance, tags를 source와 그대로 유지했습니다.
- [x] 최종 HEAD의 코드를 과거 commit 설명에 소급 사용하지 않았습니다.
- [x] 필요한 parent/직전 관련 SHA 비교를 실제 diff로 수행했습니다.
- [x] 원자료에서 확인된 사실과 내가 코드에서 확인한 사실을 구분했습니다.
- [x] fix의 기존 가정 → failure/risk → root cause → decision → code → test 연결을 필요한 곳에서 완성했습니다.
- [x] test commit의 target 불변 조건, technique, production path, proves/not-proves를 구분했습니다.
- [x] 불변 조건 ledger에 실제 코드 근거를 채웠습니다.
- [x] 이 개발 흐름의 최종 architecture/execution flow를 commit history 순서로 설명할 수 있습니다.
...
raw format text를 `t_format`으로 정규화하고, main traversal, typed `va_arg` dispatch, conversion renderer 사이의 각 부분이 맡는 일의 구분이 형성되는 과정을 복원합니다.
...
- 불변 조건: `t_format`은 parser에서 measurement와 rendering으로 전달되는 normalized field representation입니다.
- 불변 조건: 가변 인자는 conversion이 정의한 promoted type과 호환되는 방식으로 소비되어야 합니다.
- 불변 조건: format grammar와 overflow 동작은 main loop, 이후 measurement, renderer 사이에서 일관되어야 합니다.
- Engineering difficulty: normalized field grammar와 overflow 동작을 여러 소비자 사이에서 일관되게 유지하고, specifier마다 정확한 promoted type을 소비하도록 책임을 분리하는 문제입니다.

## 2. 이 개발 흐름을 이해하기 위한 핵심 질문

- `t_format`에는 어떤 상태가 저장되고 omitted precision과 `.0`은 어떻게 구분됩니까?
...
- decimal, hex, pointer conversion은 기존 parser/dispatch/output boundary 안에 어떻게 추가됩니까?
...
- `-`/`0`, `+`/space 충돌과 signed/alternate prefix 선택이 어느 각 부분이 맡는 일의 구분에서 해결되는지 설명할 수 있습니다.
...
- 태그: `ARCH, PARSER, CORE`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Establishes the normalized `t_format` representation and overflow-checked field parser.
- 커밋 분류 요약: Defines t_format and parses flags, width, precision, and specifiers with decimal overflow checks.
...
- precision omitted와 `.0`을 `has_precision`로 구분하는 상태 전이를 확인합니다.
...
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Connects field parsing to the main format traversal.
- 커밋 분류 요약: Connects parsed fields to the main traversal with temporary 대체 처리 output.
...
- 이 commit은 Thread 흐름에서 맡는 구현 역할과 필요한 state/code 변화에 집중합니다.
- 학습자 기록 — 직전 상태 대비 필요한 변화:
  - parser가 archive에는 들어갔지만 호출되지 않아 normalized field와 overflow failure가 public 동작에 영향을 주지 않았습니다.
- 학습자 기록 — 이 commit이 맡는 구현 책임:
  - `%`를 만난 main loop가 `ft_printf_parse(format + 1, &fmt)`를 호출하고, 성공 시 반환 cursor를 다음 traversal 위치로 사용합니다.
- 학습자 기록 — 해당 SHA에서 확인한 핵심 상태/flow 변화:
  - null parser result는 `ctx.error = 1` 후 loop를 중단합니다. 성공 field는 임시 대체 처리로 `%`와 specifier를 다시 출력하며, 기존 special `%%` branch는 parser보다 먼저 처리됩니다.
- 학습자 기록 — 이후 commit이 보강하거나 대체하는 부분:
...
- parse failure가 output 문맥의 sticky error로 승격되는 path를 확인합니다.
...
- 중요도: `A`
- 태그: `ARCH, FORMAT, VARARGS`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Introduces a dispatcher that owns `va_arg` type selection and routes to conversion renderers.
- 커밋 분류 요약: Introduces conversion dispatch and concrete c, s, and percent renderers.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - `c`는 default promotion에 맞춰 `int`, `s`는 `char *`를 소비하고 `%`는 argument를 소비하지 않습니다. renderer는 모두 같은 output 문맥을 사용합니다.
- 학습자 기록 — failure 또는 경계 조건:
  - null string은 renderer에서 `(null)`로 치환됩니다. unknown/trailing specifier는 아직 `%`와 spec을 literal로 쓰는 대체 처리가므로 final validity rejection은 아닙니다.
- 학습자 기록 — 보장하는 것 / 보장하지 않는 것:
...
### 해당 SHA에서 확인할 코드
- conversion dispatcher의 실제 함수와 main loop 호출 지점을 찾습니다.
...
- unknown field가 아직 prior literal 대체 처리를 유지하는 branch를 찾아 final syntax 검증이 아직 아님을 기록합니다.
- 진입점에 type-specific `va_arg`가 남아 있는지 diff로 확인합니다.
- 학습자 기록 — 실제 파일 경로 / 함수 / 구조체 / branch:
  - `src/ft_printf.c`: parse 성공 뒤 `ft_printf_dispatch(&ctx, &fmt, &args)`만 호출합니다.
  - `src/ft_dispatch.c`: `c`, `s`, `%`, 대체 처리 branches와 두 `va_arg` call.
...
- 태그: `FORMAT, VARARGS`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Adds signed and unsigned decimal conversions inside that boundary.
- 커밋 분류 요약: Adds d, i, and u dispatch and decimal digit emission.
- Importance 근거: This is normal core-feature implementation within the established dispatch and output model.

### 학습 깊이
- 이 commit은 Thread 흐름에서 맡는 구현 역할과 필요한 state/code 변화에 집중합니다.
- 학습자 기록 — 직전 상태 대비 필요한 변화:
  - dispatch와 text conversions만 있어 `d`, `i`, `u`가 대체 처리로 처리됐고 numeric argument를 올바른 type으로 소비하지 않았습니다.
- 학습자 기록 — 이 commit이 맡는 구현 책임:
...
- 태그: `FORMAT, VARARGS`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Adds hexadecimal and pointer conversions inside the same boundary.
- 커밋 분류 요약: Adds x, X, and p formatting with uintptr_t conversion.
...
- 중요도: `A`
- 태그: `PARSER, FORMAT, LAYOUT`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Normalizes conflicting flags once and applies signed and alternate-form prefixes.
- 커밋 분류 요약: Normalizes conflicting flags and adds signed and alternate-form prefixes.
...
| 기존 failure / risk | 수정 또는 변경 | 수정 decision | Test / 학습 확인 |
| --- | --- | --- | --- |
| width/precision decimal accumulation overflow | `7984ddf2dd57` | `INT_MAX` 기준 pre-multiplication check로 field parse 실패 처리 | 이 개발 흐름에서는 parser code를 확인하고 whole-call no-output 검증은 Thread 5에서 다시 추적 |
...
- 원자료에서 확인된 도달점: parsing, argument extraction, rendering이 분리되고, normalized field와 typed dispatch가 공통 boundary가 되며 parser-side flag normalization이 renderer의 conflicting states를 줄인 상태입니다.
- 학습자 기록 — 마지막 commit 기준 실제 코드에서 확인한 최종 state:
  - main loop는 전체 cursor/variadic 수명/output result를 관리하고, parser는 overflow-checked `t_format`과 next cursor를 만들며, parser 끝에서 conflict flags를 제거합니다. dispatch는 specifier별 promoted type을 정확히 소비하고 text/decimal/hex renderer에 전달합니다.
- 학습자 기록 — 이 Thread 밖에서만 해결되는 남은 문제를 source 범위 안에서 구분:
...
- failure가 끊기는 지점:
  - decimal overflow로 parser가 null이면 main이 `ctx.error`를 설정합니다. renderer/output 실패는 dispatch return `< 0`으로 main loop를 끊습니다. 이 Thread 시점의 unknown 대체 처리는 실패로 끊기지 않습니다.
- 후속 fix/test가 보장한 지점:
...
초기 decimal/hex 구현은 spaces, prefixes, field zeros, precision zeros, digits, trailing padding 순서를 중복 구현합니다. shared layout은 이 순서를 하나의 불변 조건으로 만들고, 이후 portability 및 boundary 작업이 signed endpoint와 프로젝트 고유 pointer semantics에서도 이 공통 모델을 유지합니다.

### 이 개발 흐름에 명시적으로 연결되는 source 불변 조건 / engineering difficulty

- 불변 조건: numeric output은 decimal, hexadecimal, pointer 전반에서 spaces, prefixes, field zeros, precision zeros, digits, trailing spaces를 일관된 순서로 출력합니다.
- 불변 조건: measurement와 rendering은 prefix, zero suppression, precision zeros, field width를 포함한 effective length에 동의해야 합니다.
...
- spaces, prefix, field zeroes, precision zeroes, digits, trailing spaces의 정확한 순서는 각 단계에서 어떻게 표현됩니까?
- precision이 명시되면 field-level `0` flag가 왜/어디서 무효화됩니까?
- zero value + precision zero의 digit suppression과 alternate-form prefix는 어떻게 결합됩니까?
...
- 태그: `FORMAT, LAYOUT`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Adds prefix-aware width and alignment to decimal output.
- 커밋 분류 요약: Builds decimal digit buffers and applies prefix-aware width and alignment.
...
- 태그: `FORMAT, LAYOUT`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Repeats the model for hexadecimal and pointer output.
- 커밋 분류 요약: Applies width and alignment to hexadecimal and pointer output.
...
- 중요도: `A`
- 태그: `FORMAT, LAYOUT, RISK`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Adds zero suppression, precision zeros, and zero-field padding rules.
- 커밋 분류 요약: Adds zero suppression, precision zeros, and zero-padding order for decimal and hexadecimal output.
...
- 태그: `PARSER, FORMAT, LAYOUT`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Establishes prefix selection and flag precedence.
- 커밋 분류 요약: Normalizes conflicting flags and adds signed and alternate-form prefixes.
- Importance 근거: Resolving '-' over '0' and '+' over space at the parser boundary simplifies every renderer, while nonzero-only hexadecimal prefixes fix public semantics. This is significant cross-conversion judgment.
...
- 태그: `FORMAT, TEST, EDGE`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Locks down representative prefix, precision, zero, and left-alignment interactions.
- 커밋 분류 요약: Adds focused prefix, precision, zero, and alignment regression cases.
...
### 테스트 커밋 분석
- production 불변 조건 대상: numeric prefix와 field/precision zero, left alignment의 ordering
...
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Extracts one shared numeric layout writer for decimal, hexadecimal, and pointer conversions.
- 커밋 분류 요약: Extracts one numeric layout writer shared by decimal, hexadecimal, and pointer conversions.
...
### 해당 SHA에서 확인할 코드
- 부모 커밋 SHA에서 decimal과 hex가 각각 갖고 있던 placement sequence를 나란히 비교합니다.
...
- 태그: `FORMAT, EDGE, RISK`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Removes signed-overflow dependence when formatting `INT_MIN`.
- 커밋 분류 요약: Forms negative magnitude as -(value + 1) + 1 before unsigned conversion.
...
- sign selection과 shared decimal layout은 바뀌지 않았는지 diff로 확인하여 fix 범위를 분리합니다.
- 학습자 기록 — 실제 파일 경로 / 함수 / 구조체 / branch:
...
- 수정된 decision/불변 조건: `-(value + 1)`의 representable result를 unsigned로 변환하고 마지막 1을 unsigned domain에서 더함
- 학습자 기록 — 실제 수정 코드:
...
- 태그: `FORMAT, TEST, EDGE`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Expands public boundary matrices for zero precision, prefixes, null pointers, and narrow fields.
- 커밋 분류 요약: Locks down zero precision, prefixes, null values, percent extensions, and width/precision boundary matrices.
...
- 학습자 기록 — 다음 관련 commit 연결:
  - 이 개발 흐름의 마지막 test입니다. 이후 release/sanitizer 검증은 같은 functional suite를 산출물·runtime 계층에서 다시 실행하지만 numeric contract 자체를 재정의하지 않습니다.
...
### 테스트 커밋 분석
- production 불변 조건 대상: public formatting boundary에서 zero precision, prefixes, null values, percent extension, width/precision edge semantics 유지
...
- 이 test가 증명하지 않는 것: system-call retry 상태 머신이나 archive symbol/dependency boundary까지 증명하지 않습니다.
- 분류: broad boundary regression이며 일부는 deterministic fixed-contract regression입니다.
- 후속 회귀 방지 역할: numeric/text edge semantics와 deliberate project extensions가 후속 refactor에서 libc 동작과 혼동되어 바뀌는 것을 막습니다.
- 학습자 기록 — 실제 test 함수/fixture/seam/assertion:
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout을 로컬에 구성할 수 없어 connector로 commit patch와 당시 test implementation을 검사했습니다.
...
- 원자료에서 확인된 도달점: numeric placement가 하나의 shared layout 불변 조건으로 수렴하고, signed endpoint 및 project-specific pointer/precision boundary까지 후속 fix와 tests로 보강된 상태입니다.
- 학습자 기록 — 마지막 commit 기준 실제 코드에서 확인한 최종 state:
...
- failure가 끊기는 지점:
  - shared layout의 각 `ft_printf_putnchar`/`ft_printf_write` 반환을 즉시 검사해 후속 component를 중단합니다. output 문맥의 sticky error가 최종 public `-1`을 만듭니다.
- 후속 fix/test가 보장한 지점:
...
# 문자열 정밀도: 출력 길이 제한에서 메모리 접근 제한까지
...
- 불변 조건: `%s`는 precision이 있을 때 그 limit보다 더 멀리 읽지 않으며, 그 readable range 밖의 NUL terminator를 요구하지 않습니다.
...
- 태그: `FORMAT, EDGE`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Initially computes the full string length and truncates the emitted length afterward.
- 커밋 분류 요약: Adds string truncation and zero padding for the project's percent extension.
...
- 학습자 기록 — 실제 파일 경로 / 함수 / 구조체 / branch:
  ...
  - 같은 파일의 percent renderer에서 `pad_char`가 ZERO/LEFT flags로 선택됩니다. 이 개발 흐름의 memory risk는 string branch에 한정됩니다.
- 학습자 기록 — 필요한 최소 코드 발췌:
```c
...
- 태그: `FORMAT, EDGE, RISK`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Moves the precision bound into the scan itself so memory beyond the permitted object range is never read.
- 커밋 분류 요약: Stops the string scan at precision instead of finding NUL first and truncating afterward.
...
- 수정된 decision/불변 조건: NUL 또는 precision 중 먼저 도달하면 scan을 멈추고 같은 bounded length를 width/output에 사용
- 학습자 기록 — 실제 수정 코드:
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - `%s` precision의 의미를 output-size 조정에서 caller memory access bound로 바로잡은 root-cause fix입니다. width와 write가 사용하는 길이도 같은 bounded scan 결과가므로 관찰되는 출력과 실제 읽기 범위가 일치합니다.
...
- 태그: `FORMAT, TEST, EDGE`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Uses a non-NUL-terminated three-byte object to prove that `%.3s` requires only those three readable bytes.
- 커밋 분류 요약: Formats a three-byte non-NUL array with a matching string precision.
...
### 테스트 커밋 분석
- production 불변 조건 대상: `%s` precision이 memory access와 emitted length를 같은 bound로 제한
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout이 로컬에서 불가능해 connector로 test와 production 코드를 검사했습니다.
...
- 학습자가 해당 SHA 코드와 필요한 비교 SHA를 읽은 뒤, 이 commit을 자신의 말로 설명합니다.
- 최종 설명:
  - NUL-terminated 입력으로는 드러나지 않는 bounded-read 불변 조건을 세 byte non-NUL object로 고정한 focused regression입니다. functional comparison은 visible contract를 확인하고, sanitizer와 결합하면 범위 밖 읽기의 관찰력이 강화됩니다.
...
- source가 명시한 불변 조건 범위 안에서만 필요한 행을 추가합니다. 새 불변 조건을 확정 사실처럼 만들지 않습니다.
- 추가 기록:
  - 추가 행은 만들지 않았습니다. null-string representation과 general width semantics는 이 개발 흐름의 bounded-access 변화와 분리했습니다.
...
- 원자료에서 확인된 도달점: string precision이 emitted-byte 제한과 memory-access 제한을 동시에 정의하고, precision 범위 밖 NUL terminator를 요구하지 않는 상태입니다.
- 학습자 기록 — 마지막 commit 기준 실제 코드에서 확인한 최종 state:
...
한 field의 local parse 검증만으로는 뒤쪽 invalid field가 있을 때 public no-output guarantee를 만들 수 없습니다. 두 번째 pass가 architecture를 바꾸어 format/length error는 preflight failure가 되고, device error만 partial external output을 남길 수 있게 됩니다.

### 이 개발 흐름에 명시적으로 연결되는 source 불변 조건 / engineering difficulty

- 불변 조건: unsupported specifier, unterminated field, field-number overflow, total result > `INT_MAX`는 output 없이 `-1`을 반환합니다.
- 불변 조건: 시작되거나 copy된 각 `va_list`는 독립적으로 traversal되고 정확히 한 번 종료되며 measurement와 output은 호환되는 promoted type으로 argument를 소비합니다.
- 불변 조건: measurement와 rendering은 prefixes, zero suppression, precision zeros, field width를 포함한 effective length에 동의해야 합니다.
...
- format/length error와 runtime device/write error의 atomicity 경계는 어디입니까?
- late invalid specifier나 total `INT_MAX` overflow test는 “renderer가 도달해서 실패”하는 구현과 어떻게 구별됩니까?

## 3. 완료 기준

- `7984ddf2dd57`의 local field overflow validation과 `2d773acc5bd6`의 whole-call preflight 차이를 설명할 수 있습니다.
- `ft_printf`의 두 traversal과 모든 `va_start`/`va_copy`/`va_end` 소유권을 success/failure path별로 기록할 수 있습니다.
...
- 태그: `ARCH, PARSER, CORE`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Rejects decimal width or precision values that overflow `int` while parsing one field.
- 커밋 분류 요약: Defines t_format and parses flags, width, precision, and specifiers with decimal overflow checks.
- Importance 근거: The parser creates the durable representation through which all conversions and both later passes communicate. It is indispensable to explaining the formatter's grammar and field-processing architecture.
...
- 학습자 기록 — 기존 설계가 충분하지 않았던 이유:
  - conversion별 raw parsing은 grammar와 overflow 동작을 중복시키고, width/precision을 이미 overflow한 뒤 검사하면 잘못된 state/cursor를 만들 수 있습니다.
- 학습자 기록 — 선택한 핵심 decision:
...
- 중요도: `A`
- 태그: `FORMAT, TEST, VERIFY`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Adds parser-boundary tests and a differential harness, but errors later in a format can still follow earlier output.
- 커밋 분류 요약: Creates stdout capture, libc differential checks, fixed expectations, and parser-overflow tests.
...
- 학습자 기록 — 설계 판단 / boundary 변화:
  - stdout을 pipe로 redirect해 `ft_printf` bytes를 캡처하고, portable supported cases는 `snprintf`, repository-specific null/pointer 동작은 explicit output과 비교합니다.
- 학습자 기록 — 핵심 state/불변 조건 변화:
...
```
- 학습자 기록 — 직전 관련 SHA와 비교한 핵심 diff:
  - 이전: repository 안에서 전체 public 처리 단계를 재현하는 automated harness가 없었습니다.
  - 이후: broad differential/fixed 검증이 생겼지만 parser overflow cases는 invalid field가 출력의 첫 위치에 있어 late-error atomicity를 구별하지 않습니다.

### 테스트 커밋 분석
- production 불변 조건 대상: supported formatting의 exact bytes, captured byte count, public return count 및 parser field-overflow behavior
...
- 통과하는 production path: public `ft_printf` 전체 conversion 처리 단계
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA의 repository checkout을 로컬에 확보하지 못해 GitHub connector로 Makefile/test/production diff를 검사했습니다.
...
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Adds a `va_copy` measurement pass that validates every field and the total result before any write.
- 커밋 분류 요약: Adds a va_copy measurement pass that validates grammar and total int length before output.
...
- 이 commit은 architecture/불변 조건의 핵심으로 취급합니다.
- 학습자 기록 — 직전 상태:
  - `ft_printf`는 original `va_list` 하나로 parse→dispatch→write를 반복했습니다. unsupported specifier는 dispatcher 대체 처리로 literal처럼 출력될 수 있었고, late parse/length failure는 earlier bytes가 이미 external fd에 전달된 뒤 발견될 수 있었습니다.
- 학습자 기록 — 해결하려던 문제:
...
- 학습자 기록 — 선택한 핵심 decision:
  - `va_start(args)` 뒤 `va_copy(measure_args, args)`를 만들고 `ft_printf_measure`로 whole format을 순회합니다. 성공한 경우에만 original `args`와 output 문맥으로 기존 rendering loop를 실행합니다.
- 학습자 기록 — 소유권 / lifecycle / 상태 전이:
...
- 학습자 기록 — failure scenario와 public consequence:
  - parser null, unsupported/incomplete specifier, conversion measurement failure, component/total `INT_MAX` overflow는 measurement에서 `-1`; output 문맥이 아직 초기화되거나 호출되지 않아 emitted byte 0입니다. preflight 성공 뒤 `write` 실패는 이미 accepted bytes가 남을 수 있고 public return만 `-1`입니다.
- 학습자 기록 — 이 SHA가 보장하는 것:
  - stable input과 defined 가변 인자를 전제로 grammar/length failure는 첫 write 전 판정되고, measurement/rendering은 specifier별 호환되는 promoted type으로 독립 소비됩니다.
- 학습자 기록 — 아직 보장하지 않는 것:
...
- 성공/실패 모든 path에서 copied/original variadic traversal의 `va_end`가 balanced되는지 소유권 table로 기록합니다.
...
- 수정된 decision/불변 조건: copied `va_list` measurement pass로 전체 grammar/argument length를 먼저 검증하고, 성공 후 original `va_list`로 output
- 학습자 기록 — 실제 수정 코드:
...
- 태그: `ATOMIC, TEST, RISK`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Verifies that late unsupported fields and unrepresentable total lengths fail with zero emitted bytes.
- 커밋 분류 요약: Verifies late invalid fields and INT_MAX length failures produce no bytes.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - production state는 바뀌지 않습니다. test는 preflight가 entire format을 통과하기 전에는 rendering pass가 시작되지 않는다는 observable 불변 조건을 고정합니다.
- 학습자 기록 — failure 또는 경계 조건:
...
- 학습자 기록 — 다음 관련 commit 연결:
  - 이 개발 흐름의 마지막 직접 regression입니다. 후속 functional matrices와 release/sanitizer targets가 same entry를 실행하지만 no-output contract를 새로 정의하지 않습니다.
...
### 테스트 커밋 분석
- production 불변 조건 대상: format/length error는 whole-call preflight에서 첫 output 전에 실패해야 함
...
- 학습자 기록 — 실제 test 함수/fixture/seam/assertion:
  - macro는 16-byte capture buffer면 충분합니다. 불변 조건이 맞으면 output은 0이고, 틀린 single-pass implementation이라도 prefix 몇 byte를 관찰할 수 있습니다. 모든 case는 `capture_begin` 전후 actual return/length를 직접 검사합니다.
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout을 로컬에 만들 수 없어 connector로 test macro/cases와 production preflight를 검사했습니다.
...
| atomicity boundary | format/length error는 preflight에서 external effect 이전에 판정 | runtime `write` failure는 output 문맥이 처리하지만 이미 accepted bytes rollback 불가 | 학습자가 두 failure class를 execution flow에서 분리 | preflight failure branch는 `ft_printf_init`보다 앞; rendering의 `ft_printf_write`는 positive progress를 count/offset에 반영하고 later failure 시 public `-1` |

### 학습자 추가 기록

- source가 명시한 불변 조건 범위 안에서만 필요한 행을 추가합니다. 새 불변 조건을 확정 사실처럼 만들지 않습니다.
- 추가 기록:
  - 추가 행은 만들지 않았습니다. copied/original `va_list` 소유권은 아래 responsibility 표에 구체화했습니다.
...
- 원자료에서 확인된 도달점: final 진입점이 `va_copy` measurement와 original-argument rendering의 두 traversal을 수행하여 format/length error를 output 전에 거부하고, runtime device failure만 non-atomic하게 남기는 상태입니다.
- 학습자 기록 — 마지막 commit 기준 실제 코드에서 확인한 최종 state:
...
# 실행 시점과 배포 산출물까지 확장되는 검증

## 1. 개발 흐름 목표

검증이 단순 format 결과 비교에서 deterministic syscall failure, project-specific contract, distributable archive, sanitizer runtime까지 어떻게 확장되는지 복원합니다.
...
- 불변 조건: built archive는 expected definitions와 external dependencies를 노출하고 public header만 보는 consumer와 link됩니다.
- 불변 조건: output fault 검증은 partial, interrupted, zero-progress, permanent failure를 deterministic하게 재현해야 합니다.
- 불변 조건: project-specific semantics는 portable libc oracle가 아닌 fixed expectations로 구분되어야 합니다.
- Engineering difficulty: system-call sequence를 deterministic하게 검증하면서 portable libc 동작과 formatted percent 같은 explicit project extension을 구분하고, runtime뿐 아니라 release artifact boundary까지 증거를 확장하는 문제입니다.

## 2. 이 개발 흐름을 이해하기 위한 핵심 질문

- `snprintf`를 oracle로 사용할 수 있는 case와 fixed project expectation이 필요한 case는 어떻게 구분됩니까?
- stdout capture는 emitted bytes, captured length, 반환값을 어떻게 함께 검증하는가?
- scripted writer는 nondeterministic OS behavior 없이 어떤 output 상태 전이를 재현하는가?
- release test는 source-tree 내부 성공이 아니라 어떤 archive/공개 API/dependency/consumer boundary를 검증하는가?
- UBSan과 Linux ASan 경로는 library source와 fault binary까지 실제 instrument하는가?
- 각 test layer가 증명하지 않는 범위는 무엇입니까?

## 3. 완료 기준

- 각 verification commit의 대상 불변 조건, failure/boundary, technique, production path, proved/not-proved 범위를 구분해 설명할 수 있습니다.
- libc differential test와 fixed expectation의 기준을 실제 테스트 항목으로 분리할 수 있습니다.
...
- 태그: `FORMAT, TEST, VERIFY`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Establishes byte capture, return-count comparison, and libc differential testing.
- 커밋 분류 요약: Creates stdout capture, libc differential checks, fixed expectations, and parser-overflow tests.
- Importance 근거: The harness materially changes confidence in all core conversions and return counts and becomes the basis for later regression matrices. It is significant verification rather than a defining runtime mechanism.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - production code는 바뀌지 않습니다. 검증이 textual equality뿐 아니라 embedded NUL을 포함한 captured length, exact bytes, `ft_printf` return count를 독립적으로 확인합니다.
- 학습자 기록 — failure 또는 경계 조건:
...
- 학습자 기록 — 보장하는 것 / 보장하지 않는 것:
  - 보장: 실제 실행 시 열거된 public formatting surface의 bytes/length/return과 field-at-start parser overflow 동작을 검증합니다.
...
- 학습자 기록 — 직전 관련 SHA와 비교한 핵심 diff:
  ...
  - 이후: normal conversion 처리 단계를 byte/return 기준으로 검증하지만 `write`를 대체하지 않으므로 rare syscall transition은 OS timing에 맡겨집니다.
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout을 로컬에 확보하지 못해 GitHub connector로 Makefile, 테스트 준비 코드, production diff를 검사했습니다.
...
- 태그: `OUTPUT, TEST, RISK`
- 핵심 커밋 목록: 포함
- 개발 흐름에서의 역할: Adds deterministic system-call and signal-policy verification.
- 커밋 분류 요약: Injects partial writes, EINTR, EPIPE, zero progress, and verifies SIGPIPE and padding chunks.
- Importance 근거: This provides deterministic evidence for the S-level output 상태 머신 and confirms that the library does not mutate process signal policy. It is unusually strong failure-path verification.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - production code는 바뀌지 않습니다. test layer가 requested length, call count, largest request, accepted output prefix를 관찰해 retry offset과 stop 동작을 직접 판정합니다.
- 학습자 기록 — failure 또는 경계 조건:
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout을 로컬에 구성할 수 없어 connector로 test seam, Makefile, production output loop를 검사했습니다.
...
- 태그: `FORMAT, TEST, EDGE`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Records fixed expectations for deliberate project semantics that libc cannot serve as a portable oracle for.
- 커밋 분류 요약: Locks down zero precision, prefixes, null values, percent extensions, and width/precision boundary matrices.
- Importance 근거: Several expectations are deliberate project contracts rather than portable libc behavior. Fixing them explicitly is significant for preserving the library's actual public semantics.
...
- 학습자 기록 — 직전 관련 SHA와 비교한 핵심 diff:
  ...
  - 이후: portable 동작은 matrices로 확장되고 deliberate extension/representation은 fixed exact bytes로 명시됩니다.
...
- 학습자 기록 — 직접 실행했다면 command / 환경 / 결과:
  - command: 미실행
  - environment: 해당 SHA checkout 부재로 connector를 통해 test implementation을 검사했습니다.
...
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Verifies archive members, global definitions, external dependencies, and an out-of-tree consumer.
- 커밋 분류 요약: Checks archive order, global definitions, external dependencies, and a header-only external consumer.
...
- 학습자 기록 — 핵심 state/불변 조건 변화:
  - production/archive 내용은 바뀌지 않습니다. verification contract가 source 동작에서 archive composition, global namespace, external runtime dependencies, consumer-visible packaging까지 확장됩니다.
- 학습자 기록 — failure 또는 경계 조건:
...
- consumer의 expected output/return 동작과 temporary-state cleanup assertion을 기록합니다.
- 학습자 기록 — 실제 파일 경로 / 함수 / 구조체 / branch:
...
### 테스트 커밋 분석
- production 불변 조건 대상: built archive의 members/global definitions/external dependencies/public-header-only consumer boundary
...
- 태그: `VERIFY, TEST`
- 핵심 커밋 목록: 미포함
- 개발 흐름에서의 역할: Runs normal and fault binaries under UBSan and a Linux GCC AddressSanitizer environment.
- 커밋 분류 요약: Adds UBSan targets and a Dockerized GCC AddressSanitizer path.
...
| 시점 | 원자료 기준 owner / boundary | 원자료 기준 responsibility 변화 | 해당 SHA 코드 근거 |
| --- | --- | --- | --- |
| functional suite | visible output/public return contract | byte sequence, captured count, 반환값 및 supported libc-comparable semantics | `tests/test_ft_printf.c` pipe fixture, `EXPECT_PRINTF`, `EXPECT_OUTPUT`, grouped runners |
| fault suite | output system-call boundary | scripted `write` result와 call sequence를 소유하여 상태 전이 관찰 | `tests/test_output_faults.c` global script/statistics와 `ft_printf_test_write`; Makefile seam build |
| fixed project cases | project public semantics | portable libc 동작이 아닌 explicit expectation을 test가 고정 | `run_public_contract_boundary_cases`의 null/pointer/string/percent `EXPECT_OUTPUT` calls |
...
- 원자료에서 확인된 도달점: 검증이 visible formatting에서 failure 상태 머신, project-specific semantics, archive consumer boundary, sanitizer runtime까지 계층별로 확장된 상태입니다.
- 학습자 기록 — 마지막 commit 기준 실제 코드에서 확인한 최종 state:
  - normal suite는 bytes/count/return, fault suite는 scripted syscall transitions와 real SIGPIPE 소유권, fixed cases는 repository-defined semantics, release script는 static archive/consumer packaging, sanitizer targets는 normal/fault implementation runtime diagnostics를 각각 담당합니다. 어느 한 layer도 나머지 layer의 증거를 대신하지 않습니다.
- 학습자 기록 — 이 Thread 밖에서만 해결되는 남은 문제를 source 범위 안에서 구분:
  - 실제 모든 target의 CI matrix/실행 이력, unsupported platforms, exhaustive input space, transactional device output, explicit leak proof는 scaffold와 inspected commits가 보장하지 않습니다. 이번 작업 환경에서도 commands를 실행하지 않았으므로 code-defined 검증과 runtime evidence를 구분했습니다.
...
- 핵심 상태 전이:
  - verification 대상이 visible result→system-call progress/error→oracle 소유권→archive composition/link boundary→instrumented runtime으로 확장됩니다. 각 layer는 별도 fixture와 failure condition을 갖습니다.
- failure가 끊기는 지점:
...
- 후속 fix/test가 보장한 지점:
  - deterministic seam이 retry 불변 조건을, fixed cases가 project semantics를, release script가 배포 단위를, sanitizer source builds가 실행 경로의 UB/address diagnostics를 보호합니다. 실제 pass 여부는 command 실행이 있어야만 별도 runtime evidence가 됩니다.
...
# ft_printf 개발 흐름 학습 기록

## 목적

이 디렉터리는 `commit-importance.md`와 `commit-bodies.md`에서 확정된 개발 흐름을 따라 실제 commit history와 해당 SHA의 코드를 직접 읽으며 프로젝트의 설계, 구현, 실패 처리, 수정, 검증 과정을 복원하기 위한 학습 골격입니다.

완성형 해설서가 아닙니다. 각 Thread 문서는 source에서 이미 확정된 제목, significance, SHA, subject, importance, tags, commit 관계와 역할만 미리 제공합니다. 실제 함수 동작, 변경 전후 코드, 소유권/수명, 실패 처리, test 결과, 최종 설명은 학습자가 repository의 정확한 SHA를 확인한 뒤 채웁니다.
...
이 순서는 `commit-importance.md`의 개발 흐름s 순서를 그대로 따릅니다. 동일 commit이 여러 Thread에 등장하는 경우 의도적인 중복입니다. 각 Thread에서 서로 다른 학습 관점으로 다시 확인합니다.

## Thread 문서 사용법

- 먼저 개발 흐름 목표, 핵심 질문, 완료 기준을 읽습니다.
- 커밋 목록의 순서를 바꾸지 않고 각 SHA를 차례로 checkout 또는 `git show`로 확인합니다.
...
- test commit은 production 불변 조건과 failure/boundary, test technique, 통과하는 production path를 분리해서 기록합니다.
- Thread가 끝나면 불변 조건 ledger와 실패 → 수정 → 검증 연결을 실제 코드 근거로 완성합니다.
- 마지막에 개발 흐름의 최종 상태와 architecture/execution flow를 자신의 설명으로 정리합니다.
...
최종 HEAD의 구현을 과거 commit의 정답처럼 소급해서 사용하지 않습니다. 후속 refactor/fix에서 함수 경계, state representation, failure 동작이 바뀔 수 있으므로 각 학습 기록은 반드시 해당 SHA 시점의 코드에 근거해야 합니다.

## Importance별 학습 깊이

- `S`: project-defining architecture/invariant입니다. 직전 상태, problem, 기존 설계의 한계, failure possibility, 핵심 decision, 실제 핵심 코드, 소유권/lifecycle/상태 전이, 후속 fix/test까지 깊게 추적합니다.
- `A`: 주요 subsystem, responsibility boundary, 실패 처리, integration point입니다. 핵심 코드와 설계 판단, 경계 조건, 보장/미보장 범위를 확인합니다.
- `B`: Thread 흐름에서 맡는 구현 역할과 필요한 코드/state 변화까지 확인합니다. S/A와 같은 분석란을 기계적으로 반복하지 않습니다.
- `C`: Thread 이해에 필요한 맥락만 확인합니다. 이 source의 개발 흐름s에는 C-level commit이 포함되어 있지 않지만, 학습 원칙은 동일합니다.
...
- 소유권/lifecycle은 선언만 보지 말고 생성/초기화, 전달, mutation, cleanup/종료 지점을 함께 확인합니다.
- 실패 처리는 정상 경로와 별도로 branch 조건, state mutation, 이후 호출 억제, public consequence를 추적합니다.
...
- 모든 개발 흐름 문서를 source 순서대로 완료했습니다.
...
- fix마다 가능한 범위에서 기존 가정 → failure/risk → root cause → 수정 decision → 실제 코드 → 회귀 테스트 연결을 완성했습니다.
- test commit마다 production 불변 조건과 test technique, production path, proved/not-proved 범위를 구분했습니다.
- 불변 조건 ledger와 실패 → 수정 → 검증 표를 실제 코드 근거로 완성했습니다.
