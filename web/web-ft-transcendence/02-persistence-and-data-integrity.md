...
# 개발 흐름 01 — 저장소 추상화·백엔드 동작 일치·조회 모델
...
- 제외: 세션 보안, 경기 결과 원자적 결과 확정, 채팅 영속 저장, 토너먼트 진행, 관리자 감사 원자성은 독립 개발 흐름/카테고리에서 다룹니다.
- 제외: 실제 WebSocket 접속 여부는 저장된 사용자 목록이 아니라 현재 실시간 연결 상태로 판단하므로 이 개발 흐름의 동작 일치 결론으로 일반화하지 않습니다.
...
- 자기 자신과의 친구 관계, 정규화된 사용자 쌍, 토너먼트 정원·시드 고유성, 상태 열거형이 아직 DB 제약으로 강제되지 않는다는 점을 기록합니다.
...
| 입력 → 처리 → 결과 | 마이그레이션 실행 → 확장 기능과 테이블 생성 → FK·고유·조회 인덱스 설치 → 이후 저장소가 동일 행을 읽고 씁니다. |
| 소유 주체·수명·정리 | PostgreSQL이 UUID·생성 시각·관계 행의 수명을 소유합니다. `sessions`와 참가 행은 부모 삭제 시 연쇄 삭제되고, 애플리케이션은 아직 정리를 직접 조정하지 않습니다. |
...
| 입력 → 처리 → 결과 | `users` 행 → `UserProjectionRow` → `toPublicUser(online)` → 공개 사용자, 또는 이메일을 더한 `SessionUser`로 변환됩니다. |
| 소유 주체·수명·정리 | DB 드라이버가 행 수명을 소유하고 변환기는 새 DTO 값을 반환합니다. `online` 상태는 저장 행이 아니라 호출자가 제공하는 일시적 정보입니다. |
...
| 후속 연결 | `9277572765e7`가 이 타입을 사용하는 저장소 수명주기를 만들고, 개발 흐름 3의 행 스키마 리팩터링이 변환 결과를 표준 형태로 정렬합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `4aa060c0b8df`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `0e850d24406e`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | `databaseUrl` → `Pool`/`Kysely` 생성 → `PostgresRepository` 반환 → 호출 종료 시 `close()`가 데이터베이스 자원을 해제합니다. 메모리 생성 함수는 외부 자원 없는 구현을 반환합니다. |
| 소유 주체·수명·정리 | PostgreSQL 저장소가 Kysely와 풀을 소유하며 저장소를 만든 호출자가 `close()` 호출 책임을 가집니다. 메모리 저장소는 소유할 외부 핸들이 없습니다. |
...
| 후속 연결 | `fb516f723cdf`부터 실제 저장소 연산이 추가되고, 개발 흐름 2의 `f9bb622a1117`가 마이그레이션과 시드 소유권을 분리합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `9277572765e7`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `4aa060c0b8df`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 개발 입력 → 핸들 정규화 → 기존 행/목록 검색 → 삽입 또는 갱신 → 변환기를 거친 세션 사용자 반환입니다. |
| 소유 주체·수명·정리 | PostgreSQL 행은 DB가, 메모리 사용자는 저장소의 `Map`이 소유합니다. 반환값은 변환 결과이며 저장소 종료 전까지 메모리 원본이 유지됩니다. |
...
| 후속 연결 | 개발 흐름 2의 `8da6edef28eb`가 초기 데이터 구성을 나누고 `e1a0316fbe84`가 API 시작의 암시적 시드를 제거합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `fb516f723cdf`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `9277572765e7`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 식별자 또는 핸들 → 행 검색 → 공개 사용자 정보 반환; 프로필 패치 → 대상 확인 → 행/목록 변경 → 갱신 변환 결과 반환입니다. |
| 소유 주체·수명·정리 | 저장소가 원본 행/목록 변경을 소유하고 호출자는 반환 변환 결과만 소비합니다. 메모리 갱신은 저장된 객체를 직접 바꿉니다. |
...
| 후속 연결 | `0364c42f776b`·`c7ea1ff241c8`가 조회 모델을 확장하고, 실제 온라인 여부 판단은 이후 실시간 하위 시스템이 담당합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `c5b96a06925c`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `fb516f723cdf`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 사용자 행 집합 → 레이팅/승수 정렬 → 인덱스 기반 순위와 `percentage` 계산 → 순위표 항목 배열입니다. |
| 소유 주체·수명·정리 | 저장된 레이팅/승수는 저장소가 소유하고, 순위/winRate는 호출 시 생성되는 조회 모델 값입니다. |
...
- 이 개발 흐름의 직전 관련 SHA `c5b96a06925c`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 항목 | 기록 |
| --- | --- |
| 직전 관련 상태 | `matches` 테이블은 존재했지만 조인한 쿼리 결과와 공개 경기 요약 사이에 타입·변환 계약이 없었습니다. |
...
| 입력 → 처리 → 결과 | 경기 행 + 승자/패자 핸들 + 선택적 조회 사용자 ID → 상대/승패/레이팅 증감치 계산 → ISO 타임스탬프를 가진 요약입니다. |
| 소유 주체·수명·정리 | DB가 가공 전 경기 데이터를 소유하고 변환기가 일회성 공개 요약을 소유합니다. 조회 사용자 ID는 저장 상태를 바꾸지 않습니다. |
...
| 후속 연결 | `c7ea1ff241c8`가 쿼리와 대시보드를 연결하고 개발 흐름 3의 행 변환기 테스트가 이 변환을 직접 고정합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `8ab49e5f2dd4`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `0364c42f776b`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- `MemoryRepository.listRecentMatches`의 필터, 배열 끝 선택과 역순 처리, `memoryMatchSummary`를 비교합니다.
- 두 `getDashboard`가 프로필, 최근 경기, 승률, `bestStreak`를 어떻게 조립하는지 확인합니다.
...
| 입력 → 처리 → 결과 | `userId` → 사용자 조회 → 최근 경기 8개 → 승률와 bestStreak 계산 → `DashboardSummary` 반환입니다. |
| 소유 주체·수명·정리 | 저장소가 쿼리와 집계 조립을 소유합니다. 반환 배열은 조회 모델이며 내부 행/목록의 수명과 분리됩니다. |
...
- 이 개발 흐름의 직전 관련 SHA `8ab49e5f2dd4`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 메모리 저장소 생성 → 시드/업서트 → 세션과 경기 생성 → 대시보드 조회, 별도 친구 관계·토너먼트 흐름 → 공개 결과 검증입니다. |
| 소유 주체·수명·정리 | 각 테스트가 저장소를 지역 소유하며 외부 자원 정리는 필요 없습니다. 내부 Map/배열은 저장소 수명 동안 유지됩니다. |
...
- 이 개발 흐름의 직전 관련 SHA `c7ea1ff241c8`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 최근 경기 최신순 배열 복사 → 배열 뒤집기 → 승리이면 현재 증가·최댓값 갱신, 패배면 현재=0 → 최댓값 반환입니다. |
| 소유 주체·수명·정리 | 원본 `recentMatches` 배열은 스프레드 문법로 복사해 변형하지 않습니다. 계산 함수가 파생 지표를 소유하고 저장소는 결과만 대시보드에 넣습니다. |
...
- 이 개발 흐름의 직전 관련 SHA `6509e32ba95d`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 시드·두 사용자 생성 → 네 경기 저장 → 대시보드 조회 → 결과 배열이 최신순인지 확인 → 최고 연승 2 확인입니다. |
| 소유 주체·수명·정리 | 시험 저장소가 저장 순서와 파생 지표 계산을 소유하며 테스트 종료 뒤 메모리 상태는 폐기됩니다. |
...
| 막으려는 회귀 | 임의로 만든 상수나 수식으로 되돌아가거나 최근 배열 방향을 잘못 순회하는 회귀를 막습니다. |

#### 비교 기준

- 부모 커밋의 상태와 `6b661420e060`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `035b97ca7c58`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | `pnpm postgres-integration` → `@pong-pong/db` 전용 Vitest 명령 → Testcontainers 의존성을 사용할 준비가 됩니다. |
| 소유 주체·수명·정리 | 개발 의존성과 테스트 프로세스가 컨테이너 수명을 소유합니다. 운영 환경에서 패키지를 실행할 때는 이 도구를 요구하지 않습니다. |
...
- 이 개발 흐름의 직전 관련 SHA `6b661420e060`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 컨테이너 시작 → 테스트별 스키마/검색 경로 생성 → 풀/저장소 등록 → 마이그레이션·시드·쿼리 검증 → 콜백 성공/실패와 무관하게 저장소/풀 종료·스키마 폐기 → 테스트 모음 종료 시 컨테이너 중지입니다. |
| 소유 주체·수명·정리 | 상위 테스트 모음이 컨테이너를, `withIsolatedDatabase`가 스키마와 열린 풀/저장소 목록을 소유합니다. 정리는 등록 역순으로 실행되어 의존 자원을 먼저 닫습니다. |
...
| 후속 연결 | 개발 흐름 2의 마이그레이션/준비 상태/초기화 시험과 개발 흐름 4·5의 동시성 통합 테스트가 이 테스트 실행 틀을 확장합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `e935054ce0c9`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| --- | --- | --- | --- | --- |
| 스키마와 행 | 애플리케이션 상태의 영속 소유 주체가 없었습니다. | PostgreSQL 테이블/FK가 행 신원과 관계 수명을 소유합니다. | `0e850d24406e` | DB가 영속성을 맡지만 도메인 수준 정규화 식별 방식과 용량은 이후 제약·트랜잭션이 보강해야 했습니다. |
...
이 개발 흐름의 최종 상태에서 `AppRepository`는 PostgreSQL과 메모리를 같은 인터페이스 뒤에 배치하고, 행 변환기는 관계형 데이터 행을 공유 조회 모델로 변환합니다. 프로필·순위표·최근 경기·대시보드가 저장소 연산으로 제공되며 `bestStreak`는 최근 경기 결과에서 계산됩니다. 다만 인터페이스만으로 완전한 백엔드 의미 동작 일치를 검증하지 않으며, 실제 PostgreSQL 보장은 컨테이너 통합 테스트가 다루는 범위로 제한됩니다.
...
# 개발 흐름 02 — 마이그레이션·초기 데이터·준비 상태·초기화 수명주기
...
- 실제 커밋 순서에 맞춰 `e1a0316fbe84`·`5cac4843fd9b`를 `113b3c422192` 이후가 아니라 그 앞에 배치합니다.
...
| 입력 → 처리 → 결과 | 모듈 위치를 기준으로 SQL 파일 조회 → `initialMigrationSql` 로드 → 저장소 초기화 경로에서 SQL 실행 순서입니다. |
| 소유 주체·수명·정리 | DB 패키지가 SQL 자산의 위치와 읽기 책임을 소유합니다. 실행 호출자는 반환된 SQL을 소비하지만 버전 집합을 소유하지 않습니다. |
| 실패·되돌리기·재시도 | 자산 누락·경로 오류·SQL 실행 오류는 호출자에게 전파됩니다. 적용된 마이그레이션 목록이나 되돌리기는 없습니다. |
| 보장하는 것 | 스키마 초기화 SQL을 API와 CLI가 같은 패키지 경계에서 사용할 수 있습니다. |
...
| 입력 → 처리 → 결과 | CLI 인수 해석 → URL/백엔드 저장소 생성 → `ensureSeedData()` 또는 실행 확인 연산 → `close()`입니다. |
| 소유 주체·수명·정리 | CLI 프로세스가 저장소 수명과 종료를 소유합니다. 스키마/시드의 실제 변경은 저장소 메서드가 수행합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `1140fb868714`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- 마이그레이션 실패 결과와 던져진 오류가 호출자에게 어떻게 전달되는지 확인합니다.
...
| 입력 → 처리 → 결과 | `migrateDatabase(url)` → 마이그레이션 디렉터리의 버전 파일 discovery → Kysely 마이그레이션 테이블과 비교 → 대기 중 SQL 순차 적용; 시드 명령은 별도 저장소 경로를 사용합니다. |
| 소유 주체·수명·정리 | Migrator가 스키마 변화 순서와 적용된 상태를 소유하고 저장소가 시드 데이터 의미를 소유합니다. 호출자는 두 수명주기를 명시적으로 조합합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `dea169d587a3`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 프로필 입력 → 공통 NPC와 프로필별 사용자 선택 → 핸들 기준 업서트 → 반복 실행 시 기존 신원 갱신입니다. |
| 소유 주체·수명·정리 | 저장소가 프로필별 시드 집합과 메모리 목록/DB 행을 소유합니다. 호출자가 환경에 맞는 프로필 선택 책임을 가집니다. |
...
- 이 개발 흐름의 직전 관련 SHA `f9bb622a1117`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | `migrate` → 마이그레이션 실행기 실행; `seed:dev\|demo` → 저장소 생성 → 프로필 시드 → `finally close`; 실행 확인은 메모리 경로를 사용합니다. |
| 소유 주체·수명·정리 | 마이그레이션 명령은 마이그레이션 실행기 자원을, 시드 명령은 저장소 수명주기를 소유합니다. CLI 호출자가 실행 순서를 선택합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `8da6edef28eb`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 마이그레이션 디렉터리 목록 읽기 + Kysely 적용된 행 조회 → 정렬된 집합 비교 → 동일하면 현재, 적용된 접두 경로 뒤 파일이 남으면 대기 중, 접두 경로가 깨지거나 알 수 없는 적용된가 있으면 불일치입니다. |
| 소유 주체·수명·정리 | Migrator 모듈이 마이그레이션 이벤트 종류와 비교 로직을 소유합니다. 검사 호출자는 결과를 정책으로 해석합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `981ee655559b`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | API 준비 상태 요청 → 저장소 준비 상태 → PostgreSQL 쿼리 성공 여부 → 마이그레이션 집합 검토 → 현재이면 준비 완료 응답, 아니면 오류/비준비입니다. |
| 소유 주체·수명·정리 | 저장소가 백엔드별 상태 확인 요청과 마이그레이션 해석을 소유하고 API는 결과만 소비합니다. |
...
| 보장하지 않는 것 | 장기 트랜잭션, 복제 지연, 모든 테이블의 의미상 무결성, 이후 시점의 사용 가능 상태는 보장하지 않습니다. |
| 후속 연결 | `e1a0316fbe84`가 시작 변경을 제거해 준비 상태와 초기화를 더 분리하고, 상태 확인 엔드포인트 통합은 연산 카테고리에서 검증됩니다. |

#### 비교 기준

- 부모 커밋의 상태와 `2f05d5d79c64`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `30aac132e14e`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 해결하려던 문제 | 재시작이 데이터 상태를 바꾸고, 운영/체험 설정 오류가 예시 데이터로 가려지며, 준비 상태가 초기화 성공처럼 오해될 수 있었습니다. |
| 핵심 결정 | 구성 진입점에서 `ensureSeedData()` 호출을 제거하고 시드는 DB CLI나 명시적 테스트 설정만 수행하도록 했습니다. |
| 입력 → 처리 → 결과 | 환경 파싱 → 저장소 생성 → 애플리케이션 구성 → 포트 열기로 진행하며 시작 중 시드 쓰기는 없습니다. |
| 소유 주체·수명·정리 | API 프로세스는 실행 시점 수명주기만 소유하고 초기 데이터 소유권은 명시적 DB 명령/테스트 픽스처로 돌아갑니다. |
...
- 이 개발 흐름의 직전 관련 SHA `2f05d5d79c64`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- 별칭·래퍼를 통한 간접 시드 호출은 탐지하지 못하는 보장하지 않는 범위를 기록합니다.
...
| 입력 → 처리 → 결과 | 테스트 → `apps/api/src/index.ts` 소스 파일 읽기 → 금지된 호출 방식 검색 → 존재 시 실패; 별도 실행 확인은 AI/대기열 경로를 시드 없는 조건에 맞춥니다. |
| 소유 주체·수명·정리 | 시험이 소스 계약을 소유하며 실행 시점 저장소 상태는 만들지 않습니다. |
| 실패·되돌리기·재시도 | 래퍼·computed property·다른 시작 모듈의 간접 호출은 놓칠 수 있습니다. 실제 프로세스 시작이나 DB 변경을 관찰하지 않습니다. |
...
| 테스트가 검증하지 않는 것 | 간접 래퍼, 다른 모듈, 실제 실행 시점 변경이나 배포 명령 순서는 검증하지 않습니다. |
| 막으려는 회귀 | 프로세스 시작이 예시 데이터 생성 책임을 다시 떠안는 회귀를 빠르게 막습니다. |

#### 비교 기준

- 부모 커밋의 상태와 `5cac4843fd9b`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `e1a0316fbe84`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 환경 설정 입력 → 필수값 검증 → URL 파싱과 디코딩한 DB 이름 확인 → 옵션과 `search_path` 해석 → 엄격한 허용 목록 대조 → 안전한 대상 반환입니다. 하나라도 어긋나면 예외를 발생시킵니다. |
| 소유 주체·수명·정리 | 보호 조건 모듈이 삭제 대상 검증을 소유하고 초기화 실행 객체는 검증된 대상만 받아야 합니다. 아직 DB 자원은 만들지 않습니다. |
| 실패·되돌리기·재시도 | 일반 DB, 운영 환경 설정, 여러 옵션, 임의 스키마, 모호한 검색 경로는 실패 시 차단합니다. URL이 규칙을 만족해도 실제 서버가 테스트 전용인지 외부에서 검증하지는 못합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `5cac4843fd9b`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | CLI → 대상 판별 → 제어용 풀과 클라이언트 → BEGIN → 안전하게 인용한 스키마 DROP CASCADE → CREATE → COMMIT → 클라이언트 릴리스/풀 종료 → `migrateDatabase`로 현재 스키마 재구성입니다. |
| 소유 주체·수명·정리 | 초기화 함수가 제어 연결과 트랜잭션 정리를 소유하고 마이그레이션 실행기가 새 스키마 버전 적용을 소유합니다. CLI 호출자는 명시적 호출만 소유합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `113b3c422192`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- `packages/db/src/testReset.test.ts`의 테스트 외 실행 환경, 일반 DB, 모호한 옵션, 잘못된 스키마 거부 사례를 확인합니다.
...
- 시험이 대상 스키마 외 인접 스키마를 보존하는지 직접 쿼리로 확인하는 부분을 찾습니다.
...
| 입력 → 처리 → 결과 | 단위: 환경 설정/URL 조합표 → 판별 성공 또는 안전하지 않은 실패; 통합: 대상·인접한 스키마 생성 → 대상 초기화 → 마이그레이션/현재 확인 → 인접한 표식 보존 확인입니다. |
| 소유 주체·수명·정리 | 단위 테스트는 문자열 규칙을, 통합 테스트 실행 틀은 스키마·풀 수명주기를 소유하고 기존 `withIsolatedDatabase` 정리를 재사용합니다. |
| 실패·되돌리기·재시도 | 안전하지 않은 입력은 DB 연결 전에 실패합니다. 실제 초기화 실패는 명령 오류로 전파되며 테스트 실행 틀이 자원을 정리합니다. |
| 보장하는 것 | 정의된 허용 목록에 맞지 않으면 작업을 차단하고, 초기화는 대상 스키마만 교체한 뒤 마이그레이션을 최신 상태로 만든다는 코드를 검증합니다. |
| 보장하지 않는 것 | 실제 운영 URL 오지정, 권한 상승, 이 환경에서의 컨테이너 테스트 통과는 검증하지 않습니다. |
...
| 재현한 실패·경계 | 테스트 외 실행 환경, 일반 DB, 복수/모호 옵션, 잘못된 스키마, 실제 인접한 스키마가 있는 PostgreSQL입니다. |
...
| 테스트가 검증하지 않는 것 | 물리 서버 신원, 인증 정보 탈취, 운영 데이터 복구, 이 작업 환경에서의 실제 명령 실행 성공은 검증하지 않습니다. |
| 막으려는 회귀 | 허용 목록 완화, 스키마 이름 인용 오류, 인접 스키마 삭제, 초기화 후 마이그레이션 누락 회귀를 막습니다. |

#### 비교 기준

- 부모 커밋의 상태와 `527b5f137425`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `434403a7c16a`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 시작 변경 제거 | `e1a0316fbe84` → `5cac4843fd9b` | 프로세스 시작과 데이터 초기화 책임 분리를 설명합니다. | API 진입점은 시드를 만들지 않으며 명시적 CLI/테스트 설정이 데이터 준비를 소유합니다. 소스 테스트는 직접 호출 회귀만 막고 전체 실행 시점 호출 의존성 그래프를 검증하지는 않습니다. |
| 파괴적 초기화 보호 | `113b3c422192` → `527b5f137425` | 실패 시 차단 대상, 트랜잭션, 마이그레이션, 인접한 보존을 추적합니다. | 엄격한 실행 환경·URL·스키마 허용 목록을 통과한 대상만 트랜잭션으로 폐기/생성되고 마이그레이션이 별도로 재적용됩니다. 단위 조합표와 실제 PostgreSQL 테스트가 거부 조건과 대상에만 적용되는 효과를 연결합니다. |
...
| 2 | DB 연결 성공만으로는 대기 중이거나 불일치한 스키마를 준비 완료로 오인할 수 있었습니다. | `30aac132e14e`가 마이그레이션 집합 상태를 분류하고 `2f05d5d79c64`가 저장소 준비 상태에 포함했습니다. | HTTP 상태 확인 통합은 운영 카테고리에서 검증되며 이 개발 흐름은 저장소 수준의 근거만 고정합니다. | 준비 완료는 현재 바이너리가 기대하는 마이그레이션 목록과 DB에 적용된 목록이 일치하는 순간의 상태입니다. 향후 사용 가능성이나 데이터 의미의 정확성까지 보장하지는 않습니다. |
| 3 | API 시작이 메모리 저장소 대체 실행을 선택하면 예시 데이터를 암시적으로 생성했습니다. | `e1a0316fbe84`가 시작 시 초기 데이터 생성 호출을 제거했습니다. | `5cac4843fd9b`가 진입점 직접 호출 방식을 금지합니다. | 프로세스 수명주기와 데이터 초기화를 분리합니다. 테스트는 가벼운 정적 보호 조건이므로 indirect 호출까지 보장한다고 해석하면 안 됩니다. |
| 4 | 수동 테스트 초기화는 잘못된 URL/스키마를 파괴하거나 인접 스키마를 지울 위험이 있었습니다. | `113b3c422192` 엄격한 모듈 해석기와 `434403a7c16a` 명시적 초기화 실행 객체를 추가했습니다. | `527b5f137425`가 거부 조합표와 인접한 스키마 보존을 검증합니다. | 검증은 DB 연결 전 실패 시 차단하며 실행 객체는 검증 결과만 사용합니다. 스키마 교체와 마이그레이션은 별도 수명주기이므로 마이그레이션 실패 시 명령 실패 상태가 남을 수 있습니다. |
...
| 시작 | 구성 진입점이 실행 시점 boot와 시드 변경을 함께 했습니다. | API는 boot/포트 열기만, DB CLI·테스트 픽스처가 데이터 초기화를 담당합니다. | `e1a0316fbe84` | 빈 메모리 상태는 허용되는 명시적 결과이며 시작이 예시 데이터로 감추지 않습니다. |
...
- [x] 시작 시 초기 데이터 생성 제거가 만드는 빈 상태와 명시적 초기화 책임을 설명할 수 있습니다.
- [x] 소스 수준 시작 테스트의 증거 한계를 과장하지 않습니다.
- [x] 초기화 보호 조건의 정확한 허용 목록, DDL 트랜잭션, 별도 마이그레이션 단계와 실패 상태를 설명할 수 있습니다.
- [x] 개발 흐름 2의 수정된 커밋 순서가 실제 브랜치 chronology와 일치함을 확인했습니다.
...
- 제외: 경기 결과 확정 원자성, 관리자 감사 트랜잭션, 토너먼트 참가 잠금은 별도의 데이터 무결성 개발 흐름에서 다룹니다.
- 이 개발 흐름의 리팩터링은 실행 중 검증을 새로 제공하지 않습니다. TypeScript 계약과 코드 검토 가능성이 중심입니다.
...
| 입력 → 처리 → 결과 | 메모리 사용자 입력 → 표준 변환 결과 객체 생성/저장 → 기존 변환기로 공개/세션 DTO 변환입니다. |
| 소유 주체·수명·정리 | `schema.ts`가 사용자 조회 결과 이벤트 종류를 소유하고 메모리 저장소는 그 타입의 원본 객체를 Map 수명 동안 소유합니다. |
...
| 입력 → 처리 → 결과 | 경기 명령 → 필요한 필드 선택·새 레코드 생성 → 메모리 배열 저장 → 요약 변환기가 저장된 필드만 읽습니다. |
| 소유 주체·수명·정리 | 명령 객체는 호출자 수명, 저장된 레코드는 저장소 배열 수명을 가집니다. 두 객체가 별도 값으로 분리됩니다. |
...
- 이 개발 흐름의 직전 관련 SHA `73b8ce0f0c26`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | SQL 테이블 인터페이스 → `Selectable` 행 타입 → 참가한 변환 결과 → 변환기 입력 타입으로 한 방향의 타입 의존성을 만듭니다. |
| 소유 주체·수명·정리 | `schema.ts`가 DB 행 이벤트 종류를 소유하고 변환기/저장소는 가져와 사용합니다. 실행 중 행 데이터의 수명은 변하지 않습니다. |
...
- 이 개발 흐름의 직전 관련 SHA `3d0ae79affd5`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | `TournamentMatchRow` → snake_case ID/상태/참가자 필드 선택 → camelCase 내부 레코드 조회 결과 반환입니다. |
| 소유 주체·수명·정리 | 변환기가 새 값을 소유하고 DB 행은 쿼리 결과 수명에 남습니다. 조회 결과는 공개 관계 사용자를 소유하지 않고 ID만 담습니다. |
...
- 이 개발 흐름의 직전 관련 SHA `3e3f21129369`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- 분기 중복이 늘어나는 대신 쿼리 형식 검토 용이성가 좋아지는 절충점를 기록합니다.

#### 학습자 기록

| 항목 | 기록 |
| --- | --- |
| 직전 관련 상태 | 최근 경기 쿼리는 선택적 원시 SQL 조각을 중간에 삽입해, 특정 사용자 범위 쿼리와 전체 쿼리의 최종 형태를 한눈에 검토하기 어려웠습니다. |
| 해결하려던 문제 | 타입이 지정된 쿼리 결과와 참가자 조건 함수가 브랜치별로 정확히 일치하는지 검토자가 확인하기 쉬운 표현이 필요했습니다. |
| 핵심 결정 | userId 존재 여부에 따라 완전한 두 SQL 쿼리를 선택하고 나머지 매핑·대시보드 동작은 유지했습니다. |
| 입력 → 처리 → 결과 | `userId` 있음 → 참가자 where가 포함된 쿼리, 없음 → 전역 쿼리 → 같은 `toMatchSummary` 매핑과 최신순 상한입니다. |
| 소유 주체·수명·정리 | 저장소 메서드가 쿼리 브랜치 선택을 소유하고 변환기가 출력 변환을 계속 소유합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `212650b2863d`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 경기 행 → 왼쪽·오른쪽 참가자·승자 ID별 사용자 조회 → 관계 객체 → 요약 변환기; 관리자 행 → 수행자/대상 조회 → 관리자 요약입니다. |
| 소유 주체·수명·정리 | 도우미 함수가 관계 조립을 소유하고 변환기가 형식 변환을 소유합니다. 저장소가 DB 호출 수명을 관리합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `45144a3719bc`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 토너먼트+생성자 행 → 참가 기록/경기/승자 비동기 조회 → 관련 객체 → 한 번의 순수 요약 생성입니다. |
| 소유 주체·수명·정리 | 저장소가 관계 조회 요청과 비동기 처리 수명을 소유하고 변환기가 완성된 불변 방식 DTO 생성을 소유합니다. 반환 후 변경이 사라집니다. |
...
- 이 개발 흐름의 직전 관련 SHA `ce41a880d6c6`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 경기 ID → `{tournament, match}` 조회 → 승자 변환 결과 조회 → 하위 상태/점수 변경 → 준결승이면 결승 생성, 아니면 집계 완료입니다. |
| 소유 주체·수명·정리 | 저장소 배열이 토너먼트 집계와 하위 객체 수명을 소유하고 도우미 함수는 별칭 쌍을 일시적으로 반환합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `5c8659ea233b`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- 이미 확정된 `matchId` 검사와 승자/패자가 해당 토너먼트 경기 참가자인지 검증하는 순서를 확인합니다.
...
| 해결하려던 문제 | 참가자 검증, 중복 확정 방지, 준결승·결승 상태 변경이 같은 토너먼트 데이터를 대상으로 한다는 사실을 코드에서 명확히 드러내야 했습니다. |
| 핵심 결정 | `findTournamentMatch` 결과를 하나의 로컬 값으로 사용해 참가자 집합과 subsequent 변경을 정렬했습니다. |
| 입력 → 처리 → 결과 | 명령 → 토너먼트 하위 조회 → 이미 확정됐는지와 참가자 일치 여부 검증 → 경기/통계 갱신 → 하위 결과 기록 → 준결승 결과로 결승 생성 또는 토너먼트 승자/상태 확정입니다. |
| 소유 주체·수명·정리 | 저장소가 경기 레코드, 사용자 통계, 토너먼트 집계를 같은 메서드 동안 소유합니다. 도우미 함수 결과는 원본 객체 별칭입니다. |
...
| 후속 연결 | `dc0e60e6aa35`는 변환기를 검증하고, 경기 결과 확정 원자성 자체는 별도의 영속 저장 개발 흐름에서 다룹니다. |

#### 비교 기준

- 부모 커밋의 상태와 `9d64ea406b03`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `f77e317de4c1`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 채팅 입력 → 발신자 검증 → 형식이 정해진 메시지 저장; 토너먼트 생성 → 타입이 지정된 집계 저장; 참가 → 기존/정원 초과 검사 → 항목 추가 → 개수/상태/대진 갱신입니다. |
| 소유 주체·수명·정리 | 저장소 배열이 메시지와 토너먼트 집계를 소유하고 호출자는 같은 객체 변환 결과를 받습니다. 도우미 함수가 하위 별칭을 찾습니다. |
...
| 후속 연결 | 개발 흐름 5의 `efdb5c3a4932`가 참가자를 표준 사용자 저장소에서 검증하고 동시성 테스트가 마지막 슬롯 결과를 확인합니다. |

#### 비교 기준

- 부모 커밋의 상태와 `b34fdaa1e9c2`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `9d64ea406b03`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 해결하려던 문제 | 명시적 변환 결과·관계 객체 변경 중 snake_case 누출, 조회 사용자 기준 부호, null 허용 관계, 타임스탬프 변환이 깨질 수 있었습니다. |
| 핵심 결정 | 고정 행 픽스처로 모든 주요 변환기를 직접 호출하고 정확한/부분 반영 객체 검증을 추가했습니다. |
| 입력 → 처리 → 결과 | 타입이 지정된 픽스처 행 → 변환기 호출 → 공개/내부 DTO의 ID·이름·열거형·numeric·ISO·관계 필드 비교입니다. |
| 소유 주체·수명·정리 | 테스트 픽스처가 입력 값을 소유하고 변환기는 새 출력 객체를 반환합니다. DB/풀 자원은 없습니다. |
| 실패·되돌리기·재시도 | 실행 중 잘못된 행 데이터, SQL 조인 누락, 쿼리 순서는 재현하지 않습니다. 순수 함수 검증 실패로 형식 회귀를 드러냅니다. |
...
- 이 개발 흐름의 직전 관련 SHA `b34fdaa1e9c2`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| DB 타입 이벤트 종류 | 라운드/상태/범위/동작 유니언 타입이 여러 파일에 중복됐습니다. | 표준 타입 별칭와 행 exports를 `schema.ts`가 소유합니다. | `3e3f21129369`, `212650b2863d` | 변환기는 스키마 필드에서 타입을 파생하지만 실행 중 행 데이터 검증은 여전히 별도 책임입니다. |
...
- [x] 관계 조회와 변환 결과 생성의 역할 분리를 실제 도우미 함수와 함수 이름으로 설명할 수 있습니다.
...
# 개발 흐름 04 — 표준 친구 관계와 동시 요청

## 1. 학습 목표

- 초기 방향이 있는 친구 관계 행과 메모리 요약이 왜 하나의 관계 신원을 표현하지 못하는지 설명합니다.
- 기존 데이터 정규화, 자기 자신 여부 검사, 정규화 식 고유 인덱스의 적용 순서를 재구성합니다.
- PostgreSQL 단일 SQL 문 업서트가 반복/반대 방향 요청을 어떤 상태 전이로 원자화하는지 추적합니다.
- 메모리 동작 일치와 회귀 테스트의 실제 동시성 증거 범위를 과장하지 않고 구분합니다.

## 2. 범위와 경계

- 포함: 친구 관계 저장소 연산, 표준 쌍 마이그레이션, PostgreSQL 원자적 요청, 메모리 관계 레코드, 여러 영역에 걸친 백엔드 회귀.
- `cdaca35ccf7f`에서는 친구 관계 검증만 이 개발 흐름의 핵심 근거로 사용합니다. 같은 커밋의 토너먼트 테스트는 개발 흐름 5에서 다룹니다.
...
| 입력 → 처리 → 결과 | 요청자+상대 핸들 → 사용자 조회 → 대기 중 행/요약 생성; 목록 → 요청자와 수신자 중 어느 쪽인지 계산해 상대 사용자를 포함; 수락 → 수신자가 지정 행 상태를 허용된 상태로 변경합니다. |
| 소유 주체·수명·정리 | PostgreSQL 행은 두 사용자 ID와 방향을 소유하지만 메모리 배열은 요청자와 수신자를 버리고 공개 요약만 소유합니다. 이 차이가 이후 동작 일치 문제의 원인입니다. |
...
| 후속 연결 | `ffb0a8275a4f`가 기존 데이터를 정규화하고 정규화된 사용자 쌍을 DB 불변 조건으로 만들며 `34db79005f30`이 메모리 표현을 같은 의미로 바꿉니다. |
...
- `row_number() over (partition by least(...), greatest(...))`에서 보존할 행의 우선순위가 `accepted` 상태 → `created_at` → `id` 순서인지 확인합니다.
...
| 핵심 결정 | 마이그레이션이 자기 자신 행을 삭제하고 반대 방향의 대기 중 요청을 허용된 상태로 합친 뒤 정규화된 사용자 쌍별 보존할 행을 결정적으로 남기고 식 고유/확인을 설치합니다. |
| 입력 → 처리 → 결과 | 기존 행 → 자기 자신 삭제 → 반대 방향 요청 상태 병합 → 정규화된 사용자 쌍 분할/순위 → 중복 삭제 → 방향별 고유 제약 제거 → 서로 다른 사용자 CHECK와 순서와 무관한 정규화 식 UNIQUE 설치입니다. |
| 소유 주체·수명·정리 | 마이그레이션이 기존 데이터 정규화와 새 DB 불변 조건 설치를 소유합니다. PostgreSQL 인덱스가 이후 모든 작성 기능의 순서와 무관한 신원을 소유합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `645e5a3c8e96`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 중요도 | A |
| 태그 | PERSISTENCE, RISK |
| 원문 역할 | 정규화 식 인덱스를 충돌 대상으로 사용해 반복·반대 방향 친구 관계 요청을 한 SQL 문에서 처리합니다. |
...
- `returning id, status`가 충돌 삽입/갱신 모두에서 같은 관계 신원을 반환하는지 확인합니다.
...
| 핵심 결정 | 정규화 식을 충돌 대상으로 사용하고 조건부 `DO UPDATE`를 적용해 신규 요청 삽입, 같은 방향의 반복 요청 무시, 반대 방향의 대기 중 요청 수락을 한 SQL 문으로 표현했습니다. |
| 입력 → 처리 → 결과 | 상대 핸들 조회/자기 자신 여부 검사 → 대기 중 삽입 시도 → 순서와 무관한 쌍 충돌 시 기존 행과 삽입 후보 행의 요청 방향 비교 → 반대 방향의 대기 중 요청이면 허용된+타임스탬프 갱신, 아니면 기존 상태 유지 → id/상태 반환입니다. |
| 소유 주체·수명·정리 | PostgreSQL 고유 인덱스가 쌍 신원을, 단일 SQL 문이 상태 전이 원자성을 소유합니다. 호출자는 반환 요약만 받습니다. |
...
- `PostgresRepository.requestFriend` — 정규화된 사용자 쌍 식을 충돌 대상으로 사용하고 반대 방향의 대기 중 요청일 때만 `status = 'accepted'`로 바꾸는 단일 업서트입니다.

#### 비교 기준

- 부모 커밋의 상태와 `77c555aba9a0`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `ffb0a8275a4f`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 중요도 | B |
| 태그 | PERSISTENCE |
| 원문 역할 | 메모리 저장 표현을 호출자 특정 요약에서 요청자와 수신자 ID를 보존하는 표준 관계 레코드로 바꿉니다. |
...
| 입력 → 처리 → 결과 | 요청자+상대 → 자기 자신 검사 → 순서와 무관한 기존 검색 → 동일한 방향이면 기존 반환, 반대 방향의 대기 중 요청이면 상태 허용된, 없으면 새 대기 중 레코드 저장; 목록/수락은 수행자 ID로 변환합니다. |
| 소유 주체·수명·정리 | 메모리 저장소가 관계 레코드와 사용자 Map을 프로세스 수명 동안 소유하고 반환 시 상대 사용자를 새 공개 사용자 정보로 만듭니다. |
...
- 이 개발 흐름의 직전 관련 SHA `77c555aba9a0`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
- 양쪽 `listFriends`가 동일 관계 ID와 상대 사용자를 반환하는 검증을 확인합니다.
...
- 토너먼트 마지막 슬롯 테스트는 개발 흐름 5에서 별도 해석하고 이 개발 흐름에서는 친구 관계 근거만 사용합니다.

#### 학습자 기록

| 항목 | 기록 |
| --- | --- |
| 직전 관련 상태 | 표준 마이그레이션, PostgreSQL 업서트, 메모리 레코드가 구현됐지만 같은 관찰 가능한 시나리오에서 한 관계 신원을 유지하는지 회귀 테스트 근거가 필요했습니다. |
...
| 입력 → 처리 → 결과 | 두 사용자 생성 → 자기 자신 거부 → A→B 대기 중 → A→B 반복 → B→A 역순 처리 허용된 → 양쪽 목록 확인 → PostgreSQL 실제 테이블 한 행과 CHECK 실패 확인입니다. |
| 소유 주체·수명·정리 | 각 테스트 저장소가 관계 상태를 소유하고 통합 테스트 실행 틀이 스키마/풀 정리를 소유합니다. |
| 실패·되돌리기·재시도 | 메모리 구현은 한 프로세스에서 상태를 즉시 변경하므로 실제 병렬 실행을 재현하지 않습니다. PostgreSQL 테스트도 반대 방향 요청을 순차 실행하므로 친구 관계의 동시 경쟁 상태를 직접 만들지는 않습니다. |
...
- `postgres.integration.test.ts` — 공개 저장소 결과 뒤 원시 `friendships` 테이블이 허용되는 행이 정확히 하나인지 확인하고 자기 자신 삽입의 제약 이름까지 검사합니다.
...
- 이 개발 흐름의 직전 관련 SHA `34db79005f30`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.

## 6. 불변 조건 변화

| 단계 | 관련 SHA | 조사 초점 | 학습자 기록 |
| --- | --- | --- | --- |
| 방향 기반 초기 모델 | `645e5a3c8e96` | 초기 DB·메모리 관계 신원과 권한 검사 차이를 기록합니다. | PostgreSQL은 방향을 가진 두 ID를 저장하지만 반대 방향 중복을 허용하고, 메모리는 수행자 신원 자체를 버린 `FriendSummary[]`라 양쪽 변환 결과와 수락 권한을 정확히 표현하지 못했습니다. |
| 표준 DB 신원 | `ffb0a8275a4f` | 기존 정리와 새 제약 설치 순서를 설명합니다. | 자기 자신 행 제거, 반대 방향 요청 상태 병합, 결정적 보존할 행 선택 뒤 방향별 고유 제약을 정규화 식 인덱스와 서로 다른 사용자 CHECK로 교체합니다. |
| 원자적 PostgreSQL 상태 전이 | `77c555aba9a0` | 삽입/반복·반대 방향 상태 전이를 한 SQL 문에서 추적합니다. | 순서와 무관한 정규화 식 충돌가 한 행을 선택하고 반대 방향의 대기 중 요청인 경우만 허용된 상태로 갱신합니다. 같은 방향 반복은 ID·상태·타임스탬프를 유지합니다. |
...
| 회귀 테스트 근거 | `cdaca35ccf7f` | 공개 결과와 가공 전 행 제약을 연결합니다. | 두 백엔드에서 자기 자신/반복·반대 방향/목록 시나리오를 검증하고 PostgreSQL 테이블에 허용되는 행 하나만 남는지와 CHECK 위반을 직접 확인합니다. 친구 관계 동시 요청 경쟁 상태는 직접 실행하지 않습니다. |
...
| 2 | 초기 요청 코드는 방향이 있는 충돌만 처리하고 반대 방향의 대기 중 요청의 의미를 여러 연산에 남겼습니다. | `77c555aba9a0`이 정규화된 충돌 대상과 조건부 갱신으로 한 SQL 문에 넣었습니다. | `cdaca35ccf7f`가 동일한 ID, 대기 중→허용된, 양쪽 변환 결과를 확인합니다. | 고유 인덱스와 업서트가 같은 정규화 식을 사용해야 충돌 감지와 상태 전이가 일치합니다. |
...
| 구간 | 이전 소유자/표현 | 이후 소유자/표현 | 관련 SHA | 학습자 기록 |
| --- | --- | --- | --- | --- |
| 관계 신원 | 요청 방향 또는 호출자 관점의 요약이 신원을 사실상 정의했습니다. | DB 정규화 식과 메모리 순서와 무관한 조회가 쌍 신원을 정의합니다. | `ffb0a8275a4f`, `34db79005f30` | ID 하나가 양쪽 사용자에게 공유되고 변환 결과만 수행자에 따라 달라집니다. |
...
최종 상태에서 친구 관계는 요청자와 수신자 순서와 무관한 사용자 쌍 하나로 식별됩니다. PostgreSQL은 서로 다른 사용자 CHECK와 정규화 식 고유 인덱스를 최종 강제 검사 소유 주체로 사용하고, 요청은 한 업서트에서 반복·역방향 상태를 결정합니다. 메모리는 같은 관계 레코드와 수행자별 변환 결과를 구현합니다. 시험은 주요 순번과 실제 DB 제약을 검증하지만 동시에 들어온 친구 관계 요청 부하나 여러 프로세스 메모리 일관성까지 검증하지 않습니다.

## 10. 최종 실행 흐름

1. 호출자가 요청자 ID와 상대 핸들로 `requestFriend`를 호출하고 저장소가 상대 사용자를 찾은 뒤 자기 자신 요청을 거부합니다.
2. PostgreSQL은 대기 중 삽입을 시도하고 정규화된 사용자 쌍 충돌 시 기존/excluded 방향을 비교합니다.
...
- [x] `cdaca35ccf7f`가 친구 관계에 대해 실제 동시 경쟁 상태를 검증하지 않는다는 점을 명시할 수 있습니다.
...
- `cdaca35ccf7f`에서는 토너먼트 정원 검증만 이 개발 흐름의 핵심 근거로 사용합니다. 친구 관계 검증은 개발 흐름 4에서 다룹니다.
- 제외: 토너먼트 경기 시작·완료, 결승 생성, 경기 결과 확정과 되돌리기는 독립 토너먼트/실시간 영속 저장 과정입니다.
...
| 입력 → 처리 → 결과 | 생성 → 토너먼트 행 저장 → 생성자 참가; 참가 → 현재 항목 개수 조회 → 시드=개수+1 계산 → 고유 사용자 삽입 → 집계 재조회입니다. |
| 소유 주체·수명·정리 | PostgreSQL이 행을, 메모리 토너먼트 객체가 참가 기록 배열을 소유합니다. 생성 호출자가 별도 트랜잭션 없이 두 연산을 연속 호출합니다. |
| 실패·되돌리기·재시도 | 동시 참가가 같은 개수/시드를 볼 수 있고 용량을 초과할 수 있습니다. 생성 후 생성자의 참가 처리에 실패하면 빈 토너먼트 행이 남을 수 있습니다. 메모리도 신규 참가를 용량에서 거부하지 않습니다. |
...
| 입력 → 처리 → 결과 | 기존 참가 기록 → 토너먼트별 결정적 정렬·순위 부여 → 시드 다시 쓰기 → 고유 제약 설치 → 이후 중복 시드 삽입 거부입니다. |
| 소유 주체·수명·정리 | 마이그레이션이 기존 시드 정규화를, PostgreSQL 제약이 이후 시드 고유성을 소유합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `9b1dabcc4bb4`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 입력 → 처리 → 결과 | 트랜잭션 시작 → 토너먼트 행 잠금 → 이미 참가한 사용자면 기존 결과 반환 → 참가자 수와 최대 시드 조회 → 정원이 찼으면 예외 발생 → 참가자 삽입 → 정원에 도달하면 상태를 `running`으로 바꾸고 준결승 2개 업서트 → 커밋 → 집계 재조회 순서입니다. |
| 소유 주체·수명·정리 | PostgreSQL 트랜잭션이 해당 토너먼트 참가 상태와 대진 부수 효과를 커밋/되돌리기 단위로 소유합니다. 도우미 함수는 전달받은 트랜잭션 실행 객체를 사용합니다. |
...
| 보장하는 것 | 동일 토너먼트의 동시 참가 요청이 행 잠금에서 직렬화되고 정확히 하나 최종 슬롯, 고유 시드, 용량, 실행 중+대진 전이가 한 트랜잭션으로 결정됩니다. |
...
- 이 개발 흐름의 직전 관련 SHA `3aa5958bb967`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 중요도 | B |
| 태그 | PERSISTENCE, TOURNAMENT |
| 원문 역할 | 메모리 구현도 공개 조회 결과를 그대로 신뢰하지 않고, 사용자 `Map`에 원본 사용자가 실제로 존재하는지 먼저 확인한 뒤 조회 결과를 만들도록 바꿉니다. |
...
- 기존 참가자·정원 초과·멱등성·상태·대진 로직이 그대로 유지되는지 확인합니다.
...
| 입력 → 처리 → 결과 | 토너먼트 조회 + 표준 사용자 Map 조회 → 공개 사용자 정보 생성 → 기존/정원 초과 검사 → 참가 기록 변경과 상태/대진 갱신입니다. |
| 소유 주체·수명·정리 | 메모리 사용자 Map이 참가자 신원의 기준 데이터를 소유하고 토너먼트 집계는 공개 사용자 정보를 참가 기록으로 보유합니다. |
...
- 이 개발 흐름의 직전 관련 SHA `d9a6d8dd8950`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
- 후속 관련 SHA `cdaca35ccf7f`가 이 결정의 부족한 점을 보완하거나 검증하는지 확인합니다.
...
- 이미 참가한 사용자의 재참가가 성공하고 개수/대진이 늘지 않는 멱등성 검증을 확인합니다.
...
| 입력 → 처리 → 결과 | 생성자를 자동 참가시킨 뒤 초기 참가자 2명을 추가합니다. 후보 10명의 참가 Promise를 실행하면 1건만 성공하고 9건은 정원 초과로 실패합니다. 최종 참가 기록은 4건, 고유 사용자 ID는 4개, 시드는 1~4이며 준결승 슬롯 1·2가 생성됩니다. 성공한 사용자가 다시 참가해도 참가자 수와 경기 수는 바뀌지 않습니다. |
| 소유 주체·수명·정리 | PostgreSQL 트랜잭션/행 잠금이 마지막 슬롯 승자를 소유하고 테스트 실행 틀이 스키마/풀 정리를 소유합니다. 메모리는 이벤트 루프 내 메서드 변경 순서가 결과를 결정합니다. |
| 실패·되돌리기·재시도 | 정원 초과 요청은 항목/상태/대진을 남기지 않아야 합니다. PostgreSQL 직접 쿼리가 커밋된 상태를 확인합니다. 테스트 자체가 공정성이나 어떤 후보가 이길지는 고정하지 않습니다. |
...
- 부모 커밋의 상태와 `cdaca35ccf7f`의 변경 내용을 먼저 비교합니다.
- 이 개발 흐름의 직전 관련 SHA `efdb5c3a4932`와 책임·상태·보장 범위가 어떻게 달라졌는지 비교합니다.
...
| 메모리 구현의 사용자 원본 정렬 | `efdb5c3a4932` | 참가자 신원의 원본과 공개 변환 결과를 분리합니다. | 메모리 구현은 사용자 `Map`에 원본 사용자가 있는지 확인한 뒤 공개 사용자 정보를 참가 기록에 넣습니다. 이는 사용자 원본을 일치시키는 처리이며 DB 방식의 잠금이나 되돌리기는 아닙니다. |
...
| 3 | 메모리 참가가 참가자 존재 확인을 공개 조회 도우미 함수에 의존했습니다. | `efdb5c3a4932`가 표준 사용자 Map을 직접 확인합니다. | `cdaca35ccf7f`의 메모리 용량·고유 항목·대진 시나리오입니다. | 메모리의 신원 소스는 정렬되지만 이벤트 루프 순서에 따른 프로세스 내부 변경일 뿐 PostgreSQL의 동시성 방식과 같다고 볼 수 없습니다. |
...
7. 메모리는 같은 분기를 표준 사용자 `Map`과 토너먼트 객체에 적용하고 테스트는 10개 마지막 한 자리의 동시 참가 결과와 실제 PostgreSQL 행을 비교합니다.
...
친구 관계 정규화 식별 방식, 토너먼트 참가 동시성과 데이터 삭제를 포함하는 테스트 초기화
안전성을 다룹니다.
...
- 5개 개발 흐름을 유지합니다. 경기 결과 확정, 인증 세션·WebSocket 티켓,
  비회원 격리, 채팅 경기방 무결성, 관리자 감사 원자성과 토너먼트 경기 대진 진행은
  독립된 구현 과정이므로 다른 카테고리의 책임으로 남깁니다.
- 개발 흐름 1에는 `035b97ca7c58`, `6b661420e060`을 추가해 가짜 `bestStreak`
  구현의 수정과 회귀 테스트를 연결하고, `e935054ce0c9`를 추가해
  PostgreSQL 통합 실행 경계를 포함했습니다.
- 개발 흐름 2에서는 `e1a0316fbe84`, `5cac4843fd9b`를 2026년 1월의 초기화
...
- `9b62117a6909`, `d8050004e4ce`, `d2329e8dfc1d`,
  ...
  리팩터링은 독립적인 상태·책임 변화가 없어 개발 흐름 3에 추가하지 않았습니다.
- 같은 `cdaca35ccf7f`는 친구 관계 신원과 토너먼트 정원을 한 시험 파일에서
  함께 검증하므로 개발 흐름 4와 개발 흐름 5에서 각 불변식 관점으로 교차 참조합니다.
...
- 2단계는 이 디렉터리의 동결 이후 작업 틀을 수정하지 않고 완료 문서만 채웁니다.
===== END FILE: README.md =====
