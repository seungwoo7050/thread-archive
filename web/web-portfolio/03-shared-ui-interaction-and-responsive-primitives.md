...
- 이 문서의 개발 흐름 범위, 커밋 집합, 순서, 역할과 커밋별 확인 사항은 1단계 검토에서 확정했습니다.
...
**경계:** 이 개발 흐름은 링크가 어디에 노출되고 어떤 요소·URL로 이동하는지를 소유합니다. `ProjectCard`의 카드 조립과 마우스 오버 표현은 06 개발 흐름에 남기며, 라우트 실행 주기 자체는 이 범위에 포함하지 않습니다.

### 고정 불변 조건

- 프로젝트 카드와 상세 화면의 사용 가능 여부·노출 위치는 선택자가 결정하고, 히어로는 같은 노출 위치 값을 직접 확인합니다. 링크 이동 렌더러는 어떤 링크를 표시할지 다시 판단하지 않습니다.
- 외부 링크는 기준 이동 방식과 외부 속성을 사용하고, 내부 링크는 Next Link와 현재 `view`·`디버그` 쿼리 보존 규칙을 사용합니다.
...
- 카드·상세·히어로 노출 위치가 언제 도입되고 각 소비자의 필터링이 선택자 또는 직접 판정 함수 중 어디에 남았는가?
- 09cec616f314의 테스트가 원본 순서, 쿼리 보존, 외부 링크 속성, 빈 출력을 어떤 방법으로 고정하는가?
- 44e4d062da50의 리팩터링이 동작을 바꾸지 않고 어떤 렌더링 책임만 합쳤는가?

## 3. 완료 기준

- 각 SHA의 부모 커밋과의 차이와 변경 후 파일 트리를 구분해 실제 파일·심볼·호출 경로를 기록합니다.
...
| 8 | `ee2c118a76d6` | feat(content): 홈 링크를 배치 기준으로 선택 | B | CONTENT | 히어로 소비자 전환 |
...
- 먼저 부모 커밋과의 차이와 해당 SHA의 변경 후 파일 트리를 구분합니다.
...
- **정적 검토:** `web/portfolio` 전용 커밋 분류와 해당 SHA의 커밋 변경 내용·변경 파일 화면을 대조했습니다.
- **실행:** 하지 않았습니다. 작업 환경에서 `github.com` DNS 해석이 실패해 실행 가능한 체크아웃을 만들 수 없었고, GitHub 연결 도구는 원본과 변경 내용 정적 검토만 제공했습니다. 따라서 명령, 통과·실패, 시점, 브라우저 결과를 주장하지 않습니다.
...
| 다음 커밋 또는 관련 테스트 연결 | 2d87b62dcce8은 상세 소비자를 선택자로 이동하고, ee2c118a76d6은 히어로 소비자가 동일 노출 위치 값 집합을 직접 판정 함수로 채택하게 합니다. |
...
- **Importance:** B
- **Tags:** CONTENT
- **개발 흐름에서의 역할:** 히어로 소비자 전환

#### 해당 SHA에서 확인할 실제 코드

- Classic/Design 홈 라우트에서 코드에 고정된 링크 타입 필터가 히어로 노출 위치 필터로 교체된 변경 내용을 확인합니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 홈 히어로는 github·이력서·website 타입을 코드에 직접 열거해 콘텐츠 노출 위치와 분리되어 있었습니다. |
| 실제 변경 파일·심볼·호출 경로 | 두 홈 구현이 `link.placements?.includes("hero")`를 사용하도록 변경됐습니다. |
| 데이터·상태·DOM·자원 소유 주체 | 콘텐츠가 히어로 포함 관계를 선언하고 각 디자인 라우트가 그 노출 위치 판정 함수와 레이아웃·스타일 적용을 직접 적용합니다. 이 시점에는 히어로 선택 소유 주체가 선택자로 이동하지 않았습니다. |
| 실패·누락·대체 처리 | 히어로 노출 위치가 없는 링크는 타입과 무관하게 제외됩니다. 이 커밋은 일반적인 사이트 선택자를 호출하지 않고 동일 판정 함수를 두 라우트에 둡니다. |
...
- 같은 커밋의 `design-switcher.test.tsx`가 브라우저 기본 `<details>` 닫기·포커스를 검증하지만 이 개발 흐름에서는 링크 규칙과의 경계만 기록합니다.
...
| 다음 커밋 또는 관련 테스트 연결 | 44e4d062da50은 이 테스트 규칙을 유지하면서 중복 렌더러를 추출합니다. 디자인 선택기 테스트의 후속 하이드레이션 변경 과정는 08 개발 흐름의 외부 관계 및 07 분류가 소유합니다. |
...
| 다음 커밋 또는 관련 테스트 연결 | 프로젝트 동작의 최종 layering은 콘텐츠 → 선택자 → 표시 영역 래퍼 → ProjectLinkList → `ContentLinkView`입니다. 히어로는 콘텐츠 노출 위치를 라우트 소비자가 직접 필터링한 뒤 `ContentLinkView`를 사용합니다. |
...
| 배치 기반 노출 | daa6815a6dfa → 119ff9a92090 | 카드 검사 단계, LinkPlacement와 일반적인 선택자 | 소비자 전환 필요 | 카드·상세·히어로가 콘텐츠 노출 위치를 따름 |
...
| daa/119/2d/ee | 콘텐츠 노출 위치 + 선택자·히어로 소비자 | 카드·상세는 선택자가 해석하고 히어로는 같은 노출 위치 판정 함수를 소비자가 직접 적용 |
...
- 선호 연락처와 프로젝트 카드·상세 링크는 콘텐츠 원본에서 선택자를 거쳐 파생되며, 홈 히어로는 라우트 소비자가 콘텐츠 노출 위치를 직접 필터링합니다.
- 카드·상세·히어로는 콘텐츠 노출 위치를 따릅니다. 프로젝트 데모는 카드·상세 표시 여부 경로에서 배포 상태 배포 중와 데모 href 존재를 추가로 요구합니다.
- 표시 영역 래퍼는 선택된 배열과 로컬 옵션만 처리하고, 공용 목록이 빈·classes·아이콘를 처리합니다.
- `ContentLinkView`는 외부 기준과 내부 Next Link를 분기합니다.
- 잘못된 href, 원격 접근 가능 여부, 실제 브라우저 탐색은 이 개발 흐름이 보장하지 않습니다.
...
2. 카드·상세는 선택자가 노출 위치와 사용 가능 여부를 적용해 원본 순서를 보존한 배열을 만들고, 히어로 소비자는 같은 `hero` 노출 위치 판정 함수를 직접 적용합니다.
...
- [x] SHA, 제목, 순서, 중요도, 태그와 개발 흐름 역할을 검토 후 고정한 문서 틀과 동일하게 유지했습니다.
- [x] 이전 상태, 소유 주체, 누락·실패, 보장 범위와 보장하지 않는 범위와 후속 참조 관계를 채웠습니다.
...
프로필 사진과 프로젝트 화면 캡처 공용 컴포넌트가 콘텐츠 메타데이터를 어떻게 보존하고, 히어로와 갤러리 소비자가 즉시 로딩·지연 로딩 및 고정 aspect 레이아웃을 어떻게 선택하는지 복원합니다.

**경계:** 이 개발 흐름은 공용 미디어 DOM과 로딩·레이아웃 규칙을 다룹니다. 이미지 최적화 처리 단계, 원본 자산 생성, 프로젝트 상세 정보 설계 자체는 포함하지 않습니다.
...
- Lead·상세 히어로가 우선순위를 명시하고 갤러리가 기본값 지연 로딩을 유지하는 소비자 차이는 무엇인가?
...
- 수정과 테스트는 실제로 수정·검증하는 실제 코드 경로에 연결합니다.
- 실행하지 않은 명령 결과를 만들지 않습니다.
- 이 개발 흐름은 중요도 B 커밋만 포함하므로 각 커밋의 구체적인 역할, 구분 지점, 실패와 보장하지 않는 범위를 필요한 범위로 기록합니다.
...
| 3 | `06fff9a6e93b` | feat(project): 프로젝트 상세 소개 추가 | B | RENDERER | 상세 히어로 소비자 |
...
| 다음 커밋 또는 관련 테스트 연결 | 06fff9a6e93b가 상세 히어로에도 같은 우선순위 규칙을 적용합니다. |
...
- **Importance:** B
- **Tags:** RENDERER
- **개발 흐름에서의 역할:** 상세 히어로 소비자

#### 해당 SHA에서 확인할 실제 코드

- 새 `ProjectDetailView`에서 히어로 화면 캡처 위치와 `priority` 속성을 확인합니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 프로젝트 상세 view와 그 히어로 미디어 소비자가 없었습니다. |
| 실제 변경 파일·심볼·호출 경로 | `ProjectDetailView`가 콘텐츠 요약·설명·동작 옆에 `ProjectScreenshot image={project.screenshot} priority`를 배치합니다. |
| 데이터·상태·DOM·자원 소유 주체 | Detail view가 히어로 노출 위치와 우선순위를, 공용 컴포넌트가 프레임·로딩을 소유합니다. |
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | Detail 히어로 한 장만 표현됐고 추가 근거 화면 캡처를 순서대로 보여 주는 갤러리가 없었습니다. |
...
| 보장하는 것과 보장하지 않는 것 | 히어로는 즉시 로딩하고 갤러리는 지연 로딩하도록 구분합니다. 화면에 보이는 시점을 기준으로 한 별도 사전 로딩이나 동시 실행 제어는 없습니다. |
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | `ProfilePhoto` 공용 컴포넌트는 있었지만 소개 히어로가 사진 필드를 소비하지 않아 선택적 프로필 미디어 규칙이 실제 라우트에서 완결되지 않았습니다. |
| 실제 변경 파일·심볼·호출 경로 | 소개 히어로를 문구·사진 그리드로 바꾸고 사진이 있을 때만 `ProfilePhoto`를 렌더링합니다. ContentHint 경로도 사진을 포함합니다. |
...
| 첫 화면에 보이는 우선순위 | b027f42669aa, 06fff9a6e93b | 우선순위 속성 → 즉시 로딩 | 실제 네트워크 우선순위 측정 없음 | 홈·상세 히어로가 즉시 로딩을 명시 |
...
| 모든 이미지를 같은 로딩으로 처리 | 히어로 지연 또는 갤러리 과도 즉시 로딩 가능 | 우선순위 속성과 소비자별 선택 | 정적 속성 경로만 확인; 네트워크 성능 테스트 없음 |
<!-- learner:thread-relations:end -->
...
- [x] SHA, 제목, 순서, 중요도, 태그와 개발 흐름 역할을 검토 후 고정한 문서 틀과 동일하게 유지했습니다.
- [x] 이전 상태, 소유 주체, 누락·실패, 보장 범위와 보장하지 않는 범위와 후속 참조 관계를 채웠습니다.
- [x] 이 개발 흐름에는 S·A 커밋이 없으며 중요도 B 범위에서 저장소에 맞춘 깊이를 유지했습니다.
- [x] 실행 상태를 사실대로 기록했습니다: 실행 명령은 실행하지 않았고 정적 검토와 구분했습니다.
- [x] 빈 학습자용 답변 cell을 남기지 않았습니다.
<!-- learner:thread-checklist:end -->
...
# 개발 흐름: 점진적 표시에서 서버 렌더링 우선 방식으로 전환
...
**경계:** 이 개발 흐름은 공용 등장 효과의 표시 여부·실행 주기와 동작 효과 대체 처리를 다룹니다. 개별 섹션 콘텐츠, 라우트 조립, 전역 성능 테스트 설계는 포함하지 않습니다.
...
- 최종 서버 렌더링 우선 전환을 직접 검증하는 테스트가 이 고정된 개발 흐름에 존재하는가?
...
- **개발 흐름에서의 역할:** 서버 렌더링 우선 표시 여부 전환
...
| 다음 커밋 또는 관련 테스트 연결 | 이 고정된 개발 흐름에는 이 리팩터링를 실행한 전용 회귀 테스트가 없습니다. 분류 07의 별도 성능·회귀 분류와 정적 원본 정적 검토가 후속 검증 맥락입니다. |
...
| 실패·위험 | 실제 영향·근본 원인 | 수정·결정 | 회귀 근거 또는 공백 |
| --- | --- | --- | --- |
| 서버 렌더링 시 숨김 + 하이드레이션·관찰자 필요 | JS 지연·실패 시 콘텐츠 표시 여부 위험 | b8164cfdddbd 서버에서 즉시 표시되는 마크업 | 고정된 개발 흐름 내 전용 실행 시점 테스트 없음; 정확한 변경 내용으로 정적 확인 |
...
- 고정된 개발 흐름 안에는 서버 렌더링 우선 불변 조건을 실행 검증하는 전용 테스트가 없습니다.
<!-- learner:thread-final-state:end -->
...
AnimatedTerminal의 자리표시자 형식 변환, typing/hold/erase 타이머 상태 기계, 정리, 터미널 CSS와 동작 줄이기 설정 동작, Classic 히어로 통합을 복원합니다.

**경계:** 이 개발 흐름은 터미널 미리보기 공용 컴포넌트의 상태와 표현을 다룹니다. 터미널 콘텐츠 스키마 검증, 홈 라우트 전체 섹션 설계, 일반 동작 효과 규칙은 각각 다른 개발 흐름의 책임입니다.
...
| 4 | `cdb68fdf59f9` | feat(home): 클래식 홈 히어로 구성 | B | RENDERER | Classic 히어로 통합 |
...
| 다음 커밋 또는 관련 테스트 연결 | cdb68fdf59f9가 터미널 컴포넌트를 Classic 히어로에 배치합니다. |
...
| 실제 변경 파일·심볼·호출 경로 | Classic 히어로가 AnimatedTerminal에 프로필, 프로젝트 개수, 기술 스택 개수, 화면 구성 터미널 객체를 전달하고 터미널 래퍼에 배치합니다. |
...
| 실패·위험 | 실제 영향·근본 원인 | 수정·결정 | 회귀 근거 또는 공백 |
| --- | --- | --- | --- |
| 타이머 효과가 정리되지 않을 위험 | 컴포넌트 해제 뒤 상태 갱신 또는 중복 타이머 | `f60d46857715`의 정리 함수에서 `clearTimeout` 호출 | 전용 가짜 타이머 테스트는 고정된 개발 흐름에 없음 |
| 동작 줄이기 설정 무시 위험 | typing/CSS 애니메이션 지속 | 효과 즉시 반환 + CSS 애니메이션 없음 | 정적 브랜치 확인; 브라우저 설정 테스트 없음 |
| 빈 명령 입력 | activeCommand un정의된 및 modulo 0 위험 | 이 개발 흐름에서 수정 없음 | 콘텐츠 검증이 전제이며 회귀 테스트 없음 |
<!-- learner:thread-relations:end -->
...
<!-- learner:thread-final-state:start -->
- Classic 히어로가 콘텐츠 값으로 결정되는 터미널 미리보기를 렌더링합니다.
...
**경계:** 이 개발 흐름은 기술 스택 시각 공용 컴포넌트를 다룹니다. 기술 스택 콘텐츠 스키마와 `resolveTechStackItem`의 검증·대체 처리 기준, 홈 섹션 전체 조립은 다른 개발 흐름이 소유합니다.
...
- 누락된 ID 검증, 빈 흐르는 목록 생략, 실행 시점 시각·보조 기술 테스트는 이 개발 흐름에 없습니다.
<!-- learner:thread-final-state:end -->
...
**경계:** 1단계에서 기존 `project-card-actions-and-evidence-components` 개발 흐름의 링크 규칙 커밋을 01 개발 흐름으로 이동했습니다. 이 개발 흐름은 카드 조립과 상호작용만 소유하며 링크 노출 위치·이동 방식은 01 개발 흐름의 선택자·렌더러를 소비합니다.
...
- **Importance:** B
- **Tags:** RENDERER
- **개발 흐름에서의 역할:** 사용 가능 여부 배지 공용 컴포넌트
...
- **Importance:** B
- **Tags:** RENDERER
- **개발 흐름에서의 역할:** 마우스 오버·overlay와 동작 효과 대체 처리
...
| 다음 커밋 또는 관련 테스트 연결 | 이 개발 흐름의 최종 카드 경로는 공용 하위 공용 컴포넌트 + 표시 영역 소비자 + 클릭을 막지 않는 CSS overlay입니다. |
...
| 실패·위험 | 실제 영향·근본 원인 | 수정·결정 | 회귀 근거 또는 공백 |
| --- | --- | --- | --- |
| 카드가 링크 규칙을 직접 소유할 위험 | 01 개발 흐름과 중복·불일치 | b72c52a22690은 `ProjectCardLinks`를 소비만 함 | 09cec616f314이 하위 링크 규칙을 검증 |
...
| 마우스 오버 시 떠오르는 효과가 동작 줄이기 설정을 무시할 위험 | 동작 효과 민감도 문제 | `4000a8657a62`에서 동작 줄이기 환경의 카드 효과를 비활성화 | 전역 접근성 규칙은 03 개발 흐름에서 추가로 보강 |
<!-- learner:thread-relations:end -->
...
- Action 링크 표시 여부·이동 방식은 01 개발 흐름의 `ProjectCardLinks`/`ContentLinkView`에 위임됩니다.
...
# 개발 흐름: 여정 타임라인 공용 컴포넌트와 반응형 레이아웃
...
**경계:** 이 개발 흐름은 여정 화면 구성 공용 컴포넌트와 반응형 CSS를 다룹니다. 여정 JSON 스키마·검증, 라우트 섹션 복사, 등장 효과의 최종 서버 렌더링 우선 리팩터링은 각각 콘텐츠 시스템과 03 개발 흐름이 소유합니다.
...
- 03 개발 흐름의 서버 렌더링 우선 등장 효과 전환이 `animated` 설정값의 최종 의미를 어떻게 약화시키는가?
...
| 다음 커밋 또는 관련 테스트 연결 | 29bb40579cb2/af9191fc15ad의 동작 줄이기 설정 규칙과 b8164cfdddbd의 서버 렌더링 우선 리팩터링이 개발 흐름 간으로 이 클래스 동작을 바꿉니다. |
...
| 등장 효과와 표시 여부의 결합 | JS·하이드레이션 의존 시 안내선·노드 표시 여부 위험 | 03 개발 흐름의 b8164cfdddbd 서버에서 즉시 표시되는 리팩터링 | 해당 여정 커밋 자체에는 테스트 없음 |
<!-- learner:thread-relations:end -->
...
- 날짜 검증, 빈 목록 생략, 전용 단위·시각 테스트는 이 개발 흐름에 없습니다.
<!-- learner:thread-final-state:end -->
...
# 개발 흐름: 디자인 선택기 펼침 동작과 반응형 선택 패널
...
**경계:** 1단계에서 분류 README가 약속했지만 기존 7개 개발 흐름이 다루지 않던 반응형 펼침 요소 공용 컴포넌트 구현 변경 과정을 추가했습니다. 실제 공용 셸 연결은 분류 02의 `b9571c485013`이, 후속 하이드레이션 경쟁 상태 수정·테스트와 서버 컴포넌트 리팩터링은 분류 07이 소유합니다. 이 고정된 커밋 목록에는 공용 컴포넌트 구현 커밋만 두고 인접 분류의 통합·수정·근거는 관계로 명시합니다.
...
아래 커밋은 같은 배포 환경 변경 과정의 통합·수정·근거이지만 인접 분류가 소유합니다. 분류 중복을 피하기 위해 이 개발 흐름의 커밋 목록에는 넣지 않습니다.
...
- **Importance:** B
- **Tags:** RENDERER
- **개발 흐름에서의 역할:** 모바일 아래 선택 패널 스타일 적용
...
- **Importance:** B
- **Tags:** RENDERER
- **개발 흐름에서의 역할:** 클라이언트 `<details>` 요소 열기 버튼과 현재 선택 상태
...
| 불변 조건 | 도입·변경 커밋 | 실제 코드·테스트 근거 | 부족함이 드러난 지점 | 최종 보장 범위 |
| --- | --- | --- | --- | --- |
| 브라우저 기본 펼침 상태 | e43e8addd7f3 | `<details>` 요소·요약 DOM | 클라이언트 하이드레이션 경쟁 상태 미해결 | 초기 개발 흐름에서는 브라우저 기본 `open` 속성이 상태 소유 주체 |
...
| 클라이언트에서 렌더링된 브라우저 기본 `<details>` | 하이드레이션 전 사용자가 `open` 속성을 바꾸면 속성 불일치 위험 | c702b870d57a suppress warning 후 a37cb8596733 서버 마크업 전환 | b6c0238ab8b8 서버 렌더링 → 브라우저 기본 `open` 상태 변경 → 하이드레이션 결정적인 테스트, 1ac7813155c6 원본 입력 경계 테스트 |
...
- 01에는 노출 위치 값 집합, 상세 선택자 전환, 히어로 노출 위치 adoption, 회귀 테스트와 렌더링 중복 제거 커밋을 추가해 규칙 → 소비자 → 테스트 → 리팩터링 흐름을 완성했습니다.
...
- README가 약속했지만 빠져 있던 브라우저 기본 펼침 요소·모바일 선택 패널 구현 변경 과정을 08 개발 흐름으로 추가했습니다.
- 08의 실제 공용 셸 통합은 분류 02의 `b9571c485013`이, 후속 하이드레이션 수정·테스트·서버 리팩터링은 분류 07이 소유하므로 여기서는 중복하지 않고 관계만 고정했습니다.
- 나머지 개발 흐름의 경계와 커밋 순서는 실제 이력에 맞아 유지했습니다.
- 공용 의존성과 소비자 변경 과정가 전역 이력에서 서로 교차하므로 개발 흐름 번호를 전역 커밋 시간순 기록으로 강제하지 않았습니다. 각 개발 흐름 내부는 chronological 순서이고 분류 목록은 의존성·학습 경계를 따릅니다.
...
- 실행 가능한 체크아웃은 작업 환경의 GitHub DNS 해석 실패로 만들지 못했습니다. 실행 시점 테스트 결과는 없으며 모든 완료 문서가 이를 명시합니다.

## 개발 흐름 목록

| 순서 | 개발 흐름 | 커밋 수 | 경계 |
| --- | --- | --- | --- |
| 1 | [콘텐츠 링크 보안, 노출 위치, 및 이동 방식](01-content-link-security-and-transport.md) | 10 | 이 개발 흐름은 링크가 어디에 노출되고 어떤 요소·URL로 이동하는지를 소유합니다. `ProjectCard`의 카드 조립과 마우스 오버 표현은 06 개발 흐름에 남기며, 라우트 실행 주기 자체는 이 범위에 포함하지 않습니다. |
| 2 | [콘텐츠 미디어 로딩 및 레이아웃 안정성](02-content-media-loading-and-layout-stability.md) | 5 | 이 개발 흐름은 공용 미디어 DOM과 로딩·레이아웃 규칙을 다룹니다. 이미지 최적화 처리 단계, 원본 자산 생성, 프로젝트 상세 정보 설계 자체는 포함하지 않습니다. |
| 3 | [점진적 등장 효과에서 서버 우선 렌더링으로](03-progressive-reveal-to-server-first-rendering.md) | 4 | 이 개발 흐름은 공용 등장 효과의 표시 여부·실행 주기와 동작 효과 대체 처리를 다룹니다. 개별 섹션 콘텐츠, 라우트 조립, 전역 성능 테스트 설계는 포함하지 않습니다. |
| 4 | [터미널 상태 기계 및 동작 효과 대체 처리](04-terminal-state-machine-and-motion-fallback.md) | 4 | 이 개발 흐름은 터미널 미리보기 공용 컴포넌트의 상태와 표현을 다룹니다. 터미널 콘텐츠 스키마 검증, 홈 라우트 전체 섹션 설계, 일반 동작 효과 규칙은 각각 다른 개발 흐름의 책임입니다. |
| 5 | [기술 스택 아이콘, 목록, 및 흐르는 목록](05-technology-stack-icon-list-and-marquee.md) | 5 | 이 개발 흐름은 기술 스택 시각 공용 컴포넌트를 다룹니다. 기술 스택 콘텐츠 스키마와 `resolveTechStackItem`의 검증·대체 처리 기준, 홈 섹션 전체 조립은 다른 개발 흐름이 소유합니다. |
| 6 | [프로젝트 카드 조립 및 상호작용](06-project-card-composition-and-interaction.md) | 4 | 1단계에서 기존 `project-card-actions-and-evidence-components` 개발 흐름의 링크 규칙 커밋을 01 개발 흐름으로 이동했습니다. 이 개발 흐름은 카드 조립과 상호작용만 소유하며 링크 노출 위치·이동 방식은 01 개발 흐름의 선택자·렌더러를 소비합니다. |
| 7 | [여정 타임라인 공용 컴포넌트 및 반응형 레이아웃](07-journey-timeline-primitives-and-responsive-layout.md) | 6 | 이 개발 흐름은 여정 화면 구성 공용 컴포넌트와 반응형 CSS를 다룹니다. 여정 JSON 스키마·검증, 라우트 섹션 복사, 등장 효과의 최종 서버 렌더링 우선 리팩터링은 각각 콘텐츠 시스템과 03 개발 흐름이 소유합니다. |
| 8 | [Design 선택기 펼침 요소 및 반응형 선택 패널](08-design-switcher-disclosure-and-responsive-sheet.md) | 4 | 1단계에서 분류 README가 약속했지만 기존 7개 개발 흐름이 다루지 않던 반응형 펼침 요소 공용 컴포넌트 구현 변경 과정을 추가했습니다. 실제 공용 셸 연결은 분류 02의 `b9571c485013`이, 후속 하이드레이션 경쟁 상태 수정·테스트와 서버 컴포넌트 리팩터링은 분류 07이 소유합니다. 이 고정된 커밋 목록에는 공용 컴포넌트 구현 커밋만 두고 인접 분류의 통합·수정·근거는 관계로 명시합니다. |
