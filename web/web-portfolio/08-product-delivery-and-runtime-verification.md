...
# 개발 과정: 재현 가능한 개발 도구 버전과 배포용 서버 검증

> 프로젝트: 42 Archive Portfolio (`web/portfolio`)
>
> 이 문서는 원본 개발 과정을 변경하지 않고, 같은 브랜치 이력에 제품 전달 관점을 추가한 확장 학습 문서입니다.

## 0. 분류 출처와 변경 가능 범위

- 커밋 SHA·제목·중요도·태그는 `commit/commit-importance.md`의 브랜치별 분류를 사용합니다.
- 1단계 검토에서 분류와 개발 과정 묶음, 커밋 집합을 실제 이력과 대조한 뒤 이 문서를 고정했습니다.
...
- 버전 파일과 `package.json` 선언은 어떤 소비자에게 읽히며, 지원하지 않는 로컬 실행 환경 버전을 실제로 차단하는가?
...
- 각 SHA를 부모 커밋과 비교하고, 변경 후 파일 트리에서 실제 변경 파일, 함수, 설정, 스크립트와 작업 단계를 확인했습니다.
- 원본, 생성 산출물, CI 검사 단계, 컨테이너와 실행 중 자원의 소유 주체를 구분했습니다.
- 누락된 산출물, 이식성 실패, 기준값 초과, 시작 실패와 정리 브랜치를 기록했습니다.
- 테스트·CI 명령의 방법, 실제 코드 경로, 증명 범위, 증명하지 않는 범위, 실제 실행 여부를 구분했습니다.
- 최종 제품 전달 순서와 다른 개발 과정으로 넘기는 지점을 코드 없이 설명할 수 있습니다.

## 4. 커밋 목록

| 순서 | 커밋 | 제목 | 중요도 | 태그 | 확장 개발 과정에서 확인할 역할 |
| ---: | --- | --- |:---: | --- | --- |
...
각 섹션은 반드시 해당 SHA의 파일 트리와 부모와의 변경 차이를 기준으로 작성합니다. 다른 개발 과정의 후속 커밋은 관계 설명에만 사용하고 과거 구현에 소급하지 않습니다.

### 1. `f66b880a8f97` — chore(runtime): 지원 Node.js와 npm 버전 고정

- **전체 SHA:** `f66b880a8f975443974896891a71e1dfd70fbe32`
- **중요도:** B
- **태그:** DEPLOY
- **확장 개발 과정에서의 역할:** 초기 전달 경계 — 실행 환경·패키지 관리자 버전을 저장소 규칙으로 고정합니다.
...
| 실패·누락된 산출물·정리 처리 | 이 SHA에는 버전 불일치를 직접 검사해 실패시키는 명령이 없습니다. npm의 엔진 처리는 환경 설정에 따라 경고에 그칠 수 있고, 현재 셸이 실제로 선언값을 사용했는지도 보장하지 않습니다. |
| 보장하는 것과 보장하지 않는 것 | 네 곳의 실행 환경 버전 선언과 잠금 파일 최상위 메타데이터가 같은 값으로 수렴합니다. 그러나 의존성 설치의 네트워크 가용성, OS 차이, 또는 지원하지 않는 실행 환경 버전의 강제 차단까지 보장하지 않습니다. |
...
- **전체 SHA:** `f81691072413c11707251491fdb5af0c67e716ec`
- **중요도:** A
- **태그:** VALIDATION, DEPLOY, TEST
- **확장 개발 과정에서의 역할:** 회귀·산출물 검증 — 배포용 빌드와 별도 서버 포트를 사용하는 브라우저 검증 경로를 만듭니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `9fd3541c11dc`가 이 정확한 명령을 CI의 마지막 검증 단계로 호출합니다. 뒤의 독립 실행형·컨테이너 개발 과정은 같은 빌드 산출물을 더 좁은 산출물·실행 시점 경계에서 검증합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `9fd3541c11dc`가 이 정확한 명령을 CI의 마지막 검증 단계로 호출합니다. 뒤의 독립 실행형·컨테이너 개발 과정은 같은 빌드 산출물을 더 좁은 산출물·실행 시점 경계에서 검증합니다.

### 3. `9fd3541c11dc` — ci: 기본 배포 품질 검사 추가

- **전체 SHA:** `9fd3541c11dc8fa86561c3cd0b7116449b8f0f03`
- **중요도:** A
- **태그:** DEPLOY, TEST
- **확장 개발 과정에서의 역할:** 통합 검사 단계 — 고정 개발 도구 버전과 배포용 E2E를 GitHub Actions의 기본 전달 품질 검사로 연결합니다.

#### 해당 SHA에서 확인할 실제 코드

- `.github/workflows/ci.yml`의 열기 버튼, 권한, 동시 실행, 시간 제한과 단계 순서를 부모와의 변경 차이에서 확인합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 개발 과정 3의 `c5e73853a1b6`, 개발 과정 4의 `abbd530368a0`, 개발 과정 5의 `b94fa6dd0118`이 같은 작업 정의에 산출물, 성능, 컨테이너 검사 단계를 단계적으로 추가합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 개발 과정 3의 `c5e73853a1b6`, 개발 과정 4의 `abbd530368a0`, 개발 과정 5의 `b94fa6dd0118`이 같은 작업 정의에 산출물, 성능, 컨테이너 검사 단계를 단계적으로 추가합니다.
...
| dev 서버 통과를 배포 환경 성공으로 오인 | `f81691072413`의 빌드 첫 번째 배포 환경 Playwright 설정 | `9fd3541c11dc`가 정확한 명령을 CI에서 실행 | 독립 실행형·컨테이너는 별도 개발 과정 |
...
브랜치는 지원 Node/npm을 명시하고, 배포용 빌드를 만든 뒤 별도 `next start` 프로세스에서 기존 데스크톱·모바일 라우트 조합표를 실행할 수 있습니다. CI는 새 체크아웃에서 같은 버전과 명령을 사용해 push/PR을 판정합니다. 다만 이 개발 과정의 마지막 SHA만으로는 독립 실행형 파일 배치, 성능 허용량, 컨테이너 이미지, 배포 환경 호스팅을 검증하지 않습니다.
...
- [x] 이 개발 과정이 독립 실행형, 성능, 컨테이너, 호스팅을 보장하지 않는다고 기록했습니다.
...
- webpack 컴파일러 pin, Next 패치 갱신와 GNU/musl SWC 잠금 파일 메타데이터가 이식성에 어떤 서로 다른 역할을 갖는가?
...
| 3 | `404a220e5d40` | fix(build): production build에 webpack compiler 고정 | B | DEPLOY, DEBUG | 컴파일러 선택 고정 — 배포 산출물을 후속 명세 파일 도구가 이해하는 webpack 형식으로 만듭니다. |
...
- **전체 SHA:** `7872e1214de7b5d58722358998c6d63cfbe9f279`
- **중요도:** A
- **태그:** DEPLOY, DEBUG
- **확장 개발 과정에서의 역할:** 외부 빌드 의존성 제거 — Google Font fetch를 저장소가 소유한 WOFF2와 `next/font/local`로 교체합니다.
...
- **전체 SHA:** `2f65f6a6fcb6e753497ded0f4a8948cfa0238c48`
- **중요도:** A
- **태그:** VALIDATION, DEPLOY, TEST
- **확장 개발 과정에서의 역할:** 결정적 원본·파일 회귀 — 로컬 글꼴 등록, WOFF2 파일과 라이선스 고지문을 검사합니다.
...
- 라이선스 검사가 두 파일의 특정 문구만 확인하며 측정 출처 해시나 전체 라이선스 동일성을 검증하지 않는다는 점을 구분합니다.
- 배포용 빌드·브라우저를 실행하는 테스트가 아니라 원본 + 저장소 파일 입력 검증 지점 테스트라는 방법을 분류합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `7872e1214de7`의 로컬 소유 주체 불변 조건을 직접 보호합니다. 컴파일러·CSS 변환은 뒤의 별도 수정과 개발 과정 간 빌드·시각 검증이 맡습니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `7872e1214de7`의 로컬 소유 주체 불변 조건을 직접 보호합니다. 컴파일러·CSS 변환은 뒤의 별도 수정과 개발 과정 간 빌드·시각 검증이 맡습니다.

### 3. `404a220e5d40` — fix(build): production build에 webpack compiler 고정

- **전체 SHA:** `404a220e5d408d39e360a7fe2149042a6e2af3ee`
- **중요도:** B
- **태그:** DEPLOY, DEBUG
- **확장 개발 과정에서의 역할:** 컴파일러 선택 고정 — 배포 산출물을 후속 명세 파일 도구가 이해하는 webpack 형식으로 만듭니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 개발 과정 4의 `c2fb8a7c238d` 파서와 `c24c350ce42c` 컴파일러 규칙 테스트가 이 선택을 실제 측정 불변 조건으로 사용합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 개발 과정 4의 `c2fb8a7c238d` 파서와 `c24c350ce42c` 컴파일러 규칙 테스트가 이 선택을 실제 측정 불변 조건으로 사용합니다.

### 4. `5d903132306a` — fix(deps): Next.js runtime 보안 패치 적용

- **전체 SHA:** `5d903132306a1ab6db0fe715415e1527f63ebb93`
- **중요도:** B
- **태그:** DEPLOY, DEBUG
- **확장 개발 과정에서의 역할:** 프레임워크·네이티브 모듈 이식성 유지보수 — Next, ESLint 설정과 플랫폼 SWC 해석을 같은 패치 버전 계열로 갱신합니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 전달 상태와 부족함 | Next 실행 시점과 matching ESLint·컴파일러 패키지가 16.2.4 패치 버전대에 고정돼 있었습니다. 커밋 제목은 보안 유지보수 필요를 나타내지만 저장소 근거에는 특정 CVE나 재현 상황이 없습니다. |
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 앞의 webpack pin과 뒤의 빌드 측정·컨테이너 설치가 같은 패치 버전의 의존성 관계를 사용합니다. 이 커밋은 글꼴이나 Tailwind 실패를 직접 고친 것이 아니라 이식성 개발 과정의 프레임워크·브라우저 기본 의존성 경계를 보강합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 앞의 webpack pin과 뒤의 빌드 측정·컨테이너 설치가 같은 패치 버전의 의존성 관계를 사용합니다. 이 커밋은 글꼴이나 Tailwind 실패를 직접 고친 것이 아니라 이식성 개발 과정의 프레임워크·브라우저 기본 의존성 경계를 보강합니다.

### 5. `1de3d36e3a48` — fix(build): Tailwind utility CSS 변환 복원

- **전체 SHA:** `1de3d36e3a485830b0a459cbc9dc9748ca15d763`
- **중요도:** A
- **태그:** DEPLOY, DEBUG
- **확장 개발 과정에서의 역할:** 배포 환경 변환 복원 — 설치된 Tailwind PostCSS 플러그인을 명시적인 루트 설정으로 등록합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 후속 배포 환경 시각 회귀 테스트 모음과 Lighthouse/CI가 포괄적인 산출물 결과를 검사하지만, PostCSS 설정 하나만을 격리한 직접 테스트는 브랜치에 없습니다. 개발 과정 4의 CI 활성화 전에 이 수정이 들어가므로 성능 수치가 스타일 적용이 빠진 산출물을 기준으로 확정되는 위험을 줄입니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 후속 배포 환경 시각 회귀 테스트 모음과 Lighthouse/CI가 포괄적인 산출물 결과를 검사하지만, PostCSS 설정 하나만을 격리한 직접 테스트는 브랜치에 없습니다. 개발 과정 4의 CI 활성화 전에 이 수정이 들어가므로 성능 수치가 스타일 적용이 빠진 산출물을 기준으로 확정되는 위험을 줄입니다.
...
| 컴파일러 출력 형식 | 배포용 컴파일러 기본값 | `404a220e5d40`에서 webpack을 패키지 스크립트에 고정 | 개발 과정 4 파서·규칙 테스트가 소비 | Next 내부 형식의 향후 변화 |
...
| 프레임워크 기본값 컴파일러 불일치 | `next build --webpack` | 개발 과정 4의 정확한 스크립트·파서 테스트 입력 테스트 | 직접 `next build` 호출은 우회 가능 |
...
- [x] webpack pin과 명세 파일 파서 규칙의 개발 과정 간 관계를 설명했습니다.
- [x] Next 패치 갱신의 보안 제목을 특정 CVE 대상 주장으로 확대하지 않았습니다.
...
- 이 개발 과정의 산출물 검증 규칙이 다음 Docker 개발 과정에 어떻게 인계되는가?
...
| 순서 | 커밋 | 제목 | 중요도 | 태그 | 확장 개발 과정에서 확인할 역할 |
| ---: | --- | --- |:---: | --- | --- |
| 1 | `29508f4668ea` | build: standalone server 산출물 생성 | B | DEPLOY | 산출물 형식 선택 — Next.js가 추적된 실행 의존성을 포함한 독립 실행형 서버 번들을 생성하도록 설정합니다. |
...
- **전체 SHA:** `29508f4668eaed37c393c8c2ef2e80d0e6c8e2f2`
- **중요도:** B
- **태그:** DEPLOY
- **확장 개발 과정에서의 역할:** 산출물 형식 선택 — Next.js가 추적된 실행 의존성을 포함한 독립 실행형 서버 번들을 생성하도록 설정합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `c0f7434467a0`이 최소 필수 산출물을 검사하고, 개발 과정 5의 Dockerfile이 독립 실행형 서버와 정적·공개를 명시적으로 분리해 복사합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `c0f7434467a0`이 최소 필수 산출물을 검사하고, 개발 과정 5의 Dockerfile이 독립 실행형 서버와 정적·공개를 명시적으로 분리해 복사합니다.

### 2. `c0f7434467a0` — test(build): standalone 산출물 완전성 검증

- **전체 SHA:** `c0f7434467a051f93273a7e850d7bf94cc97a215`
- **중요도:** A
- **태그:** VALIDATION, DEPLOY, TEST
- **확장 개발 과정에서의 역할:** 산출물 형식 회귀 — 서버 진입점과 생성된 정적 디렉터리의 존재를 명시적인 명령으로 검사합니다.
...
- **전체 SHA:** `c5e73853a1b69e39561748435f4768109a368544`
- **중요도:** A
- **태그:** VALIDATION, DEPLOY, TEST
- **확장 개발 과정에서의 역할:** CI promotion — 배포용 E2E 빌드 산출물에 독립 실행형 완전성 명령을 적용합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 개발 과정 5가 이 산출물 검증 규칙을 Docker 빌드 단계 선행 조건으로 사용하고, 실행 시점 스크립트가 실제 HTTP 응답과 공개 자산을 검증합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 개발 과정 5가 이 산출물 검증 규칙을 Docker 빌드 단계 선행 조건으로 사용하고, 실행 시점 스크립트가 실제 HTTP 응답과 공개 자산을 검증합니다.
...
| 브라우저 테스트는 성공하지만 배포 가능한 레이아웃 누락 | E2E 뒤 별도 산출물 단계 | 동일 `.next` 파일 트리를 순차 검사 | 공개·컨테이너는 다음 개발 과정 |
...
배포용 빌드는 독립 실행형 서버 모드를 요청하고, 저장소 명령과 CI는 `server.js` 및 생성된 정적 디렉터리의 존재를 요구합니다. 이 계약은 산출물 형식만 다루며 공개 자산 복사, 직접 서버 시작, 실행 시점 사용자와 HTTP 응답은 다음 컨테이너 개발 과정이 맡습니다.
...
- [x] 공개 자산과 실제 독립 실행형 실행 시점이 다음 개발 과정 책임임을 연결했습니다.
- [ ] 해당 SHA 실행 명령을 직접 실행해 결과를 기록했습니다. — 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
...
webpack 배포 산출물에서 라우트별 클라이언트 JS/CSS를 계측하고 검토 가능한 기준선에 대해 초과 시 차단하는 허용량을 적용한 뒤, 배포용 서버의 데스크톱 Lighthouse 조합표와 함께 CI 배포 차단 기준로 승격하는 과정을 복원합니다.
...
- 기준선 생성과 처리 검사를 분리하지 않으면 어떤 self-approval 실패가 생기는가?
...
| 5 | `57a1b0876941` | build(perf): bundle budget CLI 연결 | B | PERF, DEPLOY | operational 분리 — 기준선 쓰기와 처리 검사를 별도 패키지 명령으로 노출합니다. |
| 6 | `6ac1ea4b5055` | chore(perf): route bundle 기준값 기록 | B | ROUTING, PERF | 검토 가능한 operational 상태 — 여덟 공개 라우트 패턴의 허용된 압축하지 않은 바이트 기준선을 커밋합니다. |
...
- **전체 SHA:** `c2fb8a7c238d355c717a14359ef805fb9cc7f6f7`
- **중요도:** A
- **태그:** ARCH, ROUTING, PERF
- **확장 개발 과정에서의 역할:** 생성 산출물 검증을 통과해야 하는 지점 — webpack 클라이언트 참조 래퍼에서 JSON 페이로드만 안전하게 추출합니다.
...
- **전체 SHA:** `c24c350ce42cdbfe35eff3c6dd3bebc62ab132aa`
- **중요도:** A
- **태그:** VALIDATION, DEPLOY, TEST
- **확장 개발 과정에서의 역할:** 결정적인 규칙 테스트 — webpack 빌드 스크립트와 생성된 래퍼 파서 테스트 입력을 함께 고정합니다.
...
- **전체 SHA:** `605b64512edfdf416335ee500355fb1008100168`
- **중요도:** A
- **태그:** ARCH, ROUTING, PERF
- **확장 개발 과정에서의 역할:** 배포 산출물 측정 — app 경로와 클라이언트 참조 명세 파일을 결합해 라우트별 압축하지 않은 JS/CSS 바이트를 계산합니다.
...
- **전체 SHA:** `518ff5b51ec54c4eb4fcca9bab5ff6ba8e70a67b`
- **중요도:** A
- **태그:** ARCH, ROUTING, PERF
- **확장 개발 과정에서의 역할:** 입력에만 의존하는 규칙 계산 함수 — 커밋된 라우트 검증 범위와 JS/CSS별 최대 5% 성장을 구조화된 위반로 판정합니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 전달 상태와 부족함 | 라우트별 바이트는 계산할 수 있었지만 허용된 참조, 허용 성장률, 라우트 검증 범위 불일치를 판정하는 규칙이 없었습니다. |
...
| 빌드·실행 시점·자원 소유 주체와 수명 | 입력에만 의존하는 계산 함수가 규칙 calculation을 소유하고 호출자가 측정·기준선 입력과 위반 처리 실행 주기를 소유합니다. 함수는 입력을 수정하거나 기준선을 갱신하지 않습니다. |
...
- **전체 SHA:** `57a1b0876941d2dc1e78f4439ef9dd4f4b9edb2a`
- **중요도:** B
- **태그:** PERF, DEPLOY
- **확장 개발 과정에서의 역할:** operational 분리 — 기준선 쓰기와 처리 검사를 별도 패키지 명령으로 노출합니다.

#### 해당 SHA에서 확인할 실제 코드

- `bundle:baseline`과 `bundle:check`의 argv 차이와 둘 다 기존 `.next` 측정을 먼저 수행한다는 순서를 확인합니다.
- `--write-baseline` 브랜치가 디렉터리 생성, formatted JSON 쓰기 후 반환하고 비교을 수행하지 않는다는 점을 기록합니다.
...
| 실제 변경 파일·심볼·명령·산출물 | `bundle:baseline`과 `bundle:check`를 추가했습니다. CLI는 측정을 출력하고, 명시적인 쓰기 모드에서는 스키마·원본·라우트를 `performance/route-budgets.json`에 씁니다. 일반 모드에서는 커밋된 기준선과 규칙을 읽어 위반을 모두 출력하고 종료 코드 1을 설정합니다. |
...
| 실패·누락된 산출물·정리 처리 | 누락된·유효하지 않은 기준선 JSON, 증가 한도가 5가 아님, 측정 실패는 예외 발생·reject합니다. 위반는 전부 출력된 뒤 종료 코드 1입니다. 쓰기 모드는 현재 numbers를 승인 여부 없이 overwrite할 수 있으므로 검토 절차가 규칙 소유 주체입니다. |
| 보장하는 것과 보장하지 않는 것 | 처리 검사는 기준선을 자동 갱신하지 않고 비교만 합니다. 기준선 변경은 명시적 명령과 변경 내용을 남겨야 합니다. 누가 기준선 갱신를 승인하는지, 빌드 선행 여부, schemaVersion 실행 시점 검증은 강제하지 않습니다. |
...
- **전체 SHA:** `6ac1ea4b5055cad7e1eef05d8a8e7811b5cb8807`
- **중요도:** B
- **태그:** ROUTING, PERF
- **확장 개발 과정에서의 역할:** 검토 가능한 operational 상태 — 여덟 공개 라우트 패턴의 허용된 압축하지 않은 바이트 기준선을 커밋합니다.
...
- 이 파일이 테스트 결과 증명서가 아니라 향후 검사의 허용된 설정이라는 소유 주체를 설명합니다.
...
| 빌드·실행 시점·자원 소유 주체와 수명 | 커밋된 JSON이 허용된 참조와 라우트 검증 범위를 소유합니다. generator가 만들었더라도 병합 검토 이후 operational 설정으로 기능합니다. |
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `abbd530368a0`의 CI `bundle:check`가 이 파일을 읽습니다. 기준선 갱신는 처리 검사가 아니라 별도 검토 가능한 커밋이어야 합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `abbd530368a0`의 CI `bundle:check`가 이 파일을 읽습니다. 기준선 갱신는 처리 검사가 아니라 별도 검토 가능한 커밋이어야 합니다.

### 7. `1529ccf225c1` — build(perf): desktop Lighthouse 실행 경계 추가

- **전체 SHA:** `1529ccf225c10028607b6a8963c3280f4ab56a42`
- **중요도:** A
- **태그:** PERF, DEPLOY
- **확장 개발 과정에서의 역할:** 배포 환경 lab 검사 단계 정의 — 10 URL 데스크톱 조합표, 중앙값 집계 방식과 배포 기준값을 설정합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `f1c72dfdd16a`가 원본 reports를 검토 가능한 요약으로 만들고, `4e8f95249481`이 설정 규칙을 단위 테스트하며, `abbd530368a0`이 CI에서 실행합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `f1c72dfdd16a`가 원본 reports를 검토 가능한 요약으로 만들고, `4e8f95249481`이 설정 규칙을 단위 테스트하며, `abbd530368a0`이 CI에서 실행합니다.

### 8. `f1c72dfdd16a` — build(perf): Lighthouse 결과 요약기 추가

- **전체 SHA:** `f1c72dfdd16a0be14f7c2a650bde9ee431ec27bf`
- **중요도:** B
- **태그:** PERF, DEPLOY
- **확장 개발 과정에서의 역할:** 측정 출처 — 원본 LHR 보고서를 URL별 실행값과 중앙값과 실행 환경 정보를 포함한 JSON으로 요약합니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 전달 상태와 부족함 | LHCI는 원본 reports와 통과·실패를 만들 수 있었지만 중앙값 값과 환경을 한 검토 가능한 저장소 파일로 정규화하는 명령이 없었습니다. |
...
- **전체 SHA:** `4e8f952494812b4da0125b19ce56518505392cad`
- **중요도:** A
- **태그:** VALIDATION, PERF, TEST
- **확장 개발 과정에서의 역할:** 규칙 회귀 테스트 모음 — 컴파일러·파서 단언문을 보존하며 Lighthouse 조합표와 허용량 arithmetic을 검증합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 개발 과정 2의 Tailwind 변환 수정이 이후 산출물의 정확성를 보강하고, `abbd530368a0`이 단위 테스트로 검증한 규칙을 실제 CI 명령으로 활성화합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 개발 과정 2의 Tailwind 변환 수정이 이후 산출물의 정확성를 보강하고, `abbd530368a0`이 단위 테스트로 검증한 규칙을 실제 CI 명령으로 활성화합니다.

### 10. `abbd530368a0` — ci: 검증된 bundle과 Lighthouse gate 활성화

- **전체 SHA:** `abbd530368a03b8dc40221d663377c1f45e254ee`
- **중요도:** A
- **태그:** VALIDATION, PERF, DEPLOY
- **확장 개발 과정에서의 역할:** 배포 통합 — 배포용 빌드 산출물에 독립 실행형, 번들과 Lighthouse 검사를 순차 적용합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | `a39856cf734a`는 별도 로컬 lab 근거를 기록하고, 개발 과정 5의 `b94fa6dd0118`이 같은 작업 정의 끝에 컨테이너 실행 환경 검사를 추가합니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** `a39856cf734a`는 별도 로컬 lab 근거를 기록하고, 개발 과정 5의 `b94fa6dd0118`이 같은 작업 정의 끝에 컨테이너 실행 환경 검사를 추가합니다.

### 11. `a39856cf734a` — chore(perf): 최종 lab 성능 측정 결과 기록

- **전체 SHA:** `a39856cf734a309a32f5c7f239ba6bec90e1c259`
- **중요도:** C
- **태그:** PERF
- **확장 개발 과정에서의 역할:** 생성된 근거 스냅샷 — 데스크톱 enforced 조합표와 별도 모바일 관찰 결과의 측정 결과를 환경과 함께 보존합니다.
...
| 기준선 governance | 허용된 상태 없음 | `57a1b0876941`이 쓰기·검사 분리 | `6ac1ea4b5055` 커밋된 참조 + CI 검사 | 검토 품질·절대 cap |
...
| 처리 검사가 기준선을 자동 승인 | 쓰기·검사 명령 분리 | CI는만 `bundle:check` | 수동 기준선 approval는 프로세스 책임 |
...
- [x] 기준선 쓰기와 처리 검사의 소유 주체를 분리했습니다.
...
- **전체 SHA:** `b87a2b4537418771530ae520df448ca84142f80c`
- **중요도:** A
- **태그:** DEPLOY
- **확장 개발 과정에서의 역할:** 배포 가능한 이미지 구분 지점 — 검증된 독립 실행형 산출물, 정적·공개 자산과 비관리자 실행 시점을 다단계 Dockerfile로 구성합니다.
...
- **전체 SHA:** `b94fa6dd0118322ff57dc84b180b1c179ca8a867`
- **중요도:** A
- **태그:** ARCH, VALIDATION, ROUTING
- **확장 개발 과정에서의 역할:** 전체 과정 실행 시점 검증 규칙 — 이미지 빌드부터 비관리자 식별 정보, HTTP 라우트·자산과 정리까지 자동화하고 CI에 연결합니다.
...
| 다음 전달 커밋 또는 관련 테스트 연결 | 개발 과정 3의 독립 실행형 규칙과 이 개발 과정의 Dockerfile을 실제 실행 근거로 연결하며, CI 작업 정의의 마지막 검사 단계가 됩니다. |
...
- **실행·테스트 증거:** 실행하지 않음. 현재 작업 환경에서는 `web/portfolio`의 Git 체크아웃, npm 의존성 파일 트리, Chromium 및 Docker 데몬을 사용할 수 없었습니다. GitHub 연결 도구로 해당 SHA의 커밋 변경 내용과 변경 후 원본을 검사했으며, 명령 성공 결과는 주장하지 않습니다.
- **다음 커밋 연결:** 개발 과정 3의 독립 실행형 규칙과 이 개발 과정의 Dockerfile을 실제 실행 근거로 연결하며, CI 작업 정의의 마지막 검사 단계가 됩니다.
...
외부 호스팅 제공자 설정, 이미지 등록부 공개, 오케스트레이터와 배포 후 운영 절차는 `web/portfolio` 브랜치 이력에 근거가 없으므로 개발 과정을 만들지 않습니다.
...
- 분류 범위와 개발 과정 5개는 유지했습니다. 커밋을 추가·삭제·이동하거나 중복 편입하지 않았습니다.
- 기존 문서 틀의 일반적인 확인 문구는 정확한 파일·함수·스크립트·작업 정의·산출물·실패·보장하지 않는 범위를 묻는 커밋별 과제로 교체했습니다.
- `5d903132306a`는 단순한 글꼴·CSS 수정에 포함하지 않았습니다. Next·ESLint·SWC의 패치 버전 계열 정렬과 GNU·musl 잠금 파일 조건을 다루는 프레임워크·네이티브 이식성 단계로 역할을 좁혀 개발 과정 2에 유지했습니다.
- Tailwind 수정 뒤의 광범위한 시각 회귀 테스트는 결과를 간접적으로 보호하지만 PostCSS 설정만을 격리한 테스트는 아닙니다. 따라서 커밋 목록에 중복 추가하지 않고 완료 기록에서 테스트 공백으로 명시합니다.
- 개발 과정 2·3·4의 실제 커밋은 이력에서 일부 교차합니다. 문서 순서는 원본 → 산출물 → 배포 차단 기준 → 실행 시점이라는 학습 의존성 순서이며, 각 개발 과정 내부 커밋 순서는 실제 브랜치 순서를 유지합니다.
...
- 개발 과정 1의 고정된 개발 도구 버전과 배포용 E2E가 이후 모든 빌드·CI 경로의 실행 기반입니다.
- 개발 과정 2의 webpack·CSS·글꼴 이식성이 개발 과정 4에서 측정하는 산출물을 실제 배포 가능한 결과로 만듭니다.
- 개발 과정 3의 독립 실행형 규칙이 개발 과정 5 Docker 빌드 단계의 입력이며, 개발 과정 5가 개발 과정 3에서 검증하지 않은 `public`과 실제 실행 시점을 확인합니다.
- 개발 과정 4의 CI 활성화 뒤 개발 과정 5가 컨테이너 검사 단계를 작업 정의 마지막 단계에 추가합니다.

## 문서 사용법

1. 개발 과정 목표와 커밋 목록을 먼저 읽습니다.
...
5. 마지막에 원본 → 빌드 → 산출물 → 검증 → 실행까지 이어지는 최종 전달 순서를 코드 없이 설명합니다.
===== END FILE: README.md =====
