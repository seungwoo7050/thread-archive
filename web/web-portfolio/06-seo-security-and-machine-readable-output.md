...
기존 초안은 쿼리 상태 URL 작성, 외부 기준 이동 방식, 콘텐츠 검증을 하나로 묶었습니다. 1단계에서는 분류 02/03이 소유하는 UI 이동 방식 커밋을 제거하고, 검색 수집기와 공개 대상 범위의 정확성에 직접 영향을 주는 콘텐츠 원본 라우트 무결성만 남겼습니다.
...
- 각 SHA에서 `src/lib/content-loader.ts`의 도우미 함수와 호출자 loop를 부모 커밋과의 차이로 확인했습니다.
- 외부·프로토콜 상대 URL이 이 개발 흐름의 검증 범위 밖이라는 점을 보장과 비보장으로 구분했습니다.
...
- `b380f56f5d90^`와 `b380f56f5d90`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
...
- **Importance:** A
- **Tags:** ARCH, CONTENT, VALIDATION
- **고정된 역할:** 라우트 검증 함수를 사이트 탐색과 전역 링크에 연결
...
- `6b9e10289b64^`와 `6b9e10289b64`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 프로젝트 참조와 프로젝트 내부 링크까지 무결성 검사 확장
...
- `08b4ac81739f^`와 `08b4ac81739f`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 원본 위치가 포함된 라우트 오류를 결정적으로 재현하는 회귀 테스트
...
- `3353032ba23b^`와 `3353032ba23b`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
| 후속 연결 | 이 개발 흐름의 실제 코드 경로는 로더에서 종료됩니다. UI 이동 방식과 실제 404 동작은 다른 분류·개발 흐름이 소유합니다. |
...
이 개발 흐름의 실제 코드 경로는 로더에서 종료됩니다. UI 이동 방식과 실제 404 동작은 다른 분류·개발 흐름이 소유합니다.
...
## 9. 최종 개발 흐름 상태와 실행 흐름

<!-- learner:start thread-final-state -->
**최종 상태**

이 개발 흐름이 끝나면 사이트 탐색, 전역 링크, 프로젝트 링크에 선언된 모든 내부 라우트를 콘텐츠 로딩 중 하나의 활성 라우트 목록과 대조합니다. 지원하지 않거나 비활성화된 주소가 있으면 유효한 포트폴리오 집계 객체를 만들지 않습니다. 외부 링크 속성과 실제 브라우저 탐색은 이 개발 흐름의 범위가 아닙니다.
...
기존 초안은 `428055be3e64`의 판정 함수만 링크 보안 개발 흐름 끝에 두어 실제 소유 주체와 실행 주기를 잃었습니다. 1단계에서는 모드·오류 형식부터 빌드 전 검사 통합과 회귀 테스트까지 독립 개발 흐름으로 분리하고, 중간 중요도 B 자리표시자 검사 함수와 중요도 S 전체 검증 함수를 복원했습니다.
...
- 중요도 S `002b642d52a3`의 previous 위험, 실패 시 차단하는 결정, 종류를 구분한 결과, remaining 부족한 부분을 깊게 기록했습니다.
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION
- **고정된 역할:** 보수적인 모드 결정과 구조화된 준비 상태 오류 형식 정의
...
- `b3bd671a3243^`와 `b3bd671a3243`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 원본 파일과 경로를 보존하는 재귀 자리표시자 탐색
...
- `741bbb4caab7^`와 `741bbb4caab7`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION
- **고정된 역할:** 공개 배포 출처 URL과 `/content/` 자산 경로 범위 검증
...
- `47b99d6256ef^`와 `47b99d6256ef`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION
- **고정된 역할:** 배포 가능한 프로젝트·연락처 URL 판정 함수 정의
...
- `428055be3e64^`와 `428055be3e64`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** S
- **Tags:** ARCH, CONTENT, VALIDATION
- **고정된 역할:** 하나라도 실패하면 차단하는 전체 배포 검증 지점 마련
...
- Previous 도우미 함수가 isolated utilities에서 하나 기준이 되는 공개 결과로 바뀌는 소유 주체 전환 효과를 기록합니다.
- 이 SHA에서 아직 자산·프로젝트·연락처 완전성이 없는 부족한 부분을 명시합니다.

확인 원칙:

- `002b642d52a3^`와 `002b642d52a3`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION
- **고정된 역할:** 배포 검증 결과에 포트폴리오 근거 완전성 검사 추가
...
- `bcd87ed856bf^`와 `bcd87ed856bf`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 대상별 준비 상태 검사를 완성하고 모드별 진입점 하나만 노출
...
- `71e7ece7208f^`와 `71e7ece7208f`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION, DEPLOY
- **고정된 역할:** 일반 npm 빌드에서 준비 상태 검사 강제
...
- `37c0dbc079ff^`와 `37c0dbc079ff`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 전체 준비 결과와 공개 출처 URL 검증을 회귀 테스트
...
- 같은 커밋의 `site-metadata.test.ts`는 검색 노출 개발 흐름에서 별도 관점으로 확인합니다.

확인 원칙:

- `fb3d18fd660b^`와 `fb3d18fd660b`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
| 보장·비보장 | 입력 경계·단위 회귀 검증 근거이며 CLI 빌드 전 검사 실행, 실제 파일 시스템 자산 존재 여부, 인증 정보가 포함된 URL, 로컬 프로젝트·연락처 href, 브라우저 검색 노출을 증명하지 않습니다. |
| 후속 연결 | 검색 노출 개발 흐름에서는 같은 커밋의 `site-metadata.test.ts`가 템플릿의 `noindex`·robots 설정과 배포 출처 URL 동작을 검증합니다. |
...
검색 노출 개발 흐름에서는 같은 커밋의 `site-metadata.test.ts`가 템플릿 noindex/robots와 배포 출처 URL 동작을 검증합니다.
...
# 개발 흐름: 라우트별 메타데이터와 기준 URL 식별 정보
...
기존 초안의 라우트 순서는 대체로 맞았지만 최초 요청 헤더 메타데이터가 후속 모드에 따라 동작하는 생성 함수로 대체되는 소유 주체 이전가 빠져 있었고, sitemap 테스트 커밋까지 한 개발 흐름에 섞여 있었습니다. 1단계에서는 `55b6061e0052`와 `67aabeab1553`을 추가해 실제 전환을 복원하고, `adc392157f70`은 sitemap 개발 흐름으로 이동했습니다.
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 콘텐츠에서 만든 사이트 메타데이터를 최상위 레이아웃에 적용
...
- `1f4f93ad9a0f^`와 `1f4f93ad9a0f`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 콘텐츠 모드에만 의존하는 사이트 메타데이터 생성 함수 분리
...
- `55b6061e0052^`와 `55b6061e0052`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 최상위 메타데이터 생성을 모드별 규칙과 검증된 배포 출처 URL으로 이전
...
- `67aabeab1553^`와 `67aabeab1553`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- `1c40645caead^`와 `1c40645caead`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** ROUTING, SEO
- **고정된 역할:** 공통 규칙을 홈·프로젝트 목록·동적 프로젝트 상세에 적용
...
- `844ff4d7abcb^`와 `844ff4d7abcb`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
| 실제 변경 파일·심볼·호출 경로 | 홈은 사이트 복사, 프로젝트는 프로젝트 히어로, 상세는 matched 프로젝트 제목·요약과 `article` 타입을 `createRouteMetadata`에 전달합니다. |
...
- **Importance:** B
- **Tags:** ROUTING, SEO
- **고정된 역할:** 소개·연락처·이력서에 라우트 메타데이터 적용

#### 해당 SHA에서 확인할 실제 코드

- 각 페이지의 활성화 여부 키와 메타데이터 원본 필드를 확인합니다.
- 소개 요약, 연락처 도입부·제목, 이력서 히어로 본문·제목이 생성 함수 입력으로 전달되는지 추적합니다.
- 비활성 페이지가 메타데이터 공개에서 `notFound()`되는지 확인합니다.

확인 원칙:

- `fd5ff532bfe9^`와 `fd5ff532bfe9`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** ROUTING, SEO
- **고정된 역할:** 여정과 인터뷰 근거까지 라우트 메타데이터 적용 완료
...
- `5632c5df9b47^`와 `5632c5df9b47`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
| 실제 변경 파일·심볼·호출 경로 | 두 페이지 모듈이 콘텐츠 도입부와 화면 구성 히어로 제목을 선택해 `/journey`, `/interview-map` 메타데이터를 만듭니다. |
...
- **고정된 역할:** 모든 공개 라우트가 실제로 내보내는 메타데이터를 현재 동작으로 고정
...
- `4358bcd34f2e^`와 `4358bcd34f2e`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
| 후속 연결 | 사이트맵·라우트 도우미 함수의 추가 규칙은 `adc392157f70`에서 검색 노출 개발 흐름 관점으로 검증됩니다. |
...
사이트맵·라우트 도우미 함수의 추가 규칙은 `adc392157f70`에서 검색 노출 개발 흐름 관점으로 검증됩니다.
...
기존 초안은 핵심 커밋을 포함했지만 최상위 메타데이터 소비자, 단위 회귀, 실행 중인 애플리케이션 회귀가 빠져 있었습니다. 1단계에서는 `67aabeab1553`, `fb3d18fd660b`, `166f05f7be06`을 추가하고, `adc392157f70`을 메타데이터 개발 흐름에서 이동해 검색 수집기 출력의 구현→통합→테스트 순서를 완성했습니다.
...
- `70b69f04e8c7 → adc392157f70`의 활성 라우트 sitemap 구현과 결정적인 입력 경계 테스트를 연결했습니다.
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 콘텐츠 모드에서 페이지별 검색 노출 지시문 생성
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 모드별 robots 생성 함수와 App Router robots 라우트 추가
...
- `cb61450ad922^`와 `cb61450ad922`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** CONTENT, SEO
- **고정된 역할:** 페이지 검색 노출 지시문을 실제 최상위 메타데이터 출력에 연결
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION, SEO
- **고정된 역할:** 메타데이터와 robots의 모드별 동작을 단위 테스트
...
- **Importance:** A
- **Tags:** CONTENT, VALIDATION, SEO
- **고정된 역할:** 실행 중인 애플리케이션에서 검색 노출 규칙 검증
...
- `166f05f7be06^`와 `166f05f7be06`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** A
- **Tags:** ARCH, ROUTING, SEO
- **고정된 역할:** 검증된 공개 화면에서 사이트맵 탐색 URL 생성
...
- `70b69f04e8c7^`와 `70b69f04e8c7`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** VALIDATION, ROUTING, SEO
- **고정된 역할:** 사이트맵 공개 범위와 라우트 URL 순서를 회귀 테스트로 고정

#### 해당 SHA에서 확인할 실제 코드

- `src/lib/site-metadata.test.ts`의 `describe("sitemap")` 두 경우를 확인합니다.
- 템플릿 모드 예상한 `[]`와 배포 환경 테스트 입력의 `interviewMap: false` 변경을 추적합니다.
- 예상한 URL 목록이 최상위→프로젝트→프로젝트 상세→remaining 페이지 순서인지 확인합니다.
- 같은 커밋의 라우트 메타데이터 도우미 함수 테스트는 메타데이터 개발 흐름의 도우미 함수 보강이며 이 개발 흐름에서는 sitemap 단언문만 해석합니다.

확인 원칙:

- `adc392157f70^`와 `adc392157f70`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
# 개발 흐름: 안전한 JSON-LD 직렬화와 구조화된 정보
...
- **Importance:** A
- **Tags:** SEO
- **고정된 역할:** 원시 스크립트를 삽입하는 유일한 지점과 안전한 직렬화 함수 마련
...
- `228e40a48d64^`와 `228e40a48d64`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** SEO
- **고정된 역할:** 사이트 소유자와 웹사이트를 하나의 연결된 그래프로 구성
...
- `ee98415be696^`와 `ee98415be696`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** SEO
- **고정된 역할:** 배포 모드에서만 최상위 레이아웃에 사이트 그래프 출력
...
- `ae4ec172e45a^`와 `ae4ec172e45a`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** SEO
- **고정된 역할:** 기준 프로젝트 레코드를 연결된 CreativeWork 정보로 변환
...
- `7e09745d409e^`와 `7e09745d409e`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **Importance:** B
- **Tags:** SEO
- **고정된 역할:** 두 프로젝트 상세 렌더링 경로에 구조화 데이터 출력
...
- `f7bd33a8b403^`와 `f7bd33a8b403`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **고정된 역할:** 구조화 데이터 의미·부정 주장·스크립트 문맥 안전성 보호
...
- `c5938ea4b4f8^`와 `c5938ea4b4f8`의 부모 커밋과의 차이와 변경 후 파일 트리를 기준으로 합니다.
- 후속 커밋이나 최종 HEAD의 도우미 함수·테스트를 이 SHA의 구현으로 소급하지 않습니다.
- 실행하지 않은 결과와 정적 검토를 구분합니다.
...
- **분리:** 기존 혼합 개발 흐름 01을 콘텐츠에 선언된 내부 라우트 무결성과 배포 출처 URL·공개 URL 안전성으로 나눴습니다.
- **분류 내부 이동:** `adc392157f70`은 라우트 메타데이터 개발 흐름에서 sitemap·검색 노출 개발 흐름으로 옮겼습니다.
- **추가한 주요 커밋:** 라우트 검증 회귀 `3353032ba23b`, 공개 준비 상태 연결 순서, 메타데이터 소유 주체 이전 `55b6061e0052 → 67aabeab1553`, 검색 노출 단위 테스트·E2E 근거를 추가했습니다.
- **의도적 재사용:** `55b6061e0052`는 메타데이터 형식과 검색 노출 지시문, `67aabeab1553`는 메타데이터·출처 URL 통합과 페이지별 검색 노출 통합, `fb3d18fd660b`는 배포 준비 상태 회귀와 검색 노출 회귀라는 서로 다른 파일·결정 관점에서 두 개발 흐름에 등장합니다. 같은 코드에 대한 설명은 중복하지 않습니다.
- **최종 검증 범위:** 개발 흐름 5개, 고유 SHA 31개입니다. URL 무결성, 공개 가능성 검증, 기준 메타데이터, 검색 수집기 규칙과 안전한 JSON-LD를 독립적인 개발 과정으로 다룹니다.
...
| 검증 | 결과 |
| --- | --- |
| 문서 틀·완료 파일 집합 | PASS — README와 개발 흐름 5개의 상대 경로가 정확히 일치합니다. |
