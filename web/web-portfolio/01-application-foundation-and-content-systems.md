...
- 이 문서의 개발 과정 묶음·목표·역할·확인 지점은 1단계 분류 검토에서 저장소 근거를 바탕으로 확정했습니다.
- 2단계에서는 이 확정 정보를 바꾸지 않고 학습자용 기록만 작성했습니다.
- 다른 브랜치나 최종 HEAD의 구현을 과거 SHA의 설명으로 소급하지 않습니다.

## 1. 개발 과정 목표

문서만 있던 저장소에 고정된 Next.js 애플리케이션 설정, 전역 스타일 진입점, 콘텐츠 집계 객체를 사용하는 첫 라우트가 추가되는 과정을 복원합니다.

### 계획된 핵심 불변 조건

- 실행에 필요한 구성은 패키지 스크립트, TypeScript/Next/PostCSS 설정, App Router 최상위 구성으로 명시됩니다.
- 전역 스타일은 하나의 `globals.css`를 최상위 레이아웃에서 불러와 적용합니다.
- 첫 라우트는 JSON을 직접 조립하지 않고 포트폴리오 집계 객체를 호출합니다.

## 2. 이 개발 과정을 이해하기 위한 핵심 질문

- 문서 저장소가 실행 가능한 애플리케이션으로 바뀌는 커밋은 무엇인가?
- `globals.css`가 추가된 시점과 실제 불러온 시점을 구분하면 무엇이 보이는가?
- 초기 라우트가 콘텐츠와 렌더러 사이에서 맡은 책임과 아직 맡지 않은 책임은 무엇인가?

## 3. 완료 기준

- 각 SHA를 부모 커밋과 비교하고, 변경 후 파일 트리에서 실제 파일과 심볼을 확인합니다.
...
- 개발 과정 마지막에는 불변 조건의 변화와 최종 실행 순서를 코드 없이 설명합니다.

## 4. 커밋 목록

| 순서 | 커밋 | 제목 | 중요도 | 태그 | 이 개발 과정에서의 역할 |
| ---: | --- | --- |:---: | --- | --- |
...
| 3 | `0a28cb050bc8` | style(theme): 포트폴리오 기본 디자인 토큰 추가 | B | RENDERER | 전역 스타일 값 도입 |
| 4 | `03c4e1f7b439` | feat(app): 콘텐츠 기반 디자인 홈 연결 | B | CONTENT | 첫 콘텐츠·렌더러 연결 |
...
- **중요도:** C
- **태그:** CONTENT
- **개발 과정에서의 역할:** 문서 기반 초기 상태
- **조사 깊이:** 개발 과정의 출발점을 이해하는 데 필요한 맥락과 후속 제약만 기록합니다.
...
- 먼저 `cce7dd020563^`와 `cce7dd020563`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
...
| 보장하는 것과 보장하지 않는 것 | 콘텐츠를 편집할 위치와 코드를 둘 위치는 제시하지만, 애플리케이션이 실제로 실행되는지는 보장하지 않습니다. |
| 다음 커밋 또는 관련 테스트 연결 | `448bc2510f34`가 이 문서 규칙에 따라 실행 가능한 Next 애플리케이션 구성을 추가합니다. |

#### 코드·실행 증거

정적 근거: `cce7dd020563`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `448bc2510f34` — build(next): 실행 가능한 애플리케이션 골격 구성

- **중요도:** A
- **태그:** DEPLOY
- **개발 과정에서의 역할:** 실행 가능한 애플리케이션 기반
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `448bc2510f34^`와 `448bc2510f34`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실패·누락·대체 처리 | 설정 파일이 생겨도 당시 페이지는 정적 자리표시자이며 콘텐츠 스키마·로더·실제 렌더러 통합은 없습니다. Node/npm 재현성 버전 고정도 이 개발 과정이 아니라 분류 08의 후속 책임입니다. |
...
정적 근거: `448bc2510f34`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 정적 증거: 해당 SHA의 변경 내용에서 패키지·설정·App Router 파일 추가를 확인했습니다. 저장소 명령은 로컬 체크아웃 부재로 실행하지 않았습니다.

### 3. `0a28cb050bc8` — style(theme): 포트폴리오 기본 디자인 토큰 추가

- **중요도:** B
- **태그:** RENDERER
- **개발 과정에서의 역할:** 전역 스타일 값 도입
...
- 먼저 `0a28cb050bc8^`와 `0a28cb050bc8`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 보장하는 것과 보장하지 않는 것 | 공용 디자인 토큰을 보장하지만 실제 적용은 다음 커밋까지 보장하지 않습니다. |
| 다음 커밋 또는 관련 테스트 연결 | `03c4e1f7b439`의 최상위 레이아웃 가져오기가 이 파일을 실제 애플리케이션 시작점에 연결합니다. |

#### 코드·실행 증거

정적 근거: `0a28cb050bc8`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `03c4e1f7b439` — feat(app): 콘텐츠 기반 디자인 홈 연결

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 첫 콘텐츠·렌더러 연결
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `03c4e1f7b439^`와 `03c4e1f7b439`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 다음 커밋 또는 관련 테스트 연결 | T2가 집계 객체 모델을, T3 이후가 화면 구성·다중 라우트 규칙을 확장합니다. |

#### 코드·실행 증거

정적 근거: `03c4e1f7b439`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
| 전역 토큰은 최상위 스타일시트에서 정의되고 레이아웃이 불러옵니다. | `0a28cb050bc8` → `03c4e1f7b439` | `src/app/globals.css`, `src/app/layout.tsx` | 토큰 의미의 시각 회귀는 이 개발 과정이 검증하지 않습니다. |
...
| --- | --- | --- | --- |
| 문서만 있고 실행에 필요한 구성이 없음 | `448bc2510f34` | 패키지·설정·App Router 최상위 구성 추가 | 이번 작업에서는 저장소 명령 미실행; 후속 분류 08 검증과 연결 |
...
개발 과정 종료 시점에는 Next App Router가 실행 구조를 갖고 전역 스타일과 사이트 메타데이터를 최상위에서 적용하며, 첫 페이지가 포트폴리오 집계 객체를 Design 홈 렌더러에 전달합니다. 다만 개발 도구 버전 고정, 배포 환경 기본 동작 확인, 실행 시점 콘텐츠 검증과 다중 디자인 라우팅은 별도 후속 책임입니다.
...
| --- | --- | --- | --- |
| 애플리케이션 실행 주기를 선택합니다. | `package.json` | 스크립트와 의존성 관계 | 명령 자체의 환경 오류는 이 개발 과정에서 실행 검증하지 않음 |
| 최상위 문서를 구성합니다. | `src/app/layout.tsx` | 사이트 메타데이터, 언어, 글꼴, 전역 CSS | 잘못된 원본 값은 당시 실행 시점 파싱 없이 소비 |
| 포트폴리오 집계 객체를 요청합니다. | `src/app/page.tsx` → `getPortfolioContent()` | 한 개 집계 객체 | 로더·스키마 실패 경로는 후속 개발 과정 |
| Design 홈을 렌더링합니다. | `DesignHomeRoute` | 콘텐츠와 고정된 디버그 값 | 다른 디자인·라우트 선택 없음 |

## 11. 학습 완료 확인

완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. 후속 분류 08의 개발 도구 버전·배포 환경 기본 동작 확인와 분류 07의 통합·시각 테스트가 실행·회귀 증거를 추가합니다. 이 개발 과정에서 저장소 명령은 실행하지 않았습니다.
===== END FILE: 01-runnable-next-application-boundary.md =====

===== BEGIN FILE: 02-portfolio-domain-and-aggregate-model.md =====
# 개발 과정: 포트폴리오 데이터 모델과 집계 객체
...
| 2 | `5eb01dfecabb` | feat(content): 링크와 프로젝트 도메인 정의 | B | CONTENT | 프로젝트·링크 기본 데이터 형식 |
...
- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 기본 식별 정보 원본과 타입 도입
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `efb1e2e26b74^`와 `efb1e2e26b74`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 보장하는 것과 보장하지 않는 것 | 기본 식별 정보 형식을 제공하지만 집계 객체·검증은 아직 없습니다. |
| 다음 커밋 또는 관련 테스트 연결 | `5eb01dfecabb`부터 프로젝트·링크 데이터 영역이 추가됩니다. |

#### 코드·실행 증거

정적 근거: `efb1e2e26b74`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `5eb01dfecabb` — feat(content): 링크와 프로젝트 도메인 정의

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 프로젝트·링크 기본 데이터 형식
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `5eb01dfecabb^`와 `5eb01dfecabb`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 보장하는 것과 보장하지 않는 것 | 향후 렌더러가 사용할 프로젝트·링크 기본 필드만 보장합니다. |
| 다음 커밋 또는 관련 테스트 연결 | `0d891d41cf4c`이 기술·경험·여정 데이터 영역을 추가합니다. |

#### 코드·실행 증거

정적 근거: `5eb01dfecabb`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `0d891d41cf4c` — feat(content): 기술과 여정 콘텐츠 모델 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 경력·기술·여정 원본 확장
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `0d891d41cf4c^`와 `0d891d41cf4c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `0d891d41cf4c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `8661cc00c45d` — feat(content): 연락과 이력 집계 모델 완성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 집계 객체 타입 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `8661cc00c45d^`와 `8661cc00c45d`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `8661cc00c45d`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `a365b3d19118` — feat(content): 정적 포트폴리오 콘텐츠 로딩

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** JSON 직접 가져오기 기반 조립
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `a365b3d19118^`와 `a365b3d19118`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a365b3d19118`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `0b134b1a6cf6` — feat(content): 여정 정렬과 콘텐츠 인덱스 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 전체 원본과 파생 목록 연결
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `0b134b1a6cf6^`와 `0b134b1a6cf6`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `0b134b1a6cf6`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 7. `7c95a6f387b4` — feat(content): 환경 링크를 반영한 콘텐츠 집계

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 초기 집계 객체 규칙 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `7c95a6f387b4^`와 `7c95a6f387b4`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `7c95a6f387b4`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
개발 과정 종료 시점에는 `getPortfolioContent()`가 모든 JSON 원본을 하나의 집계 객체로 묶고 여정 정렬, 기술 조회, 활성 항목 필터링, 환경 변수에 따른 href 결정을 수행합니다. 그러나 이 집계 객체는 실행 시점 스키마나 파일 간 무결성 검증 없이 원본을 단언문으로 신뢰합니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`, `dc07871c4d24`, `b77b386b344e`, `527b9f872333`이 이후 공개 항목, 복제 범위, 선택자·뷰 모델 결과를 테스트합니다. 이 개발 과정에서는 테스트 명령을 실행하지 않았습니다.
===== END FILE: 02-portfolio-domain-and-aggregate-model.md =====

===== BEGIN FILE: 03-presentation-contracts-for-multi-route-ui.md =====
# 개발 과정: 여러 라우트의 화면 문구와 섹션 규칙
...
한 개 홈 자리표시자에서 다섯 디자인과 여러 라우트가 공유하거나 따로 사용하는 문구, 섹션 순서, 지표 키, ARIA·빈 상태 문구까지 `presentation.json` 계약으로 성장하는 과정을 복원합니다.

### 계획된 핵심 불변 조건

- 라우트·디자인별 표시 문구와 섹션 순서는 렌더러 코드가 아니라 화면 구성 원본이 소유합니다.
- 개수 키, 섹션 ID, 빈 상태·ARIA 문구는 미리 정의된 값만 사용합니다.
- 자리표시자 추가와 실제 복사 완성을 구분하고, JSON 변경만으로 렌더러 지원을 과장하지 않습니다.

## 2. 이 개발 과정을 이해하기 위한 핵심 질문

- Design/Classic의 초기 홈 화면 구성 규칙에서 다섯 디자인·다중 라우트로 어떻게 확장되는가?
- 새 복사가 추가된 커밋과 실제 자리표시자가 완성된 커밋은 각각 무엇인가?
- 공용 문구와 디자인별 문구의 경계는 어디인가?
...
| 순서 | 커밋 | 제목 | 중요도 | 태그 | 이 개발 과정에서의 역할 |
| ---: | --- | --- |:---: | --- | --- |
| 1 | `7f5017b21d37` | feat(content): 디자인 홈 표현 모델 추가 | B | CONTENT | Design 홈 화면 구성 규칙 시작 |
| 2 | `04a810bb0ab4` | feat(content): 클래식과 공용 홈 표현 추가 | B | CONTENT | Classic·공용 홈 화면 구성 규칙 |
| 3 | `d21d53591b5c` | feat(content): 프로젝트 목록 표현 계약 정의 | B | CONTENT | 프로젝트 라우트 타입 규칙 |
| 4 | `d55a2017e725` | feat(content): 프로젝트 목록 화면 문구 추가 | B | CONTENT | 프로젝트 라우트 자리표시자 원본 |
| 5 | `d6468cbea9e2` | feat(content): 보조 페이지 표현 계약 정의 | B | CONTENT | 상세·소개·이력서·연락처 화면 구성 규칙 |
| 6 | `da3941184155` | feat(content): 상세 소개 이력 연락 문구 추가 | B | CONTENT | 초기 라우트 문구 원본 |
| 7 | `96c8ba5733f5` | feat(content): 공용 UI 표현 콘텐츠 구성 | B | CONTENT | 공용 ARIA·빈 상태 문구 |
...
| 11 | `61d1976cde0d` | feat(content): 프로젝트 목록 표현 콘텐츠 구성 | B | CONTENT | 다섯 디자인 프로젝트 라우트 문구 |
...
- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** Design 홈 화면 구성 규칙 시작
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `presentation.json`의 `defaultHomeTemplate`, 템플릿, `home.design`을 확인합니다.
- `types.ts`의 홈 섹션·개수 키·Design 소개 영역 타입을 확인합니다.

확인 원칙:

- 먼저 `7f5017b21d37^`와 `7f5017b21d37`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | Design/Classic 템플릿 ID와 Design 소개 영역 통계, 섹션 순서, 대표 복사를 화면 구성 원본·타입으로 이동합니다. |
...
| 보장하는 것과 보장하지 않는 것 | Design 홈의 데이터 기반 화면 구성 규칙을 보장하지만 여러 라우트 지원은 보장하지 않습니다. |
| 다음 커밋 또는 관련 테스트 연결 | `04a810bb0ab4`가 Classic 터미널과 공용 홈 문구를 추가합니다. |

#### 코드·실행 증거

정적 근거: `7f5017b21d37`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `04a810bb0ab4` — feat(content): 클래식과 공용 홈 표현 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** Classic·공용 홈 화면 구성 규칙
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `04a810bb0ab4^`와 `04a810bb0ab4`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | Classic 소개 영역·대표·터미널과 두 렌더러가 공유할 홈 섹션 복사를 화면 구성 원본에 추가합니다. |
...
| 보장하는 것과 보장하지 않는 것 | 두 초기 디자인에서 공용 문구와 전용 문구를 나누지만 완성된 editorial 콘텐츠는 아닙니다. |
| 다음 커밋 또는 관련 테스트 연결 | `d21d53591b5c`부터 프로젝트 라우트 규칙이 분리됩니다. |

#### 코드·실행 증거

정적 근거: `04a810bb0ab4`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `d21d53591b5c` — feat(content): 프로젝트 목록 표현 계약 정의

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 프로젝트 라우트 타입 규칙
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `ProjectPageContent`와 Design/Classic 소개 영역 통계, 터미널, 선택 목록·그룹 목록 형식을 확인합니다.
- `countKey`가 렌더러 계산 결과와 연결될 단순 문자열 유니언인지 확인합니다.

확인 원칙:

- 먼저 `d21d53591b5c^`와 `d21d53591b5c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 데이터·상태·자원의 소유 주체와 수명 | 화면 구성 타입이 라우트 문구 형식을 소유하지만 JSON 값은 다음 커밋까지 없습니다. |
...
정적 근거: `d21d53591b5c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `d55a2017e725` — feat(content): 프로젝트 목록 화면 문구 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 프로젝트 라우트 자리표시자 원본
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `d55a2017e725^`와 `d55a2017e725`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | Design/Classic 프로젝트 페이지의 소개 영역, 통계, 대표, 터미널, 선택 목록·그룹 목록 문구를 자리표시자 형태로 추가합니다. |
| 데이터·상태·자원의 소유 주체와 수명 | 라우트 문구는 JSON이 소유하지만 그룹은 빈 배열이고 실제 그룹 설명은 아직 연결되지 않습니다. |
...
정적 근거: `d55a2017e725`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `d6468cbea9e2` — feat(content): 보조 페이지 표현 계약 정의

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 상세·소개·이력서·연락처 화면 구성 규칙
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `d6468cbea9e2^`와 `d6468cbea9e2`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 데이터·상태·자원의 소유 주체와 수명 | `PresentationContent.pages`가 여러 라우트 문구 규칙을 소유합니다. |
...
| 다음 커밋 또는 관련 테스트 연결 | `da3941184155`가 실제 라우트 문구를 채웁니다. |

#### 코드·실행 증거

정적 근거: `d6468cbea9e2`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `da3941184155` — feat(content): 상세 소개 이력 연락 문구 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 초기 라우트 문구 원본
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `da3941184155^`와 `da3941184155`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 데이터·상태·자원의 소유 주체와 수명 | 라우트 문구 소유 주체가 컴포넌트에서 JSON으로 이동할 기반이 생깁니다. |
| 실패·누락·대체 처리 | 여러 값은 자리표시자이고 모든 컴포넌트가 즉시 이 원본을 소비한다는 보장은 없습니다. |
| 보장하는 것과 보장하지 않는 것 | 필수 라우트 문구 구조를 제공합니다. |
| 다음 커밋 또는 관련 테스트 연결 | `96c8ba5733f5`가 공용 UI 허용 값 집합과 템플릿 설명을 추가합니다. |

#### 코드·실행 증거

정적 근거: `da3941184155`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 7. `96c8ba5733f5` — feat(content): 공용 UI 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 공용 ARIA·빈 상태 문구
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `96c8ba5733f5^`와 `96c8ba5733f5`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | 공용 UI 문구와 ARIA 템플릿, 빈 상태 문구을 화면 구성 원본에 중앙화합니다. |
...
정적 근거: `96c8ba5733f5`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 8. `9a7d41edfad0` — feat(content): Design과 Classic 홈 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 초기 두 홈의 의미 있는 구성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `9a7d41edfad0^`와 `9a7d41edfad0`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 두 홈 화면 구성 규칙은 있었지만 빈 섹션 순서와 자리표시자 터미널·복사 때문에 실제 정보 구조가 정해지지 않았습니다. |
...
정적 근거: `9a7d41edfad0`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 9. `2b9b35d4b8de` — feat(content): 확장 디자인 홈 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** Editorial/Brutalist/Cinematic 계약
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `2b9b35d4b8de^`와 `2b9b35d4b8de`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | 세 추가 디자인의 셸·홈 섹션 순서, 소개 영역·동작, 디자인별 문구 형식을 추가합니다. |
...
| 다음 커밋 또는 관련 테스트 연결 | `8886459d1b0d`과 후속 라우트 문구 커밋이 자리표시자를 실제 콘텐츠로 바꿉니다. |

#### 코드·실행 증거

정적 근거: `2b9b35d4b8de`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 10. `8886459d1b0d` — feat(content): 공용 홈 섹션 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 공용 홈 문구 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `8886459d1b0d^`와 `8886459d1b0d`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 다음 커밋 또는 관련 테스트 연결 | `61d1976cde0d`부터 다섯 디자인의 프로젝트 라우트 문구가 완성됩니다. |

#### 코드·실행 증거

정적 근거: `8886459d1b0d`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 11. `61d1976cde0d` — feat(content): 프로젝트 목록 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 다섯 디자인 프로젝트 라우트 문구
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `61d1976cde0d^`와 `61d1976cde0d`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 프로젝트 라우트는 초기 두 디자인 자리표시자만 갖고 추가 디자인 화면 구성 규칙이 비어 있었습니다. |
| 실제 변경 파일·심볼·호출 경로 | 그룹 복사와 다섯 디자인의 소개 영역·아카이브 허용 값 집합을 실제 콘텐츠로 구성합니다. |
| 데이터·상태·자원의 소유 주체와 수명 | 프로젝트 라우트의 디자인별 문구는 화면 구성 원본이 소유합니다. |
...
정적 근거: `61d1976cde0d`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 12. `a6c72a6b3b34` — feat(content): 프로젝트 상세 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 프로젝트 상세 복사 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `a6c72a6b3b34^`와 `a6c72a6b3b34`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a6c72a6b3b34`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 13. `20dfc298375c` — feat(content): About과 Journey 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 소개·여정 설명 계약
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `pages.about.curation`과 `pages.journey` 소개 영역·설명·타임라인·now를 확인합니다.
- 상태·이유·결과 문구와 기준 문구를 확인합니다.

확인 원칙:

- 먼저 `20dfc298375c^`와 `20dfc298375c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 소개 선별 기록과 여정 설명이 독립 원본을 표시할 라우트 문구 계약이 부족했습니다. |
| 실제 변경 파일·심볼·호출 경로 | 선별 기록 설명·섹션 문구와 여정의 현재 상태·이유·결과·타임라인·현재 위치 허용 값 집합을 추가합니다. |
| 데이터·상태·자원의 소유 주체와 수명 | 라우트 문구는 화면 구성 원본이, 실제 주요 시점·프로젝트 해석은 데이터 영역·뷰 모델이 소유합니다. |
...
정적 근거: `20dfc298375c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 14. `13c8c52c54d9` — feat(content): Interview Map과 Resume 표현 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 이력서·인터뷰 맵 계약 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `pages.resume`의 경력·학력·안내·식별 정보·editorial/brutalist를 확인합니다.
- `pages.interviewMap`의 소개 영역·트랙·부족한 부분 문구와 빈 문구를 확인합니다.

확인 원칙:

- 먼저 `13c8c52c54d9^`와 `13c8c52c54d9`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `13c8c52c54d9`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 15. `a7a2000ff462` — feat(content): Contact 표현 콘텐츠와 최종 문서 형식 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 문서형 라우트 최종 구성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `a7a2000ff462^`와 `a7a2000ff462`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a7a2000ff462`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
| 공용 UI·빈 상태·ARIA 문구은 한 노드에서 공유합니다. | `96c8ba5733f5` | `presentation.ui` | 의미상 동작 자체는 별도 검증 |
...
| 자리표시자·빈 섹션이 정상 콘텐츠처럼 노출될 위험 | `9a7d41edfad0`, `8886459d1b0d`, 라우트 문구 커밋 | 실제 섹션 순서와 본문으로 교체 | 시각·콘텐츠 완전성 자체는 자동 증명하지 않음 |
...
| 실제 데이터 해석 | 혼재 가능 | 화면 구성 밖 선택자·뷰 모델 | T4·후속 뷰 모델 개발 과정 |

## 9. 개발 과정 최종 상태

개발 과정 종료 시점에는 다섯 디자인과 프로젝트, 프로젝트 Detail, 소개, 이력서, 연락처, 여정, 인터뷰 맵이 화면 구성 원본의 명시적 복사·섹션 계약을 공유합니다. JSON은 무엇을 말하고 어떤 순서로 보여 줄지를 소유하지만 실제 데이터 해석, DOM 의미 구조, 실행 시점 스키마와 시각 동작은 별도 계층이 책임집니다.
...
- 이 개발 과정은 화면 구성 데이터의 의미를 다루며 실행 시점 파싱과 렌더러 동작을 직접 보장하지 않습니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`은 다섯 디자인과 화면 구성 검증을, `b77b386b344e`·`527b9f872333`은 라우트별 뷰 모델·범위가 제한된 페이로드를 후속 검증합니다. 이번 작업에서 테스트는 실행하지 않았습니다.
===== END FILE: 03-presentation-contracts-for-multi-route-ui.md =====

===== BEGIN FILE: 04-selectors-links-and-derived-content-policy.md =====
# 개발 과정: 선택자·링크·파생 콘텐츠 규칙
...
| 9 | `ee2c118a76d6` | feat(content): 홈 링크를 배치 기준으로 선택 | B | CONTENT | 홈 소개 영역 소비자 전환 |
...
- 먼저 `eb988f5e09e4^`와 `eb988f5e09e4`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `eb988f5e09e4`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `ba8da56d3fcf` — feat(portfolio): 연락과 프로젝트 링크 선택기 추가

- **중요도:** A
- **태그:** CONTENT
- **개발 과정에서의 역할:** 링크·배포 상태 규칙 중앙화
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `ba8da56d3fcf^`와 `ba8da56d3fcf`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실패·누락·대체 처리 | 아직 `placements`가 없어 카드·상세·소개 영역 문맥을 링크 타입으로 추정합니다. URL 라우트 무결성도 검사하지 않습니다. |
...
정적 근거: `ba8da56d3fcf`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 후속 `dc07871c4d24`가 공개 항목 표시 영역을, `b77b386b344e`가 선택자 결과를 테스트합니다.

### 3. `3e2e95a3a28c` — feat(content): 페이지 활성화 selector 추가

- **중요도:** B
- **태그:** CONTENT, ROUTING
- **개발 과정에서의 역할:** 페이지 활성화 여부 선택자
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `3e2e95a3a28c^`와 `3e2e95a3a28c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `3e2e95a3a28c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `7c539b142d6d` — feat(content): 프로젝트 지표 selector 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 선언형 지표 계산 함수
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `7c539b142d6d^`와 `7c539b142d6d`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `7c539b142d6d`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `daa6815a6dfa` — feat(project): 카드 링크를 콘텐츠 배치 기준으로 선택

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 카드 노출 위치 최초 소비
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `daa6815a6dfa^`와 `daa6815a6dfa`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실패·누락·대체 처리 | 노출 위치 누락은 제외이며 오류가 아닙니다. 다른 소개 영역·상세·연락처 소비자는 아직 이전 규칙일 수 있습니다. |
| 보장하는 것과 보장하지 않는 것 | 프로젝트 카드가 명시적인 노출 위치를 따르는 것을 보장합니다. |
| 다음 커밋 또는 관련 테스트 연결 | `119ff9a92090`이 공용 노출 위치 값 집합·선택자를 정리합니다. |

#### 코드·실행 증거

정적 근거: `daa6815a6dfa`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `383a3b86e119` — feat(content): 프로젝트 지표를 화면에 적용

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 지표 선택자 소비자 전환
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `383a3b86e119^`와 `383a3b86e119`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `383a3b86e119`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 7. `119ff9a92090` — feat(content): 링크 배치 selector 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 공용 LinkPlacement 허용 값 집합
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `LinkPlacement` 타입과 노출 위치 기반 선택자들을 확인합니다.
- 소개 영역·연락처·카드·상세·푸터별 선택과 데모 배포 중 검사가 어디에서 공유되는지 확인합니다.

확인 원칙:

- 먼저 `119ff9a92090^`와 `119ff9a92090`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `119ff9a92090`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 8. `2d87b62dcce8` — refactor(project): 상세 링크를 배치 기준으로 선택

- **중요도:** B
- **태그:** RENDERER, REFACTOR
- **개발 과정에서의 역할:** 상세 컴포넌트 소비자 전환
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `2d87b62dcce8^`와 `2d87b62dcce8`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 다음 커밋 또는 관련 테스트 연결 | `ee2c118a76d6`이 홈 소개 영역 소비자를 같은 규칙으로 전환합니다. |

#### 코드·실행 증거

정적 근거: `2d87b62dcce8`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 9. `ee2c118a76d6` — feat(content): 홈 링크를 배치 기준으로 선택

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 홈 소개 영역 소비자 전환
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- Design/Classic 홈 렌더러에서 링크 타입 기반 선택이 소개 영역 노출 위치 선택자로 바뀌는지 확인합니다.
- 배포 중 데모 검사와 대체 처리 동작이 유지되는지 확인합니다.

확인 원칙:

- 먼저 `ee2c118a76d6^`와 `ee2c118a76d6`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 홈 소개 영역이 링크 타입을 직접 해석해 상세·카드와 다른 규칙을 가질 수 있었습니다. |
...
정적 근거: `ee2c118a76d6`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

## 6. 불변 조건 변화 기록

| 추적할 불변 조건 | 도입·변화 SHA | 실제 소유 주체·근거 | 제한·후속 보호 |
| --- | --- | --- | --- |
| 대체 처리·제외·선택 규칙은 선택자가 소유합니다. | `eb988f5e09e4` → `ba8da56d3fcf` | `src/lib/portfolio/selectors.ts` | 알 수 없는 기술은 대체 처리, 누락된 참조는 제외 |
...
| 링크 타입이 문맥을 암묵적으로 결정 | `daa6815a6dfa`, `119ff9a92090` | 노출 위치 우선 선택자 | `3353032ba23b`의 카드·배포 중 선택자 회귀 |
...
개발 과정 종료 시점에는 기술·프로젝트·연락처·이력서 조회, 페이지 활성화 여부, 지표 계산, 배포 상태·배포 중 링크와 노출 위치 규칙이 선택자로 집중됩니다. 소비자는 데이터 영역 규칙을 재해석하지 않고 선택자 결과를 표시하지만, 누락된 설정의 상당수는 오류가 아니라 대체 처리·제외/0으로 처리됩니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`은 지표 차이 비교, 대표·이력서·카드·배포 중 선택자를, `b77b386b344e`와 `527b9f872333`은 라우트별 파생된·범위가 제한된 뷰 모델을 후속 테스트합니다. 이 작업에서는 실행하지 않았습니다.
===== END FILE: 04-selectors-links-and-derived-content-policy.md =====

===== BEGIN FILE: 05-runtime-schema-vocabulary.md =====
# 개발 과정: 실행 시점 스키마의 공통 값 집합
...
- 이 개발 과정이 검증하는 단일 파일 형식과 후속 로더가 검증하는 참조 무결성은 어디서 갈리는가?
...
- **개발 과정에서의 역할:** 실행 시점 검증 도구 기반
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- `package.json`과 잠금 파일에서 `zod`와 `tsx` 추가를 확인합니다.
- `zod`가 실행 의존성이고 `tsx`가 개발 명령 실행 도구인지 구분합니다.

확인 원칙:

- 먼저 `a1977dc7f026^`와 `a1977dc7f026`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a1977dc7f026`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `51ceb76ad88a` — feat(content): 콘텐츠 경로와 기본 식별자 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 공용 컴포넌트 허용 값 집합
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `51ceb76ad88a^`와 `51ceb76ad88a`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 데이터·상태·자원의 소유 주체와 수명 | `content-schema.ts`의 공용 컴포넌트가 후속 모든 객체 스키마의 입력 검증 지점를 소유합니다. |
...
정적 근거: `51ceb76ad88a`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `c2f3d376e96b` — feat(content): 사이트와 프로필 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 사이트·프로필 실행 시점 형식
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `c2f3d376e96b^`와 `c2f3d376e96b`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `c2f3d376e96b`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `857fa82a2030` — feat(content): 링크와 배포 상태 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 링크·배포 상태·이미지 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `857fa82a2030^`와 `857fa82a2030`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `857fa82a2030`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `f1163dc120bc` — feat(content): 프로젝트 분류와 지표 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 프로젝트 그룹·지표 허용 값 집합
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `f1163dc120bc^`와 `f1163dc120bc`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `f1163dc120bc`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `a944c73f0557` — feat(content): 프로젝트 사례 schema 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 완전한 프로젝트 카탈로그 스키마
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `a944c73f0557^`와 `a944c73f0557`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a944c73f0557`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 단일 프로젝트 레코드가 아닌 카탈로그 전체의 필수 필드와 최소 개수를 고정해 이후 로더·공개 모듈의 핵심 입력 규칙이 됩니다.

### 7. `d93ec9730edd` — feat(content): 기술과 경력 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 기술·기술 목록·경력 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `d93ec9730edd^`와 `d93ec9730edd`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `d93ec9730edd`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 8. `6fc79f058744` — feat(content): 여정과 연락 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 여정·링크 목록·연락처 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `6fc79f058744^`와 `6fc79f058744`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `6fc79f058744`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 9. `03aacfddc364` — feat(content): Resume 콘텐츠 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** résumé 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `03aacfddc364^`와 `03aacfddc364`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `03aacfddc364`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 10. `80152dae761f` — feat(content): 여정 narrative schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 여정 설명 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `80152dae761f^`와 `80152dae761f`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `80152dae761f`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 11. `51ce1c15a0e5` — feat(content): Interview Map 콘텐츠 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 인터뷰 근거 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `51ce1c15a0e5^`와 `51ce1c15a0e5`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `51ce1c15a0e5`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 12. `d0a62a7da4bd` — feat(content): 큐레이션 schema와 타입 export 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 선별 기록 스키마 및 스키마에서 파생된 원본 타입
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `d0a62a7da4bd^`와 `d0a62a7da4bd`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `d0a62a7da4bd`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 스키마가 실행 시점 검증과 정적 원본 타입을 함께 소유하는 방향으로 전환되어 후속 공개 모듈의 중복 타입 제거 기반이 됩니다.
...
개발 과정 종료 시점에는 모든 주요 데이터 영역 JSON 파일의 알려진 형식이 Zod 스키마로 표현되고 프로젝트 카탈로그의 최소 개수까지 고정됩니다. 그러나 스키마는 아직 원본 가져오기에 연결되지 않았고, ID 고유성·파일 간 참조·내부 라우트·자산 존재는 보장하지 않습니다.
...
| 단계 | 담당 위치·호출 경로 | 입력·출력 | 실패·보장하지 않는 범위 |
| --- | --- | --- | --- |
| 원시 JSON 값을 스키마에 입력할 준비를 합니다. | `content-schema.ts` 공개 항목 | Zod 스키마 objects | 이 개발 과정에서는 아직 호출되지 않음 |
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b` 이후 유효하지 않은 스키마, 중복된·참조, 자산 경우가 테스트됩니다. 이 작업에서는 저장소 테스트 명령을 실행하지 않았습니다.
===== END FILE: 05-runtime-schema-vocabulary.md =====

===== BEGIN FILE: 06-runtime-presentation-schema-contracts.md =====
# 개발 과정: 화면 문구 스키마의 실행 시점 검증 규칙
...
- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 화면 구성 공용 컴포넌트와 섹션 고유성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `807214624c87^`와 `807214624c87`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `807214624c87`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `97ff48de55b8` — feat(content): 프로젝트 목록 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 다섯 디자인 프로젝트 페이지 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `97ff48de55b8^`와 `97ff48de55b8`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | 다섯 디자인의 소개 영역·통계·터미널·그룹 복사와 그룹 설명을 스키마로 정의합니다. |
...
정적 근거: `97ff48de55b8`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `42a81197af82` — feat(content): 표현 공용 UI schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 화면 구성 최상위·UI 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `42a81197af82^`와 `42a81197af82`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `42a81197af82`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `a3825a49a055` — feat(content): Design과 Classic 홈 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 초기 홈 디자인 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- Design/Classic 셸, 소개 영역, 섹션, 대표, 터미널 명령·출력 스키마를 확인합니다.
- 중첩된 strictness와 `home` outer 추가 필드 허용을 확인합니다.

확인 원칙:

- 먼저 `a3825a49a055^`와 `a3825a49a055`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `a3825a49a055`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `2a02781859b8` — feat(content): Editorial 홈 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** Editorial 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- Editorial 셸·홈 소개 영역·대표·대표·원칙·연락처 필드와 섹션 고유성 스키마 연결을 확인합니다.

확인 원칙:

- 먼저 `2a02781859b8^`와 `2a02781859b8`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `2a02781859b8`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `23d5297b42bd` — feat(content): Brutalist 홈 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** Brutalist 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `23d5297b42bd^`와 `23d5297b42bd`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `23d5297b42bd`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 7. `ad07ab4b31a9` — feat(content): Cinematic 홈 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** Cinematic 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- Cinematic 셸 subtitle, 홈 섹션·소개 영역·statement·포커스·연락처·프로젝트 사례 문구를 확인합니다.
- 홈 outer 추가 필드 허용 안의 Cinematic 노드가 엄격한인지 확인합니다.

확인 원칙:

- 먼저 `ad07ab4b31a9^`와 `ad07ab4b31a9`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `ad07ab4b31a9`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 8. `3c873e373bbb` — feat(content): 공용 홈 섹션 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 공용 홈 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `3c873e373bbb^`와 `3c873e373bbb`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `3c873e373bbb`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 9. `edcf1eaa6f71` — feat(content): About과 Contact 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 소개·연락처 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- 소개 영역·원칙·여정·기술 목록·선별 기록·editorial/brutalist와 연락처 사용 가능 여부·안내 디자인 필드를 확인합니다.
- 소개·연락처·페이지 outer `passthrough()`를 기록합니다.

확인 원칙:

- 먼저 `edcf1eaa6f71^`와 `edcf1eaa6f71`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 확인·기록 항목 | 학습자 기록 |
| --- | --- |
| 직전 상태와 부족함 | 소개과 연락처 라우트 문구가 실행 시점 스키마에 포함되지 않았습니다. |
...
정적 근거: `edcf1eaa6f71`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 10. `4eb0db7b9656` — feat(content): Interview Map 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 인터뷰 맵 스키마
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- 소개 영역, 트랙 문구·템플릿, 부족한 부분 ARIA/eyebrow 스키마를 확인합니다.
- 중첩된 엄격한와 라우트 추가 필드 허용을 구분합니다.

확인 원칙:

- 먼저 `4eb0db7b9656^`와 `4eb0db7b9656`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `4eb0db7b9656`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
- 소개 영역·설명 문구·타임라인·now 스키마와 중첩된 strictness를 확인합니다.

확인 원칙:

- 먼저 `7f3c16b50990^`와 `7f3c16b50990`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 데이터·상태·자원의 소유 주체와 수명 | 여정 라우트 문구 규칙은 스키마가 소유합니다. |
| 실패·누락·대체 처리 | 주요 시점의 시간 순서나 기준 프로젝트 참조는 검사하지 않습니다. |
| 보장하는 것과 보장하지 않는 것 | 여정 알려진 화면 구성 형식을 보장합니다. |
| 다음 커밋 또는 관련 테스트 연결 | `50b78a557344`가 프로젝트 상세·프로젝트 페이지 linkage를 추가합니다. |

#### 코드·실행 증거

정적 근거: `7f3c16b50990`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 12. `50b78a557344` — feat(content): 프로젝트 상세 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 프로젝트 상세 및 프로젝트 라우트 스키마 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `50b78a557344^`와 `50b78a557344`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `50b78a557344`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 13. `4e2454cfc9c4` — feat(content): Resume 표현 schema 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 이력서 화면 구성 스키마 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.

#### 해당 SHA에서 확인할 실제 코드

- 이력서 소개 영역·요약·프로젝트·교육·경력·학력·안내·식별 정보·editorial/brutalist 필드를 확인합니다.
- 라우트 노드 추가 필드 허용와 중첩된 strictness를 기록합니다.

확인 원칙:

- 먼저 `4e2454cfc9c4^`와 `4e2454cfc9c4`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `4e2454cfc9c4`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
개발 과정 종료 시점에는 다섯 디자인과 주요 라우트의 알려진 화면 구성 필드, 공용 UI·빈 상태 문구, 섹션 ID·개수 키·순서 결정 개수가 실행 시점 스키마로 표현됩니다. 다만 최상위와 선택된 outer 페이지 노드는 확장 키를 허용하며 기본 디자인 완전성, 라우트·프로젝트 참조, 렌더러 동작은 보장하지 않습니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`은 다섯 디자인과 유효하지 않은·지원하지 않는 디자인 경우를 후속 테스트합니다. 이 작업에서는 실행하지 않았습니다.
...
- 초기 예시 원본의 파일 간 ID는 서로 맞물려야 하지만 이 개발 과정 시점에는 관례로만 유지됩니다.
...
- **중요도:** B
- **태그:** CONTENT, REFACTOR
- **개발 과정에서의 역할:** 기존·새 프로젝트 원본 호환 경계
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `d717c35cf80c^`와 `d717c35cf80c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `d717c35cf80c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `6caa6debdb01` — feat(content): 사이트와 프로필 starter 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 식별 정보 초기 예시 원본
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `6caa6debdb01^`와 `6caa6debdb01`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `6caa6debdb01`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `f8ea6376c65c` — feat(content): 링크와 기술 starter 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 링크·기술 초기 예시 참조 관계
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `f8ea6376c65c^`와 `f8ea6376c65c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `f8ea6376c65c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 4. `247ad421101a` — feat(content): 프로젝트 starter 분류와 지표 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 카탈로그 envelope와 지표 초기 예시
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `247ad421101a^`와 `247ad421101a`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
| 실제 변경 파일·심볼·호출 경로 | 초기 예시 그룹 설명과 선언형 지표를 추가하고 새 카탈로그 envelope를 도입합니다. |
...
정적 근거: `247ad421101a`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 5. `c58a1be43009` — feat(content): 프로젝트 starter 상세 구성

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** 전체 초기 예시 프로젝트 경우
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `c58a1be43009^`와 `c58a1be43009`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `c58a1be43009`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `83bd41a353ce` — feat(content): Resume와 여정 starter 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 시간순 기록·résumé 초기 예시 참조
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `83bd41a353ce^`와 `83bd41a353ce`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `83bd41a353ce`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 7. `7ba62b311776` — feat(content): Interview Map과 큐레이션 starter 콘텐츠 구성

- **중요도:** B
- **태그:** CONTENT, RENDERER
- **개발 과정에서의 역할:** 근거·선별 기록 초기 예시 참조 관계
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `7ba62b311776^`와 `7ba62b311776`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `7ba62b311776`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
개발 과정 종료 시점에는 프로젝트 원본이 `{groups, metrics, items}` 문서로 전환되고 하나의 전체 초기 예시 경우가 기술, résumé, 여정, 인터뷰 맵, 선별 기록, 연락처·링크 원본과 일관된 ID 참조 관계를 이룹니다. 그러나 전환 브랜치는 단언문 기반이고 실행 시점 파싱·참조·자산 검증은 아직 없습니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. 후속 `03d2c9be0a43`와 T9 무결성 커밋이 이 초기 예시 원본을 실패 시 차단하는 검증 지점에 연결합니다. 이 작업에서는 원본 실행 검증을 수행하지 않았습니다.
===== END FILE: 07-starter-catalog-migration.md =====

===== BEGIN FILE: 08-source-aware-schema-parsing-boundary.md =====
# 개발 과정: 원본 위치를 보존하는 스키마 파싱 지점
...
14개 JSON 원본을 단순 단언문에서 파일명·JSON 경로가 포함된 구조화된 검증 오류와 단일 `loadPortfolioSource()` 파싱 지점로 전환하는 과정을 복원합니다.
...
- 먼저 `70e49ea34194^`와 `70e49ea34194`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `70e49ea34194`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 이후 모든 검증 경로와 테스트가 의존하는 공개 오류 타입과 원본을 교체할 테스트 지점을 정의합니다.

### 2. `830f02688d63` — feat(content): JSON 경로 진단 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 결정적인 JSON 경로 형식화 함수
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `830f02688d63^`와 `830f02688d63`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `830f02688d63`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `d50870c8b8c4` — feat(content): JSON schema 파싱 경계 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 일반적인 파일별 파서
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `d50870c8b8c4^`와 `d50870c8b8c4`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `d50870c8b8c4`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 단언문과 실행 시점 신뢰 사이의 실제 변환 함수가며 후속 로더 불변 조건의 핵심입니다.

### 4. `03d2c9be0a43` — feat(content): 콘텐츠 파일 schema 파싱 연결

- **중요도:** S
- **태그:** ARCH, CONTENT, VALIDATION
- **개발 과정에서의 역할:** 14개 원본을 검증하고 실패 시 중단하는 로더
...
- 먼저 `03d2c9be0a43^`와 `03d2c9be0a43`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `03d2c9be0a43`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 S 근거: 원본 신뢰 모델이 “JSON 가져오기 + `as`”에서 “알 수 없는 입력 + 정확한 스키마 파싱 + 구조화된 실패 + 검증된 집계 객체”로 바뀌며 이후 콘텐츠 설계 전체의 입력 검증 지점이 됩니다.
...
개발 과정 종료 시점에는 14개 JSON 원본이 각각 정확한 Zod 스키마를 통과한 `PortfolioSource`로만 반환되고, 기본 원본은 모듈 가져오기 시 검증에 실패하면 차단됩니다. 오류는 원본 파일과 JSON 형식 경로를 보존합니다. 그러나 중복된·참조·라우트·디자인·자산 무결성은 후속 개발 과정 책임입니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`이 테스트용 덮어쓰기를 사용해 잘못된 형식의 원본과 구조화된 오류를 검증합니다. 이 작업에서는 Vitest나 콘텐츠 명령을 실행하지 않았습니다.
===== END FILE: 08-source-aware-schema-parsing-boundary.md =====

===== BEGIN FILE: 09-repository-wide-content-integrity.md =====
# 개발 과정: 저장소 전체 콘텐츠 무결성
...
- 먼저 `f8a4aa2109e8^`와 `f8a4aa2109e8`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `f8a4aa2109e8`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 2. `b380f56f5d90` — feat(content): 내부 route 참조 검증 추가

- **중요도:** A
- **태그:** ARCH, CONTENT, VALIDATION
- **개발 과정에서의 역할:** 내부 라우트 규칙 도우미 함수
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `b380f56f5d90^`와 `b380f56f5d90`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `b380f56f5d90`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 콘텐츠 authoring과 라우팅·페이지 활성화 여부를 연결하는 설계 수준 규칙이며 여러 원본의 href가 이 한 도우미 함수를 공유합니다.

### 3. `b9d74d8ccf08` — feat(content): 콘텐츠 식별자 중복 검증 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 전역 식별자·순서 고유성
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `b9d74d8ccf08^`와 `b9d74d8ccf08`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `b9d74d8ccf08`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 원본별 형식 검증을 저장소 전체 namespace 불변 조건으로 확장하고 모든 후속 참조 검사의 기준 Sets를 만듭니다.

### 4. `b87da7ca505c` — feat(content): 지원 디자인 구성 검증 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 디자인 등록부 완전성
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `b87da7ca505c^`와 `b87da7ca505c`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `b87da7ca505c`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 다섯 렌더러의 콘텐츠 등록부가 부분 구성이나 지원하지 않는 진입점으로 배포되는 것을 원본 구분 지점에서 차단합니다.

### 5. `6b9e10289b64` — feat(content): 사이트와 링크 route 참조 검증 추가

- **중요도:** A
- **태그:** ARCH, CONTENT, VALIDATION
- **개발 과정에서의 역할:** 탐색·최상위 링크 라우트 통합
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `6b9e10289b64^`와 `6b9e10289b64`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `6b9e10289b64`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 사이트 셸과 전역 링크의 공개 라우팅 표시 영역을 콘텐츠 무결성 구분 지점에 연결합니다.

### 6. `08b4ac81739f` — feat(content): 프로젝트 내부 참조 검증 추가

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 프로젝트 그룹·기술 스택·링크 무결성
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `08b4ac81739f^`와 `08b4ac81739f`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `08b4ac81739f`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 가장 큰 데이터 영역 레코드의 내부 참조 관계와 공개 링크를 한 번에 검증에 실패하면 차단합니다.

### 7. `6514b4e0bcff` — feat(content): 지표와 Resume 참조 검증 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 지표·이력서 프로젝트 참조
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `6514b4e0bcff^`와 `6514b4e0bcff`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `6514b4e0bcff`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 8. `805072d7b610` — feat(content): 여정과 Interview 참조 검증 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION, RENDERER
- **개발 과정에서의 역할:** 여정·근거 프로젝트 참조
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `805072d7b610^`와 `805072d7b610`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `805072d7b610`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 9. `d5afc69ae9da` — feat(content): 큐레이션과 연락 참조 검증 추가

- **중요도:** B
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 선별 기록·연락처 참조 완성
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `d5afc69ae9da^`와 `d5afc69ae9da`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `d5afc69ae9da`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.
...
개발 과정 종료 시점에는 스키마 파싱된 원본이 중복된 식별자·순서, 지원하는 디자인 등록부, 내부 라우트, 프로젝트 그룹·기술 스택·링크, 지표·résumé/여정·인터뷰·선별 기록·연락처 참조를 통과해야만 반환됩니다. 가능한 무결성 문제는 한 배열에 누적되지만 파일별 스키마 파싱 실패는 앞 단계에서 즉시 중단됩니다. 외부 URL, 자산 존재, 모든 의미상 순서 결정·중복된 경우는 범위 밖입니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`은 중복된, 누락된 디자인, 지원하지 않는·비활성화된 라우트, 비활성화된 참조 등의 결정적인 덮어쓰기 경우를 후속 테스트합니다. 이 작업에서는 해당 테스트를 실행하지 않았습니다.
===== END FILE: 09-repository-wide-content-integrity.md =====

===== BEGIN FILE: 10-validated-facade-assets-and-build-gate.md =====
# 개발 과정: 검증된 공개 모듈·자산·빌드 차단 단계
...
- **중요도:** A
- **태그:** ARCH, CONTENT, VALIDATION
- **개발 과정에서의 역할:** 스키마에서 파생된 공개 모듈 타입 연결부
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `85df59454b46^`와 `85df59454b46`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `85df59454b46`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 스키마 모듈과 공개 포트폴리오 타입 표시 영역 사이의 소유 주체를 재정렬하는 설계 수준 리팩터링입니다.

### 2. `16bdf03ce979` — feat(content): 여정과 큐레이션 콘텐츠 타입 추가

- **중요도:** B
- **태그:** CONTENT
- **개발 과정에서의 역할:** new 설명 공개 모듈 타입
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `16bdf03ce979^`와 `16bdf03ce979`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `16bdf03ce979`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 3. `508e0b71024b` — refactor(content): 검증된 콘텐츠를 portfolio facade에 연결

- **중요도:** S
- **태그:** ARCH, CONTENT, VALIDATION
- **개발 과정에서의 역할:** 원본 가져오기 제거와 검증된 공개 모듈 전환
- **조사 깊이:** 설계 전환, 이전 신뢰·소유 주체 모델, 실패 경로, 실행 주기, 후속 회귀까지 깊게 복원합니다.
...
- 먼저 `508e0b71024b^`와 `508e0b71024b`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `508e0b71024b`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 S 근거: 실제 소비자 설계의 신뢰 경로를 원본 단언문에서 검증된 로더로 교체하고 전환·env 변경까지 제거하는 소유 주체 전환입니다.

### 4. `ff2ecadf3489` — feat(content): 저장소 자산 참조 경계 검증

- **중요도:** A
- **태그:** CONTENT, VALIDATION
- **개발 과정에서의 역할:** 공개 자산 파일 시스템 무결성
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 추가된 프로필 사진 자리표시자 SVG가 어떤 초기 예시 참조를 만족하는지 확인합니다.

확인 원칙:

- 먼저 `ff2ecadf3489^`와 `ff2ecadf3489`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `ff2ecadf3489`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 데이터 스키마로 표현할 수 없는 저장소 파일 시스템 상태를 콘텐츠 검증 규칙에 통합합니다.

### 5. `0e0ed9e50323` — build(content): 콘텐츠 검사 명령 추가

- **중요도:** B
- **태그:** CONTENT, DEPLOY
- **개발 과정에서의 역할:** 명시적인 콘텐츠 검증 명령
- **조사 깊이:** 이 커밋이 맡은 실제 구현 역할, 변경된 심볼, 상태·누락 처리와 다음 연결을 복원합니다.
...
- 먼저 `0e0ed9e50323^`와 `0e0ed9e50323`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `0e0ed9e50323`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다.

### 6. `28b0db56190f` — build(content): 콘텐츠 검사를 prebuild에 연결

- **중요도:** A
- **태그:** CONTENT, DEPLOY
- **개발 과정에서의 역할:** 빌드 차단 검사 단계
- **조사 깊이:** 주요 하위 기능의 결정 경로, 소유 주체, 실패 처리, 보장하지 않는 범위, 통합 근거를 구체적으로 복원합니다.
...
- 먼저 `28b0db56190f^`와 `28b0db56190f`를 비교합니다. 루트 커밋이면 부모가 없음을 명시합니다.
- 변경 후 파일 트리에 실제로 존재하는 파일과 심볼만 이 SHA의 사실로 사용합니다.
- 실행하지 않은 명령과 후속 테스트 결과를 직접 확인한 것처럼 기록하지 않습니다.
...
정적 근거: `28b0db56190f`를 부모 커밋과 비교하고 변경 후 파일 트리에서 위 파일·심볼을 확인했습니다. 실행 근거: 없음. 로컬 환경에서 GitHub 도메인 DNS가 차단되어 대상 브랜치 체크아웃과 저장소 명령 실행을 수행하지 못했고, GitHub 커밋·파일 조회로만 검토했습니다. 코드 발췌 판단: 별도 코드 블록은 넣지 않았습니다. 함수·필드·분기 관계를 위 기록에 최소 단위로 직접 명시했습니다. 중요도 A 근거: 검증을 선택적 도구에서 표준 배포 빌드의 선행 조건으로 승격합니다.
...
개발 과정이 끝나면 포트폴리오 공개 모듈은 검증된 `portfolioSource`만 사용하고 렌더러에 전달할 그룹·분류·화면 구성·여정·활성화 상태를 계산합니다. 저장소 로컬 자산은 별도 검증 함수가 `public` 최상위 디렉터리 안에 실제로 존재하는지 확인하며, 표준 `npm run build`는 `content:check`를 먼저 실행합니다. 깊은 복사, 원격 URL·MIME·이미지 디코딩, 심볼릭 링크의 실제 경로 검사, 모든 빌드 우회와 실제 명령 성공은 보장하지 않습니다.
...
완료했습니다. 모든 커밋은 해당 SHA의 부모와의 변경 차이·변경 후 파일 트리를 기준으로 기록했고, 직접 실행 근거와 정적 검토를 구분했습니다. `3353032ba23b`은 원본·자산·모델 검증, `dc07871c4d24`는 공개 항목과 복제 범위, `b77b386b344e`/`527b9f872333`은 라우트 뷰 모델·범위가 제한된 페이로드를 후속 테스트합니다. 이번 환경에서는 저장소 체크아웃이 없어 테스트·빌드를 실행하지 않았습니다.
...
- Next/App Router의 최초 실행에 필요한 구성와 콘텐츠 집계 객체를 소비하는 첫 라우트
...
- 기존 개발 과정 초안 6개는 핵심 신뢰 검증 전환을 빠뜨렸습니다. 데이터 스키마와 화면 구성 스키마를 분리하고, 파싱·저장소 전체 무결성·검증된 공개 모듈·빌드 차단 단계에 관한 개발 과정을 추가해 총 10개로 보강했습니다.
...
- 화면 구성만 변경한 커밋 `61d1976cde0d`, `a6c72a6b3b34`, `20dfc298375c`는 초기 카탈로그 개발 과정에서 화면 구성 개발 과정으로 옮겼습니다. `d55a2017e725`, `9a7d41edfad0`, `8886459d1b0d`, `13c8c52c54d9`도 빠진 중간 규칙과 완성 단계로 추가했습니다.
...
| 8 | [원본 위치를 포함한 스키마 파싱 지점](./08-원본 위치를 포함한-schema-parsing-boundary.md) | 4 |
