...
> **[확인 필요]** 이 블록은 `$BIN`(small-shell)을 직접 실행하지 않는다 — `$TIMEOUT`
> 하니스가 감싼 `/bin/sh -c 'exec sleep 30'`을 직접 background로 띄우고, 그 harness
> 프로세스를 TERM으로 죽인 뒤 그 아래 `sleep`이 함께 죽는지를 본다. diff만으로는
> 이것이 small-shell 자신의 자식 정리 로직을 검증하는 것인지, 아니면 위 timeout 테스트
> (`sleep 30 | cat | cat`에 1초 timeout을 걸어 124를 기대하는 케이스)를 신뢰하기 위해
> `tests/timeout-runner` 자체의 시그널 전파를 미리 검증해두는 것인지 diff만으로는
> 단정하기 어렵다. `tests/timeout-runner`의 실제 구현을 봐야 확정할 수 있다.

### 증명하는 것 / 증명하지 않는 것
이 정도 규모(60회 반복, FD 48개 한도)에서 누수가 없다는 것을 보인다. 무한히 오래
실행되는 세션에서의 누적 누수나, 이 Thread가 다루지 않는 다른 코드 경로(예: builtin
자체의 FD 사용)의 누수는 증명 범위 밖이다.
